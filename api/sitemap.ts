...
async function publicRows(path:string):Promise<DynamicRow[]>{const response=await fetch(`${SUPABASE_PROJECT_URL}/rest/v1/${path}`,{method:"GET",headers:{...supabaseRestHeaders(),accept:"application/json"},signal:AbortSignal.timeout(8000)});if(!response.ok)throw new Error(`SITEMAP_SOURCE_${response.status}`);const payload=await response.json();return Array.isArray(payload)?payload as DynamicRow[]:[];}
function toXml(entries:SitemapEntry[],origin:string):string{const unique=new Map<string,SitemapEntry>();for(const entry of entries){if(entry.path.startsWith("/"))unique.set(entry.path,entry);}const urls=[...unique.values()].map(entry=>{const lastmod=entry.lastmod?`<lastmod>${xmlEscape(new Date(entry.lastmod).toISOString())}</lastmod>`:"";const changefreq=entry.changefreq?`<changefreq>${entry.changefreq}</changefreq>`:"";const priority=typeof entry.priority==="number"?`<priority>${entry.priority.toFixed(1)}</priority>`:"";return `<url><loc>${xmlEscape(`${origin}${entry.path}`)}</loc>${lastmod}${changefreq}${priority}</url>`;}).join("");return `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${urls}</urlset>`;}
