...
s = replaceOnce(s,
  'class="min-h-screen bg-slate-950 text-slate-300 font-sans pb-20"',
  'class="min-h-screen bg-slate-950 text-slate-300 font-sans pb-20 overflow-x-hidden"',
  'valuation page overflow');
s = replaceOnce(s,
...
s = replaceOnce(s,
  'class="bg-slate-900 text-white py-16 mb-12"',
  'class="bg-slate-900 text-white py-10 sm:py-16 mb-8 sm:mb-12"',
  'valuation hero spacing');
s = replaceOnce(s,
  'class="text-4xl md:text-5xl font-bold mb-6"',
  'class="text-3xl sm:text-4xl md:text-5xl font-bold mb-4 sm:mb-6 text-balance leading-tight"',
  'valuation hero title');
s = replaceOnce(s,
  'class="text-xl text-slate-300 max-w-3xl mx-auto mb-8"',
  'class="text-base sm:text-lg md:text-xl text-slate-300 max-w-3xl mx-auto mb-8 text-pretty leading-relaxed"',
  'valuation hero copy');
s = replaceOnce(s,
  'class="flex justify-center gap-8 text-slate-300"',
  'class="grid grid-cols-1 min-[360px]:grid-cols-3 justify-center gap-5 sm:gap-8 text-slate-300 text-sm sm:text-base"',
  'valuation trust items');
...
s = replaceOnce(s,
  ...
  'remove file accessibility');

s = replaceOnce(s,
  'rows="3"\n                      class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"',
  'rows="4"\n                      aria-label="Ek notlar"\n                      class="w-full min-h-28 px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none resize-y"',
  'notes textarea');
...
s = replaceOnce(s,
  ...
  'allow invalid submit for validation feedback');
s = replaceOnce(s,
...
s = replaceOnce(s,
  ...
  '  onFileSelected(event: Event) {\n    const input = event.target as HTMLInputElement;\n    const incoming = Array.from(input.files || []);\n    const maxBytes = 50 * 1024 * 1024;\n    const allowed = incoming.filter((file) => file.size <= maxBytes);\n    const rejectedCount = incoming.length - allowed.length;\n\n    this.selectedFiles.update((current) => [...current, ...allowed].slice(0, 10));\n    input.value = "";\n\n    if (rejectedCount > 0) {\n      this.toastService.show(`${rejectedCount} dosya 50 MB sınırını aştığı için eklenmedi.`, "error");\n    } else if (incoming.length + this.selectedFiles().length > 10) {\n      this.toastService.show("En fazla 10 dosya seçebilirsiniz.", "error");\n    }\n  }',
