...
assert(!fs.existsSync('src/components/analytics-consent.component.ts'), 'retired custom customer consent popup must not return');
assert(!app.includes('AnalyticsConsentComponent'), 'root customer shell must not import the retired custom consent popup');
assert(!app.includes('<app-analytics-consent'), 'root customer shell must not mount a custom consent popup');
