...
for (const key of ['db_reservations_v2', 'db_partnerRequests_v2', 'db_feedbacks_v2', 'db_notifications', 'db_visits']) {
  if (!car.toLowerCase().includes(key.toLowerCase().replace('db_', ''))) fail(`legacy cache cleanup does not cover ${key}`);
...
if (!adminReservations.includes('BookingService')) fail('admin reservations must use BookingService');
