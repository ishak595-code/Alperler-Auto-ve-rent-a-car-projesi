...
for (const token of ['<option value="PAYTR">PayTR</option>','<option value="IYZICO">iyzico</option>','<option value="NONE">Online kart kapalı</option>','PayTR Güvenli Bağlantı','iyzico Sandbox Bağlantısı','iyzico Canlı Bağlantısı','iyzico Fraud Bildirim / IFN URL','iyzicoFraudNotificationUrl()','autocomplete="new-password"']) must(paymentAdmin,token,`Admin no-code provider setup missing ${token}`);
