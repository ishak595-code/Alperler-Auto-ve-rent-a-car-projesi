...
for(const token of ['service_record_offline_payment_v221','p_actor:admin.id','x-request-id','requestId(request)','byCurrency','receivablesByCurrency','pendingReceivablesCount','save_message_template','sendPaymentNotification','service_notification_provider_secret_status_v2213','save_notification_provider','clear_notification_provider','service_set_notification_provider_secrets_v2213','service_clear_notification_provider_secrets_v2213','payment_transaction_id','PAYMENT_LEDGER_REVERSAL_REQUIRED','AUTOMATIC_LEDGER_VOID_FORBIDDEN','VOID_REASON_REQUIRED','externalReference=`manual:${idempotencyKey}`','finance_transactions?source=eq.MANUAL&external_reference=eq.'])must(financeEdge,token,`Finance edge contract missing ${token}`);
...
for(const token of ['FinanceCurrencySummary','byCurrency','receivablesByCurrency','messageTemplates','saveMessageTemplate','FinanceNotificationProviders','notificationProviders','saveResendProvider','saveTwilioProvider','clearNotificationProvider','StableWriteAttempt','offlinePaymentAttempt','manualFinanceAttempt','fingerprint','requestId:crypto.randomUUID()','this.action({action:\'record_payment\'','this.action({action:\'create_transaction\'',"this.offlinePaymentAttempt=null","this.manualFinanceAttempt=null"])must(financeService,token,`Finance client contract missing ${token}`);
...
for(const token of ['Admin > Muhasebe & Tahsilat','E-posta ve SMS Bağlantıları','Resend','Twilio','Supabase Vault','Ofiste ödeme','ödeme makbuzu','payment_transactions','finance_transactions','notification_deliveries','Messaging Service','Sender Pool','e-posta ve SMS metinleri','aynı değiştirilmemiş tahsilat formunun yeniden gönderimi aynı request-id kullanır','sıradan manuel muhasebe satırı gibi `VOID` yapılamaz'])must(operatorDocs,token,`Notification operator documentation missing ${token}`);
