...
for(const token of ['service_record_offline_payment_v221','p_actor:admin.id','byCurrency','receivablesByCurrency','pendingReceivablesCount','save_message_template','sendPaymentNotification','service_notification_provider_secret_status_v2213','save_notification_provider','clear_notification_provider','service_set_notification_provider_secrets_v2213','service_clear_notification_provider_secrets_v2213'])must(financeEdge,token,`Finance edge contract missing ${token}`);
...
for(const token of ['FinanceCurrencySummary','byCurrency','receivablesByCurrency','messageTemplates','saveMessageTemplate','FinanceNotificationProviders','notificationProviders','saveResendProvider','saveTwilioProvider','clearNotificationProvider'])must(financeService,token,`Finance client contract missing ${token}`);
