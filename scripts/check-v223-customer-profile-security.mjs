...
for(const token of ['AccountFavoritesV213Component','AccountProfileSettingsV225Component','AccountDashboardV150Component','@switch (section())',"@case ('favorites')","@case ('profile')",'<app-account-favorites-v213>','<app-account-profile-settings-v225>','<app-account-dashboard-v150>']) must(shell,token,`Customer account shell canonical ownership missing ${token}`);
...
for(const token of ['AccountSecurityV223Component','readonly securityOpen=signal(false)','(click)="toggleSecurity()"','[attr.aria-expanded]="securityOpen()"','@if(securityOpen())','<app-account-security-v223 />','Güvenlik Ayarlarını Aç']) must(profile,token,`Profile security lazy ownership missing ${token}`);
...
if(failures.length){console.error('V223/V225 customer profile security: FAIL');for(const failure of failures)console.error(`- ${failure}`);process.exit(1);}
console.log('V223/V225 customer profile security: PASS');
