...
for(const token of ['AccountSecurityV223Component','Hesap Güvenliği','<app-account-favorites-v213>','<app-account-security-v223>','<app-account-dashboard-v150>']) must(shell,token,`Customer account shell security ownership missing ${token}`);
const favoritesAt=shell.indexOf('<app-account-favorites-v213>');
const securityAt=shell.indexOf('<app-account-security-v223>');
const dashboardAt=shell.indexOf('<app-account-dashboard-v150>');
if(!(favoritesAt>=0&&securityAt>favoritesAt&&dashboardAt>securityAt))failures.push('Customer account order must remain Favorites, Security, then transaction dashboard.');
...
if(failures.length){console.error('V223 customer profile security: FAIL');for(const failure of failures)console.error(`- ${failure}`);process.exit(1);}
console.log('V223 customer profile security: PASS');
