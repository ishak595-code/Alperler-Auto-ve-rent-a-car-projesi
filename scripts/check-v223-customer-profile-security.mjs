...
for(const token of ['CustomerAuthService','this.auth.changePassword(this.newPassword)','autocomplete="new-password"','t().accountSecurity.confirmPassword','t().accountSecurity.title','t().accountSecurity.hint','role="alert"','role="status"']) must(security,token,`Customer security UI contract missing ${token}`);
for(const token of ['confirmPassword: "Yeni parola tekrar"', 'title: "Parolanızı güvenle yönetin"', 'hint: "En az 10 karakter, bir büyük harf, bir küçük harf ve bir rakam kullanın. Veri sızıntılarında görülen parolalar kabul edilmez. Daha önce başka hesaplarda kullandığınız parolaları tercih etmeyin."']) must(ui,token,`TR dictionary must keep accountSecurity label: ${token}`);
