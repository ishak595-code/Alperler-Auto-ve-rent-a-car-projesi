...
const ui = readFileSync('src/services/ui.service.ts', 'utf8');
...
for (const token of ['tour.duration','tour.locationName','tour.capacity','t().tourCatalog.perPerson']) requireText(tourCatalog, token, `Tour cards must expose the factual decision signal: ${token}`);
requireText(ui, 'perPerson: "Kişi başı"', 'TR dictionary must keep tourCatalog.perPerson.');
...
requireText(tourCatalog, 'resultsCountLabel()', 'Paginated tour catalog must describe the loaded result count truthfully via i18n.');
requireText(tourCatalog, 't().tourCatalog.resultsCount', 'Paginated tour catalog must bind result count to tourCatalog.resultsCount.');
requireText(ui, 'resultsCount: "{n} tur gösteriliyor"', 'TR dictionary must keep tourCatalog.resultsCount.');
