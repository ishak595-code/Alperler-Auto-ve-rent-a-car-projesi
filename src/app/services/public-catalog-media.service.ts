...
export type PublicCatalogMediaOwnerType = "VEHICLE" | "TOUR" | "BLOG";
...
export class PublicCatalogMediaService {
  ...
  load(ownerType: PublicCatalogMediaOwnerType, ownerId: string): Promise<PublicCatalogMediaItem[]> {
    if (ownerType === "TOUR") return this.loadForTour(ownerId);
    if (ownerType === "BLOG") return this.loadForBlog(ownerId);
    return this.loadForVehicle(ownerId);
  }

  loadForVehicle(vehicleId: string): Promise<PublicCatalogMediaItem[]> {
    return this.loadOwned("vehicle_id", vehicleId);
  }

  loadForTour(tourId: string): Promise<PublicCatalogMediaItem[]> {
    return this.loadOwned("tour_id", tourId);
  }

  loadForBlog(blogPostId: string): Promise<PublicCatalogMediaItem[]> {
    return this.loadOwned("blog_post_id", blogPostId);
  }
  ...
  hydrate(records: Vehicle[], media: PublicCatalogMediaItem[]): Vehicle[] {
    ...
    return records.map((record) => {
      const cloudId = String(record.cloudId || record.id || "").trim();
  ...
  private loadOwned(column: "vehicle_id" | "tour_id" | "blog_post_id", ownerId: string): Promise<PublicCatalogMediaItem[]> {
    const id = String(ownerId || "").trim();
    if (!id) return Promise.resolve([]);
    return this.loadPath(`catalog_media?is_active=eq.true&${column}=eq.${encodeURIComponent(id)}&select=${this.select}&order=sort_order.asc,created_at.asc`);
  }
  ...
  private fromRow(row: PublicCatalogMediaRow): PublicCatalogMediaItem | null {
    ...
    return {
      ...
      kind,
      url,
      posterUrl: row.poster_url ? this.resolveMediaUrl(row.poster_url) : undefined,
      sourceUrl: this.safeExternalUrl(row.source_url || "") || undefined,
  ...
  private resolveMediaUrl(value: string): string {
    const trimmed = String(value || "").trim();
    if (!trimmed) return "";
    if (trimmed.startsWith("/catalog-media/")) return trimmed;
    return this.safeExternalUrl(trimmed);
  }
