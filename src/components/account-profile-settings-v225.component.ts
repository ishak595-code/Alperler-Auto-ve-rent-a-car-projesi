...
@Component({
  ...
  template:`
    <section class="profile-settings" aria-labelledby="profile-settings-title">
      <header class="section-head"><div><p>PROFİL AYARLARI</p><h2 id="profile-settings-title">Bilgilerim</h2><span>Rezervasyonlarda kullanacağınız iletişim ve adres bilgilerinizi güncel tutun.</span></div><a routerLink="/account/wallet">Cüzdan ve Belgeler</a></header>
    ...
    <section class="security-launch" aria-labelledby="security-launch-title">
      <div>
        <p>HESAP GÜVENLİĞİ</p>
        <h2 id="security-launch-title">Parola ve güvenlik</h2>
        <span>Güvenlik ayarlarını yalnız ihtiyaç duyduğunuzda açın.</span>
      </div>
      <button type="button" (click)="toggleSecurity()" [attr.aria-expanded]="securityOpen()" aria-controls="account-security">
        {{securityOpen()?'Güvenliği Kapat':'Güvenlik Ayarlarını Aç'}}
      </button>
    </section>
    @if(securityOpen()){
