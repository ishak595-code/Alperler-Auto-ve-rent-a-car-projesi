      ...
      <div class="security-card">
        <header>
          <div>
            ...
            <h2 id="account-security-title">Parolanızı güncelleyin</h2>
            <span>Hesabınızı korumak için güçlü ve yalnız burada kullandığınız bir parola tercih edin.</span>
          </div>
        ...
        @if (open()) {
          <form id="account-password-form" (ngSubmit)="save()" novalidate>
            <label><span>Yeni parola</span><input [(ngModel)]="newPassword" name="accountNewPassword" type="password" autocomplete="new-password" minlength="10" required /></label>
            <label><span>Yeni parola tekrar</span><input [(ngModel)]="confirmPassword" name="accountConfirmPassword" type="password" autocomplete="new-password" minlength="10" required /></label>
...
export class AccountSecurityV223Component {
  ...
  toggle(): void { this.open.update((value) => !value); this.error.set(null); this.message.set(null); if (!this.open()) this.clearFields(); }
  async save(): Promise<void> {
    if (this.saving()) return;
    this.error.set(null); this.message.set(null);
    if (!this.newPassword || !this.confirmPassword) { this.error.set('Yeni parola alanlarını doldurun.'); return; }
    if (this.newPassword !== this.confirmPassword) { this.error.set('Yeni parolalar birbiriyle eşleşmiyor.'); return; }
    this.saving.set(true);
    try {
      if (!(await this.auth.changePassword(this.newPassword))) { this.error.set(this.auth.lastError() || 'Yeni parola kaydedilemedi.'); return; }
      this.clearFields(); this.open.set(false); this.message.set('Parolanız güncellendi.');
    } finally { this.saving.set(false); }
  }
  private clearFields(): void { this.newPassword = ''; this.confirmPassword = ''; }
}
