      ...
      @if(!open()){
        ...
        <section class="panel" aria-labelledby="customer-360-title">
          ...
          @else if(service.summary();as s){
            ...
            <section class="block"><h3>İlişki geçmişi</h3><dl><div><dt>Müşteri başlangıcı</dt><dd>{{s.customerSince|date:'dd.MM.yyyy'}}</dd></div><div><dt>Müşteri süresi</dt><dd>{{tenure()}}</dd></div><div><dt>İlk kiralama</dt><dd>{{s.firstRentalAt?(s.firstRentalAt|date:'dd.MM.yyyy'):'-'}}</dd></div><div><dt>Kiralama geçmişi</dt><dd>{{rentalTenure()}}</dd></div><div><dt>Son kiralama</dt><dd>{{s.lastRentalAt?(s.lastRentalAt|date:'dd.MM.yyyy'):'-'}}</dd></div><div><dt>İlk tamamlanan</dt><dd>{{s.firstCompletedAt?(s.firstCompletedAt|date:'dd.MM.yyyy'):'-'}}</dd></div><div><dt>Son tamamlanan</dt><dd>{{s.lastCompletedAt?(s.lastCompletedAt|date:'dd.MM.yyyy'):'-'}}</dd></div><div><dt>Referral indirimi</dt><dd>{{s.referralDiscountUses}} kullanım</dd></div></dl></section>
...
export class AdminCustomerLifetimePanelComponent implements OnInit{
  ...
  rentalTenure():string{const s=this.service.summary();if(!s||!s.firstRentalAt)return'Kiralama yok';return s.rentalTenureFullYears>0?`${s.rentalTenureFullYears} yıl ${s.rentalTenureMonths%12} ay`:s.rentalTenureMonths>0?`${s.rentalTenureMonths} ay`:'1 aydan az';}
