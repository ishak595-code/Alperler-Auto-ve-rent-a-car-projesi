      ...
      <div class="fixed inset-0 z-[200] flex flex-col bg-black text-white" role="dialog" aria-modal="true" aria-label="Fotoğraf ve video galerisi">
        ...
        <footer class="border-t border-white/10 bg-black/90 px-3 py-3">
          <div class="mx-auto flex max-w-6xl gap-2 overflow-x-auto pb-1">
            @for (item of items(); track item.id; let i = $index) {
              <button type="button" (click)="goTo(i)" class="relative h-16 w-24 shrink-0 overflow-hidden rounded-lg border-2 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400" [class.border-blue-500]="activeIndex() === i" [class.border-transparent]="activeIndex() !== i" [attr.aria-label]="(i + 1) + '. medyaya git'">
                @if (item.type === 'VIDEO') {
...
export class CatalogMixedGalleryComponent implements OnChanges {
  ...
  safeYouTubeUrl(url: string): SafeResourceUrl {
    ...
    const id = this.youtubeId(url); if (!id) return this.sanitizer.bypassSecurityTrustResourceUrl("about:blank");
    const safe = this.sanitizer.bypassSecurityTrustResourceUrl(`https://www.youtube-nocookie.com/embed/${id}?rel=0&modestbranding=1`); this.youtubeCache.set(url, safe); return safe;
  }

  private youtubeId(url: string): string | null {
    try {
      const parsed = new URL(url); const host = parsed.hostname.toLowerCase().replace(/^www\./, ""); let candidate = "";
