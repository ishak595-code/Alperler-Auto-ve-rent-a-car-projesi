  ...
  template: `
    @if (items().length && inlineLauncher) {
      <button
        type="button"
        (click)="open(0, $event)"
        class="group relative block h-full w-full overflow-hidden bg-slate-950 text-left focus:outline-none focus-visible:ring-4 focus-visible:ring-inset focus-visible:ring-blue-500"
        [attr.aria-label]="fallbackAlt + ' galerisini tam ekran aç. ' + items().length + ' medya.'"
      >
        ...
        <span class="absolute bottom-4 right-4 inline-flex min-h-11 items-center gap-2 rounded-full border border-white/20 bg-black/70 px-4 text-xs font-black text-white backdrop-blur">
          <mat-icon class="!h-5 !w-5 !text-[20px]">photo_library</mat-icon>
          {{ items().length }} Medya · Tam Ekran
        </span>
      </button>
    ...
    @if (isOpen()) {
      <div class="fixed inset-0 z-[200] flex flex-col bg-black text-white" role="dialog" aria-modal="true" aria-label="Fotoğraf ve video galerisi">
        <header class="flex min-h-16 items-center justify-between gap-4 border-b border-white/10 bg-black/90 px-4 backdrop-blur">
          ...
          <button #closeButton type="button" (click)="close()" class="flex h-11 w-11 items-center justify-center rounded-full bg-white/10 hover:bg-white/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-white" aria-label="Galeriyi kapat"><mat-icon>close</mat-icon></button>
        </header>
...
export class CatalogMixedGalleryComponent implements OnChanges, OnDestroy {
  ...
  open(index: number, event?: Event): void {
    ...
    queueMicrotask(() => {
      ...
      this.host.nativeElement.querySelector<HTMLButtonElement>("[aria-label='Galeriyi kapat']")?.focus();
    });
  }
  ...
  goTo(index: number, smooth = true): void {
    ...
    const scroller = this.host.nativeElement.querySelector<HTMLElement>("[role='dialog'] .snap-x");
    if (!scroller) return;
  ...
  private pauseVideos(exceptIndex = -1): void {
    this.host.nativeElement.querySelectorAll<HTMLVideoElement>("video").forEach((video, index) => { if (index !== exceptIndex) video.pause(); });
  }
