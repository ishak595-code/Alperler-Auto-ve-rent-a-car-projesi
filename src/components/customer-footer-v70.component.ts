    ...
    <footer class="customer-footer" aria-label="Alperler Auto alt bilgi">
      <div class="footer-shell">
        <section class="brand-column" aria-labelledby="footer-brand-title">
          ...
          <p class="brand-summary">Yüksekova'da araç kiralama, ikinci el satış ve bölgesel tur hizmetlerini tek yerde planlayın. Net bilgi, kolay iletişim ve yerel destek.</p>
          <div class="contact-row">
            @if (phoneHref()) { <a [href]="phoneHref()" class="contact-pill" aria-label="Telefon">Telefon</a> }
            @if (whatsappHref()) { <a [href]="whatsappHref()" target="_blank" rel="noopener noreferrer" class="contact-pill whatsapp" aria-label="WhatsApp">WhatsApp</a> }
            @if (config().instagramUrl) { <a [href]="config().instagramUrl" target="_blank" rel="noopener noreferrer" class="contact-pill" aria-label="Instagram">Instagram</a> }
          </div>
        </section>
        <nav class="link-column" aria-label="Hizmetler"><h2>Hizmetler</h2><a routerLink="/fleet">Kiralık Araçlar</a><a routerLink="/sales">Satılık Araçlar</a><a routerLink="/tours">Turlar</a><a routerLink="/campaigns">Kampanyalar</a><a routerLink="/branches">Şubelerimiz</a><a routerLink="/appointment">Randevu</a></nav>
        <nav class="link-column" aria-label="Kurumsal"><h2>Alperler Auto</h2><a routerLink="/about">Hakkımızda</a><a routerLink="/contact">İletişim</a><a routerLink="/blog">Blog</a><a routerLink="/faq">Sık Sorulan Sorular</a><a routerLink="/legal">Yasal Bilgilendirmeler</a><button type="button" (click)="openFeedback()">Geri Bildirim Gönder</button></nav>
        <section class="newsletter" aria-labelledby="newsletter-title-v70">
          <h2 id="newsletter-title-v70">Yeni araç ve fırsatları kaçırmayın</h2><p>Sadece yeni ilan, tur ve kampanya olduğunda haber alın. Abonelik ücretsizdir.</p>
          <form (submit)="subscribe($event)" novalidate><label for="footer-email-v70">E-posta adresi</label><div class="subscribe-row"><input id="footer-email-v70" type="email" [(ngModel)]="email" name="footerEmail" autocomplete="email" inputmode="email" required placeholder="ornek@eposta.com" aria-label="Bülten e-posta adresi" /><button type="submit" [disabled]="submitting()">{{ submitting() ? 'Kaydediliyor' : 'Ücretsiz Abone Ol' }}</button></div>@if (subscribed()) { <p class="success" role="status" aria-live="polite">Aboneliğiniz kaydedildi.</p> } @if (subscriptionError()) { <p class="error" role="alert">{{ subscriptionError() }}</p> }</form>
        </section>
      </div>
