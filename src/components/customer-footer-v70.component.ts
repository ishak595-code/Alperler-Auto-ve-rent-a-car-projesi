      ...
      <footer class="customer-footer" aria-label="Alperler Auto alt bilgi">
        <div class="footer-shell">
          <section class="brand-column" aria-labelledby="footer-brand-title">
            <a routerLink="/" class="brand-link" aria-label="Alperler Auto ana sayfa"><span class="brand-mark" aria-hidden="true">A</span><span class="brand-copy"><strong id="footer-brand-title">{{ config().companyName || 'Alperler Auto' }}</strong><small>{{ cleanTagline() }}</small></span></a>
            ...
            @if (footer.settings().showSocial) {
              <nav class="social-row" aria-label="Sosyal medya hesapları">
                @for (social of socialLinks(); track social.name) {
                  @if (social.url) {
                    <a [href]="social.url" target="_blank" rel="noopener noreferrer" class="social-link" [attr.aria-label]="social.name + ' hesabını aç'" [title]="social.name"><img class="social-icon" [src]="social.icon" alt="" aria-hidden="true" /></a>
                  } @else {
                    <span class="social-link disabled" aria-disabled="true" [attr.aria-label]="social.name + ' bağlantısı henüz eklenmedi'" [title]="social.name + ' bağlantısı henüz eklenmedi'"><img class="social-icon" [src]="social.icon" alt="" aria-hidden="true" /></span>
                  }
                }
              </nav>
            }
          ...
          <nav class="link-column" aria-label="Hizmetler"><h2>{{ footer.settings().servicesTitle }}</h2><a routerLink="/fleet">Kiralık Araçlar</a><a routerLink="/sales">Satılık Araçlar</a><a routerLink="/tours">Turlar</a><a routerLink="/campaigns">Kampanyalar</a><a routerLink="/branches">Şubelerimiz</a><a routerLink="/appointment">Randevu</a></nav>

          <nav class="link-column" aria-label="Kurumsal"><h2>{{ footer.settings().corporateTitle }}</h2><a routerLink="/about">Hakkımızda</a><a routerLink="/contact">İletişim</a><a routerLink="/blog">Blog</a><a routerLink="/faq">Sık Sorulan Sorular</a>@if (footer.settings().showFeedback) { <button type="button" (click)="openFeedback()">Geri Bildirim Gönder</button> }</nav>
          ...
          @if (footer.settings().newsletterEnabled) {
            <section class="newsletter" aria-labelledby="newsletter-title-v70">
              ...
              <form (submit)="subscribe($event)" novalidate>
                ...
                <div class="subscribe-row"><input id="footer-email-v70" type="email" [(ngModel)]="email" name="footerEmail" autocomplete="email" inputmode="email" required placeholder="ornek@eposta.com" aria-label="Bülten e-posta adresi" /><button type="submit" [disabled]="submitting()" aria-label="Ücretsiz bültene abone ol">{{ submitting() ? 'Kaydediliyor…' : footer.settings().newsletterButtonText }}</button></div>
                ...
                @if (subscribed()) { <p class="success" role="status" aria-live="polite">Aboneliğiniz veritabanına kaydedildi.</p> }
                @if (subscriptionError()) { <p class="error" role="alert">{{ subscriptionError() }}</p> }
              </form>
            </section>
          }
        </div>
        <div class="footer-bottom"><span>© {{ currentYear }} {{ config().companyName || 'Alperler Auto' }}. Tüm hakları saklıdır.</span><a routerLink="/admin/login" aria-label="Yönetici girişi">Yönetici</a></div>
      </footer>
    }
  `,
  styles: [`
    :host{display:block}.customer-footer{background:#030817;color:#a8b4c7;border-top:1px solid rgba(148,163,184,.14);padding:2.25rem 0 calc(1rem + env(safe-area-inset-bottom));font-family:ui-sans-serif,system-ui,sans-serif}.footer-shell{width:min(100% - 1.5rem,80rem);margin:auto;display:grid;gap:2rem}.brand-link{display:flex;width:max-content;max-width:100%;align-items:center;gap:.8rem;color:#fff;text-decoration:none;border-radius:14px}.brand-link:focus-visible,.link-column a:focus-visible,.link-column button:focus-visible,.contact-pill:focus-visible,.footer-bottom a:focus-visible,.social-link:focus-visible,.consent-note a:focus-visible{outline:3px solid #60a5fa;outline-offset:3px}.brand-mark{display:grid;width:48px;height:48px;place-items:center;border-radius:15px;background:linear-gradient(145deg,#2563eb,#1d4ed8);font:900 1.35rem/1 Georgia,serif}.brand-copy{display:flex;min-width:0;flex-direction:column}.brand-copy strong{font-family:Georgia,"Times New Roman",serif;font-size:1.3rem}.brand-copy small{margin-top:.2rem;color:#94a3b8;font-size:.65rem;font-weight:850;text-transform:uppercase}.brand-summary{max-width:430px;margin:1rem 0 0;font-size:.84rem;line-height:1.65;color:#8f9db2}.contact-row,.social-row{display:flex;flex-wrap:wrap;gap:.55rem;margin-top:1rem}.contact-pill{display:inline-flex;min-height:42px;align-items:center;border:1px solid rgba(148,163,184,.2);border-radius:12px;background:#091224;padding:0 .85rem;color:#dbe7f6;font-size:.73rem;font-weight:850;text-decoration:none}.contact-pill.whatsapp{border-color:rgba(16,185,129,.28);color:#a7f3d0}.social-link{display:grid;width:43px;height:43px;place-items:center;border:1px solid rgba(148,163,184,.2);border-radius:13px;background:#091224;color:#f8fafc}.social-link.disabled{opacity:.38;cursor:default}.social-icon{display:block;width:20px;height:20px;object-fit:contain;filter:brightness(0) invert(1)}.link-column{display:flex;flex-direction:column;align-items:flex-start;gap:.2rem}.link-column h2,.newsletter h2{margin:0 0 .65rem;color:#fff;font-size:.78rem;font-weight:900;text-transform:uppercase}.link-column a,.link-column button{display:flex;min-height:40px;align-items:center;border:0;background:transparent;padding:0;color:#9ba9bc;font:750 .78rem/1.2 inherit;text-decoration:none}.legal-primary{display:flex;flex-direction:column;align-items:flex-start}.legal-more{width:100%;margin-top:.15rem}.legal-more summary{min-height:38px;display:flex;align-items:center;cursor:pointer;color:#93c5fd;font-size:.7rem;font-weight:900;list-style:none}.legal-more summary::-webkit-details-marker{display:none}.legal-more summary::after{content:"＋";margin-left:.35rem;font-size:.72rem}.legal-more[open] summary::after{content:"−"}.legal-more-links{display:flex;flex-direction:column;align-items:flex-start;padding:.1rem 0 .25rem}.legal-more-links a{min-height:34px;font-size:.7rem;color:#7f8da1}.newsletter p{margin:0 0 .85rem;max-width:380px;color:#8391a6;font-size:.78rem;line-height:1.55}.newsletter label{display:block;margin-bottom:.35rem;color:#aab7ca;font-size:.69rem;font-weight:850}.subscribe-row{display:grid;grid-template-columns:1fr auto;gap:.45rem;max-width:440px}.subscribe-row input{min-width:0;min-height:46px;border:1px solid rgba(148,163,184,.22);border-radius:12px;background:#071020;padding:0 .75rem;color:#fff}.subscribe-row button{min-height:46px;border:0;border-radius:12px;background:#2563eb;padding:0 .85rem;color:#fff;font-weight:900}.subscribe-row button:disabled{opacity:.58}.newsletter .consent-note{margin:.55rem 0 0;font-size:.66rem;color:#64748b}.consent-note a{color:#93c5fd}.success{margin-top:.55rem!important;color:#86efac!important;font-weight:800}.error{margin-top:.55rem!important;color:#fda4af!important;font-weight:800}.footer-bottom{width:min(100% - 1.5rem,80rem);margin:2rem auto 0;border-top:1px solid rgba(148,163,184,.13);padding-top:1.2rem;display:flex;justify-content:space-between;gap:1rem;color:#64748b;font-size:.68rem}.footer-bottom a{color:#64748b;text-decoration:none}@media(max-width:430px){.subscribe-row{grid-template-columns:1fr}.subscribe-row button{width:100%}}@media(min-width:720px){.footer-shell{grid-template-columns:1.3fr 1fr 1fr}.customer-footer{padding-bottom:2.25rem}}@media(min-width:1100px){.footer-shell{grid-template-columns:1.25fr .75fr .75fr 1fr 1.15fr}.customer-footer{padding-top:3rem}}
  `],
})
export class CustomerFooterV70Component {
  ...
  readonly subscribed = signal(false); readonly submitting = signal(false); readonly subscriptionError = signal("");
  email = "";
  ...
  private safeSocialUrl(value:string|undefined):string{const raw=String(value||'').trim();return /^https:\/\//i.test(raw)?raw:'';}
}
