...
@Component({
  ...
  template: `
    @if (!hidden()) {
      <nav class="customer-command-dock" aria-label="Alt menü">
  ...
  styles: [`
    ...
    @media (max-width:767px), (max-width:950px) and (max-height:500px) and (pointer:coarse){
      .customer-command-dock{
        ...
        left:max(.38rem,env(safe-area-inset-left));right:max(.38rem,env(safe-area-inset-right));
        ...
        min-height:62px;padding:3px 2px;
    ...
    .dock-action{
      position:relative;display:flex;min-width:0;min-height:54px;flex-direction:column;
      ...
      font-size:8.7px;font-weight:820;line-height:1;white-space:nowrap;touch-action:manipulation;
    ...
    @media (max-width:370px){
      ...
      .dock-action{font-size:7.9px}.dock-action mat-icon{width:19px;height:19px;font-size:19px}
    }
