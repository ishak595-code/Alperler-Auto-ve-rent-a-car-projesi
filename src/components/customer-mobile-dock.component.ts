...
@Component({
  ...
  styles: [`
    ...
    @media (max-width:767px) and (pointer:coarse){
      .customer-command-dock{position:fixed;z-index:88;left:max(.42rem,env(safe-area-inset-left));right:max(.42rem,env(safe-area-inset-right));bottom:max(.42rem,env(safe-area-inset-bottom));display:flex;align-items:stretch;min-height:67px;overflow-x:auto;overscroll-behavior-inline:contain;scrollbar-width:none;padding:4px;border:1px solid color-mix(in srgb,var(--alper-border,#27364a) 78%,transparent);border-radius:min(var(--site-radius,20px),22px);background:linear-gradient(180deg,color-mix(in srgb,var(--alper-surface,#0b1420) 97%,transparent),color-mix(in srgb,var(--alper-bg,#060a12) 99%,transparent));box-shadow:0 16px 40px rgba(2,6,23,.38),inset 0 1px 0 rgba(255,255,255,.05);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);transition:transform .22s ease,opacity .18s ease}
    ...
    .dock-action{position:relative;display:flex;min-width:64px;min-height:58px;flex:1 0 64px;flex-direction:column;align-items:center;justify-content:center;gap:4px;border-radius:13px;background:transparent;padding:4px 3px;color:var(--alper-subtle,#94a3b8);text-decoration:none;font-size:9px;font-weight:900;line-height:1.05;text-align:center;touch-action:manipulation;transition:background-color .16s ease,color .16s ease,transform .16s ease}.dock-action::before{content:"";position:absolute;left:32%;right:32%;top:1px;height:2px;border-radius:999px;background:transparent}.dock-action mat-icon{width:21px;height:21px;font-size:21px}.dock-action span{display:-webkit-box;overflow:hidden;max-width:100%;-webkit-box-orient:vertical;-webkit-line-clamp:2}.dock-action:active{transform:translateY(1px)}.dock-action:focus-visible{outline:2px solid var(--alper-blue-light,#7899b8);outline-offset:-2px}.dock-action.dock-active{background:color-mix(in srgb,var(--alper-blue,#315e86) 19%,transparent);color:var(--alper-text,#f8fafc)}.dock-action.dock-active::before{background:var(--alper-blue-light,#7899b8)}
    @media(max-width:350px) and (pointer:coarse){.customer-command-dock{left:3px;right:3px}.dock-action{min-width:60px;flex-basis:60px;font-size:8.4px}.dock-action mat-icon{width:20px;height:20px;font-size:20px}}
