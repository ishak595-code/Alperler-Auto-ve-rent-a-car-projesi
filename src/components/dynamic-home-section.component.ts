      ...
      <section class="home-section" [ngClass]="sectionClasses()" [ngStyle]="sectionStyles()" [attr.aria-labelledby]="section.sectionKey + '-title'">
        <div class="section-inner">
          @if (renderer() === 'PARTNER') {
            ...
            <div class="promo-card">@if (coverImage()) {<div class="promo-media"><img [src]="coverImage()" [alt]="section.title" /></div>}<div class="promo-copy">@if (profileImage()) {<img class="section-profile" [src]="profileImage()" [alt]="section.title + ' profil görseli'" />}<p class="section-kicker">{{ badge('Alperler Auto') }}</p><h2 [id]="section.sectionKey + '-title'">{{ section.title }}</h2><p>{{ description('Detayları keşfedin.') }}</p>@if (ctaLabel('') && internalCta('')) {<a class="primary-cta" [routerLink]="internalCta('')">{{ ctaLabel('') }} <mat-icon aria-hidden="true">arrow_forward</mat-icon></a>}</div></div>
          } @else {
                  ...
                  <article class="campaign-card">
                    <button type="button" class="campaign-button" (click)="openCampaign(campaign)" [attr.aria-label]="campaignAriaLabel(campaign)">
                      <div class="campaign-media">
                        ...
                        <div class="campaign-topline"><span class="campaign-label">KAMPANYA</span>@if (campaign.discountPercent) {<span class="discount-label">%{{ campaign.discountPercent }} İNDİRİM</span>} @else if (campaign.badge) {<span class="discount-label">{{ campaign.badge }}</span>}</div>
                      ...
                      <div class="campaign-body">
                        ...
                        <p class="campaign-copy">{{ campaign.shortDescription || campaign.description || 'Kampanyanın avantajını ve ilgili araç ya da tur detayını inceleyin.' }}</p>
                        <div class="campaign-price-row"><div>@if (campaign.oldPrice && campaign.newPrice && campaign.oldPrice > campaign.newPrice) {<span class="old-price">{{ formatPrice(campaign.oldPrice) }}</span>}@if (campaign.newPrice != null) {<strong>{{ formatPrice(campaign.newPrice) }}</strong>}</div>@if (campaignSavings(campaign) > 0) {<span class="saving">{{ formatPrice(campaignSavings(campaign)) }} kazanç</span>} @else if (campaign.discountPercent) {<span class="saving">%{{ campaign.discountPercent }} avantaj</span>}</div>
                        <span class="campaign-cta"><span>{{ campaign.ctaLabel || 'Kampanyayı İncele' }}</span><mat-icon aria-hidden="true">arrow_forward</mat-icon></span>
                      </div>
                    </button>
                  </article>
                }
            ...
            @if (section.sectionType === 'TOURS') {
              <div class="content-rail" [class.grid-layout]="isGrid()">@for (tour of tours(); track stableVehicleKey(tour)) {<a class="image-card" [routerLink]="['/tour', tour.id]"><div class="media">@if (tour.image) {<img [src]="mediaUrl(tour.image)" [alt]="entityTitle(tour)" loading="lazy" />} @else {<span class="media-placeholder"><mat-icon aria-hidden="true">landscape</mat-icon></span>}</div><div class="card-body"><div class="title-row"><h3>{{ entityTitle(tour) }}</h3>@if (tour.price) {<strong>{{ formatPrice(tour.price) }}</strong>}</div><p>{{ tour.description || tour.location || 'Rotayı ve deneyim ayrıntılarını keşfedin.' }}</p><span class="text-link">Turu Keşfet <mat-icon aria-hidden="true">arrow_forward</mat-icon></span></div></a>}</div>
            }

            @if (renderer() === 'BRANCHES') {
              <div class="content-rail" [class.grid-layout]="isGrid()">@for (branch of branches(); track branch.id) {<a class="branch-card" [routerLink]="branch.slug ? ['/branches', branch.slug] : ['/branches']"><span class="branch-icon"><mat-icon aria-hidden="true">storefront</mat-icon></span><p class="micro">{{ branch.networkType === 'FRANCHISE' ? 'Yetkili Bayi' : 'Alperler Auto Noktası' }}</p><h3>{{ branch.name }}</h3><p>{{ branch.publicDescription || ((branch.district || branch.city) + ' bölgesindeki araç ve hizmet seçeneklerini inceleyin.') }}</p><div class="branch-meta"><span>{{ branch.city }} / {{ branch.district }}</span>@if (branch.isPickupPoint) {<span>Teslim alma</span>}@if (branch.isReturnPoint) {<span>İade</span>}</div><strong>Şubeyi Keşfet <mat-icon aria-hidden="true">arrow_forward</mat-icon></strong></a>}</div><a class="partner-inline" routerLink="/branch-partner"><span><mat-icon aria-hidden="true">add_business</mat-icon><b>Kendi bölgenizde Alperler Auto ile büyümek ister misiniz?</b></span><strong>Bayilik Başvurusu <mat-icon aria-hidden="true">arrow_forward</mat-icon></strong></a>
            }

            @if (section.sectionType === 'BLOG') {
              <div class="content-rail" [class.grid-layout]="isGrid()">@for (post of blogs(); track post.cloudId || post.id) {<a class="image-card" [routerLink]="['/blog', post.id]"><div class="media">@if (post.image) {<img [src]="post.image" [alt]="post.title" loading="lazy" />} @else {<span class="media-placeholder"><mat-icon aria-hidden="true">article</mat-icon></span>}</div><div class="card-body"><h3>{{ post.title }}</h3><p>{{ post.summary }}</p><span class="text-link">Yazıyı Oku <mat-icon aria-hidden="true">arrow_forward</mat-icon></span></div></a>}</div>
            }
...
export class DynamicHomeSectionComponent {
  ...
  campaignCountdown(value:string): string { const remaining=new Date(value).getTime()-this.clock(); if(!Number.isFinite(remaining)||remaining<=0) return "Süre doldu"; const hours=Math.floor(remaining/3_600_000); const days=Math.floor(hours/24); if(days>1) return `${days} gün kaldı`; if(days===1) return "1 gün kaldı"; return `${Math.max(1,hours)} saat kaldı`; }
