        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="mx-auto mb-10 max-w-3xl text-center">
            ...
            <h2 id="campaign-showcase-title" class="mt-5 text-3xl font-black tracking-tight text-white sm:text-4xl lg:text-5xl">
              Bugün karar vermeyi kolaylaştıran fırsatlar
            </h2>
            <p class="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-slate-300 sm:text-base">
              Kiralama, araç satın alma ve tur deneyimi için fiyatı ve temel koşulları açık üç kampanya. Sahte sayaç yok, gizli fiyat yok.
            </p>
          </div>
              ...
              >
                <div class="relative h-56 overflow-hidden bg-slate-800">
                  ...
                  <div class="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-4">
                    ...
                    @if (daysRemaining(campaign.endsAt) !== null) {
                      <span class="rounded-full border border-white/20 bg-slate-950/75 px-3 py-1.5 text-[10px] font-black text-white backdrop-blur">
                        {{ daysRemaining(campaign.endsAt) }} gün
                      </span>
                    }
                  </div>
                ...
                <div class="flex flex-1 flex-col p-5 sm:p-6">
                  ...
                  <div class="mt-6 rounded-2xl bg-slate-50 p-4">
                    <div class="flex items-end justify-between gap-3">
                      <div>
                        <p class="text-[10px] font-black uppercase tracking-[.12em] text-slate-400">Net fiyat</p>
                    ...
                    @if (campaign.endsAt) {
                      <p class="mt-3 border-t border-slate-200 pt-3 text-[11px] font-bold text-slate-500">Kampanya vitrini {{ formatDate(campaign.endsAt) }} tarihine kadar aktif.</p>
                    }
                  </div>
          ...
          <div class="mt-8 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-[11px] font-bold text-slate-400">
            <span>Şeffaf fiyat</span><span aria-hidden="true">•</span><span>Gerçek katalog verisi</span><span aria-hidden="true">•</span><span>Tek dokunuşla talep</span>
          </div>
        </div>
      </section>
    }
...
export class HomeCampaignShowcaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  intentLabel(campaign: CampaignRecord): string {
    ...
    if (intent === "SALE" || intent === "VEHICLE") return "Satın alma fırsatı";
  ...
  daysRemaining(value?: string): number | null {
  ...
  private async load(): Promise<void> {
    try {
      ...
      this.campaigns.set(rows.slice(0, 3));
      this.relocateWhenReady();
    } catch {
      this.campaigns.set([]);
    }
