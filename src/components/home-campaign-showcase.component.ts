      ...
      <section class="relative overflow-hidden bg-slate-950 py-16 sm:py-20" aria-labelledby="campaign-showcase-title">
        ...
        <div class="relative mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8">
          <div class="mx-auto mb-10 max-w-3xl text-center">
            ...
            <h2 id="campaign-showcase-title" class="mt-5 text-3xl font-black tracking-tight text-white sm:text-4xl lg:text-5xl">
              Doğru zamanda, açık koşullarla seçilmiş fırsatlar
            </h2>
            <p class="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-slate-300 sm:text-base">
              Özel gün, kiralama, araç satın alma ve tur deneyimi için gerçek bitiş zamanı, açık fiyat ve doğrudan talep akışı.
            </p>
          </div>
          ...
          <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            @for (campaign of campaigns(); track campaign.id; let i = $index) {
              <article
                class="campaign-card group relative flex min-h-[555px] flex-col overflow-hidden rounded-[30px] border border-white/10 bg-white shadow-2xl transition-transform duration-300 hover:-translate-y-2 focus-within:ring-4 focus-within:ring-blue-400/40"
                [style.animation-delay.ms]="i * 90"
              >
                <div class="relative h-56 overflow-hidden bg-slate-800">
                  @if (campaign.coverImage) {
                    <img [src]="campaign.coverImage" [alt]="campaign.title" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" referrerpolicy="no-referrer" />
                  }
                  ...
                  <div class="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-4">
                    <span class="max-w-[67%] rounded-full bg-amber-400 px-3 py-1.5 text-[10px] font-black uppercase tracking-wide text-slate-950 shadow-lg">{{ campaign.badge || 'KAMPANYA' }}</span>
                    @if (countdown(campaign); as remaining) {
                      <span class="countdown-chip rounded-full border border-white/20 bg-slate-950/80 px-3 py-1.5 text-[10px] font-black text-white backdrop-blur" [class.countdown-urgent]="isUrgent(campaign)" [attr.aria-label]="'Kampanyanın bitmesine ' + remaining">
                        {{ remaining }}
                      </span>
                    }
                  </div>
                ...
                <div class="flex flex-1 flex-col p-5">
                  ...
                  <div class="mt-6 rounded-2xl bg-slate-50 p-4">
                    <div class="flex items-end justify-between gap-3">
                      <div>
                        ...
                        <p class="mt-1 text-2xl font-black tracking-tight text-slate-950">{{ formatPrice(campaign.newPrice) }}</p>
                        @if (campaign.oldPrice && campaign.oldPrice > (campaign.newPrice || 0)) {
                          <p class="mt-1 text-xs font-bold text-slate-400 line-through">{{ formatPrice(campaign.oldPrice) }}</p>
                        }
                      </div>
                      <span class="rounded-xl bg-white px-3 py-2 text-right text-[10px] font-black leading-tight text-slate-500 shadow-sm">
                        {{ priceSuffix(campaign) }}
                      </span>
                    </div>
                    @if (campaign.endsAt) {
                      <p class="mt-3 border-t border-slate-200 pt-3 text-[11px] font-bold text-slate-500">{{ formatDate(campaign.endsAt) }} tarihine kadar, stok ve uygunluk koşullarına bağlı.</p>
                    }
                  </div>
                  ...
                  <div class="mt-auto grid grid-cols-[1fr_auto] gap-2 pt-5">
                    <button
                      ...
                      class="min-h-12 rounded-xl bg-slate-950 px-4 text-sm font-black text-white shadow-lg transition-all hover:bg-blue-700 focus:outline-none focus-visible:ring-4 focus-visible:ring-blue-500/30"
                    >
                      {{ campaign.ctaLabel || 'Detayları Gör' }}
                    </button>
                    <button
                      type="button"
                      (click)="openWhatsapp(campaign)"
                      class="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-600 text-white transition-colors hover:bg-emerald-500 focus:outline-none focus-visible:ring-4 focus-visible:ring-emerald-500/30"
                      [attr.aria-label]="campaign.title + ' için WhatsApp ile bilgi al'"
                    >
          ...
          <div class="mt-8 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-[11px] font-bold text-slate-400">
            <span>Gerçek bitiş zamanı</span><span aria-hidden="true">•</span><span>Şeffaf fiyat</span><span aria-hidden="true">•</span><span>Kaynaklı medya</span><span aria-hidden="true">•</span><span>Tek dokunuşla talep</span>
          </div>
        </div>
      </section>
    }
...
export class HomeCampaignShowcaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.clockId = setInterval(() => {
      ...
      this.campaigns.update((rows) => rows.filter((campaign) => this.campaignService.isLive(campaign, current)));
    }, 1_000);
    this.refreshId = setInterval(() => void this.load(), 60_000);
  }
  ...
  ngOnDestroy(): void {
    ...
    if (this.legacyContainer) this.legacyContainer.style.removeProperty("display");
  }
  ...
  intentLabel(campaign: CampaignRecord): string {
    const intent = this.campaignService.intentOf(campaign);
    if (intent === "WEDDING") return "Özel gün paketi";
    if (intent === "RENTAL") return "Kiralama fırsatı";
    if (intent === "SALE") return "Satın alma fırsatı";
    if (intent === "TOUR") return "Tur fırsatı";
    return "Seçilmiş fırsat";
  }

  priceLabel(campaign: CampaignRecord): string {
    const custom = campaign.metadata?.["priceLabel"];
    return typeof custom === "string" && custom.trim() ? custom : "Net fiyat";
  }

  priceSuffix(campaign: CampaignRecord): string {
    const custom = campaign.metadata?.["priceSuffix"];
    if (typeof custom === "string" && custom.trim()) return custom;
    const intent = this.campaignService.intentOf(campaign);
    if (intent === "WEDDING") return "paket";
    if (intent === "RENTAL") return "günlük";
    if (intent === "TOUR") return "kişi başı";
    return "satış fiyatı";
  }
  ...
  countdown(campaign: CampaignRecord): string | null {
    const ms = this.campaignService.remainingMs(campaign, this.now());
  ...
  isUrgent(campaign: CampaignRecord): boolean {
    const ms = this.campaignService.remainingMs(campaign, this.now());
    return ms != null && ms > 0 && ms <= 72 * 60 * 60 * 1_000;
  }
  ...
  openWhatsapp(campaign: CampaignRecord): void {
    ...
    const message = campaign.whatsappMessage?.trim() || `${campaign.title} hakkında bilgi almak istiyorum.`;
  ...
  private async load(): Promise<void> {
    try {
      ...
      this.campaigns.set(this.selectShowcase(rows));
      this.relocateWhenReady();
    } catch {
      this.campaigns.set([]);
    }
  ...
  private selectShowcase(rows: CampaignRecord[]): CampaignRecord[] {
    const preferred = ["WEDDING", "RENTAL", "SALE", "TOUR"] as const;
    const selected: CampaignRecord[] = [];
    for (const intent of preferred) {
      const match = rows.find((campaign) => this.campaignService.intentOf(campaign) === intent && !selected.some((item) => item.id === campaign.id));
  ...
  private relocateWhenReady(): void {
    ...
    requestAnimationFrame(() => {
      const marker = document.getElementById("recommended-cars");
      if (!marker) return;
      const legacy = marker.closest("div.max-w-7xl") as HTMLElement | null;
      if (!legacy || legacy === this.host.nativeElement) return;
      const parent = legacy.parentElement;
      if (!parent) return;
      this.legacyContainer = legacy;
      legacy.style.display = "none";
      parent.insertBefore(this.host.nativeElement, legacy);
    });
  }
}
