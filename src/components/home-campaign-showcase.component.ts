        ...
        <div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="mx-auto mb-9 max-w-3xl text-center">
            <span class="inline-flex items-center gap-2 rounded-full border border-amber-400/25 bg-amber-400/10 px-4 py-2 text-[11px] font-black uppercase tracking-[.18em] text-amber-300">
              ...
              {{ campaigns().length }} Seçilmiş Fırsat
            </span>
            <h2 id="campaign-showcase-title" class="mt-5 text-3xl font-black tracking-tight text-white sm:text-4xl lg:text-5xl">Kaçırılmayacak fırsatlar</h2>
            <p class="mx-auto mt-4 max-w-2xl text-sm leading-relaxed text-slate-300 sm:text-base">Kiralama, araç satın alma, özel gün ve tur deneyimlerinden seçilmiş güncel teklifler. Fiyat, kapsam ve yönlendirme tek kartta açıkça gösterilir.</p>
          </div>
          <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            @for (campaign of campaigns(); track campaign.id; let i = $index) {
              <article class="campaign-card group relative flex min-h-[540px] flex-col overflow-hidden rounded-[28px] border border-white/10 bg-white shadow-2xl transition-transform duration-300 hover:-translate-y-2 focus-within:ring-4 focus-within:ring-blue-400/40" [style.animation-delay.ms]="i * 90">
                <div class="relative h-52 overflow-hidden bg-slate-800">
                  @if (campaign.coverImage) { <img [src]="campaign.coverImage" [alt]="campaign.title" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" referrerpolicy="no-referrer" loading="lazy" /> }
                  ...
                  <div class="absolute inset-x-0 top-0 flex items-start justify-between gap-2 p-4">
                    ...
                    @if (daysRemaining(campaign.endsAt) !== null) { <span class="rounded-full border border-white/20 bg-slate-950/75 px-3 py-1.5 text-[10px] font-black text-white backdrop-blur">{{ daysRemaining(campaign.endsAt) }} gün</span> }
                  </div>
                  <div class="absolute bottom-4 left-4 right-4"><p class="text-[10px] font-black uppercase tracking-[.15em] text-amber-300">{{ intentLabel(campaign) }}</p><h3 class="mt-1 text-lg font-black leading-tight text-white sm:text-xl">{{ campaign.title }}</h3></div>
                </div>
                <div class="flex flex-1 flex-col p-5">
                  @if (imageCredit(campaign)) {
                    <div class="mb-3 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] font-bold text-slate-400">
                      <span>{{ isRepresentative(campaign) ? 'Temsili model görseli' : 'Doğrulanmış rota/model görseli' }}</span><span aria-hidden="true">•</span>
                  ...
                  <div class="mt-5 rounded-2xl bg-slate-50 p-4">
                    <div class="flex items-end justify-between gap-3"><div><p class="text-[10px] font-black uppercase tracking-[.12em] text-slate-400">Net fiyat</p><p class="mt-1 text-xl font-black tracking-tight text-slate-950 sm:text-2xl">{{ formatPrice(campaign.newPrice) }}</p></div><span class="rounded-xl bg-white px-3 py-2 text-right text-[10px] font-black leading-tight text-slate-500 shadow-sm">{{ priceSuffix(campaign) }}</span></div>
                    @if (campaign.oldPrice && campaign.oldPrice > (campaign.newPrice || 0)) { <div class="mt-2 flex items-center gap-2 text-xs font-bold"><span class="text-slate-400 line-through">{{ formatPrice(campaign.oldPrice) }}</span>@if (campaign.discountPercent) { <span class="text-emerald-700">%{{ campaign.discountPercent }} avantaj</span> }</div> }
                    @if (campaign.endsAt) { <p class="mt-3 border-t border-slate-200 pt-3 text-[11px] font-bold text-slate-500">{{ formatDate(campaign.endsAt) }} tarihine kadar aktif.</p> }
                  </div>
          ...
          <div class="mt-8 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-[11px] font-bold text-slate-400"><span>Şeffaf fiyat</span><span aria-hidden="true">•</span><span>Kaynaklı katalog verisi</span><span aria-hidden="true">•</span><span>Tek dokunuşla talep</span></div>
        </div>
      </section>
    }
  `,
...
export class HomeCampaignShowcaseComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnDestroy(): void { if (this.legacyContainer) this.legacyContainer.style.removeProperty("display"); }
  ...
  intentLabel(campaign: CampaignRecord): string { const intent = String(campaign.metadata?.["intent"] || campaign.targetType || ""); if (intent === "RENTAL") return "Kiralama fırsatı"; if (intent === "WEDDING") return "Özel gün fırsatı"; if (intent === "SALE" || intent === "VEHICLE") return "Satın alma fırsatı"; if (intent === "TOUR") return "Tur fırsatı"; return "Seçilmiş fırsat"; }
  priceSuffix(campaign: CampaignRecord): string { const custom = campaign.metadata?.["priceSuffix"]; if (typeof custom === "string" && custom.trim()) return custom; const intent = String(campaign.metadata?.["intent"] || ""); if (intent === "RENTAL") return "günlük"; if (intent === "TOUR") return "kişi başı"; if (intent === "WEDDING") return "özel gün"; return "satış fiyatı"; }
  ...
  daysRemaining(value?: string): number | null { if (!value) return null; const ms = new Date(value).getTime() - Date.now(); return Math.max(0, Math.ceil(ms / 86_400_000)); }
  ...
  private async load(): Promise<void> { try { const rows = await this.campaignService.loadPublic(); this.campaigns.set(rows.slice(0, 4)); this.relocateWhenReady(); } catch { this.campaigns.set([]); } }
