...
@Component({
  ...
  template: `
    <div
      class="fixed inset-0 z-[9999] bg-black/95 flex flex-col items-center justify-center p-4 md:p-8 animate-in fade-in duration-300"
      (click)="close.emit()"
    >
      <!-- Close Button -->
      <button
        ...
        aria-label="Kapat"
        class="absolute top-4 right-4 text-white hover:text-[#E15A62] transition-colors z-50 p-2 bg-black/50 rounded-full"
      >
      ...
      >
        ...
        <button
          *ngIf="items().length > 1"
          ...
          class="absolute left-0 md:-left-16 top-1/2 -translate-y-1/2 text-white hover:text-[#E15A62] transition-all p-4 z-50"
        >
        ...
        <button
          *ngIf="items().length > 1"
          ...
          class="absolute right-0 md:-right-16 top-1/2 -translate-y-1/2 text-white hover:text-[#E15A62] transition-all p-4 z-50"
        >
      ...
      <div
        class="absolute bottom-20 left-0 w-full flex justify-center gap-2 px-4 overflow-x-auto py-2 scrollbar-hide"
        (click)="$event.stopPropagation()"
      >
        <div
