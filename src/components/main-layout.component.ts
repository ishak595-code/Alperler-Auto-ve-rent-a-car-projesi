...
@Component({
  ...
  template: `
    <div class="layout-root">
      ...
      @defer (on viewport; prefetch on idle) {
        ...
        <div class="footer-defer-anchor" aria-hidden="true"></div>
      }
  ...
  `,
  styles: [`
    :host{display:block}.layout-root{position:relative;display:flex;min-height:100vh;min-height:100dvh;min-width:0;flex-direction:column;overflow-x:hidden;background:#f8fafc;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.customer-main{min-width:0;flex:1;padding-top:72px}.skip-link{position:fixed;left:12px;top:12px;z-index:220;transform:translateY(calc(-100% - 28px));border-radius:10px;background:#fff;padding:11px 15px;color:#0f172a;font-weight:900;text-decoration:none;box-shadow:0 16px 36px rgba(2,6,23,.28);transition:transform .16s ease}.skip-link:focus,.skip-link:focus-visible{transform:translateY(0);outline:3px solid #3b82f6;outline-offset:2px}.footer-defer-anchor{min-height:1px;width:100%}.whatsapp-fab{position:fixed;right:max(1rem,env(safe-area-inset-right));bottom:max(1rem,env(safe-area-inset-bottom));z-index:87;display:grid;width:48px;height:48px;place-items:center;border-radius:999px;background:#22c55e;color:#fff;box-shadow:0 12px 30px rgba(2,6,23,.25);transition:bottom .2s ease,transform .16s ease,box-shadow .16s ease,background-color .16s ease}.whatsapp-fab svg{width:24px;height:24px}.whatsapp-fab:active{transform:scale(.96)}.whatsapp-fab:focus-visible{outline:2px solid #fff;outline-offset:3px}.whatsapp-fab:hover{background:#16a34a;box-shadow:0 16px 34px rgba(2,6,23,.3)}
    @media(min-width:768px){.customer-main{padding-top:84px}}
    @media(min-width:1280px){.customer-main{padding-top:96px}}
    @media(max-width:767px), (max-width:950px) and (max-height:500px) and (pointer:coarse){.whatsapp-fab.dock-offset{bottom:calc(max(1rem,env(safe-area-inset-bottom)) + 5.15rem)}}
