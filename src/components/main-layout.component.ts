...
@Component({
  ...
  styles: [`
    ...
    @media(prefers-reduced-motion:reduce){.skip-link,.whatsapp-fab{transition:none;animation:none}}
  `],
})
export class MainLayoutComponent {
  uiService=inject(UiService);carService=inject(CarService);navigation=inject(NavigationConfigService);router=inject(Router);location=inject(Location);isHomePage=signal(true);
  constructor(){this.router.events.pipe(filter(event=>event instanceof NavigationEnd)).subscribe(()=>this.updatePageState());this.updatePageState();}
