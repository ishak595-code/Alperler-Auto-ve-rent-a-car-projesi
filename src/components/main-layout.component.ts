    ...
    <div class="min-h-screen flex flex-col font-sans relative bg-slate-50 overflow-x-hidden">
      ...
      @if (showWhatsapp() && getWhatsappNumber()) {
        <a
          ...
          class="fixed right-[max(1rem,env(safe-area-inset-right))] bottom-[max(1rem,env(safe-area-inset-bottom))] z-[87] w-12 h-12 bg-green-500 text-white rounded-full flex items-center justify-center shadow-md active:scale-95 md:hover:scale-110 hover:shadow-lg transition-all hover:bg-green-600 animate-fade-in-up focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-green-600"
