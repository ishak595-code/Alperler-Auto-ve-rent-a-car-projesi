...
@Component({
          ...
          <a routerLink="/" aria-label="Alperler Rent A Car ana sayfa" class="inline-flex min-w-0 flex-1 items-center rounded-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 md:max-w-[300px] xl:flex-none">
            @if (config().logoUrl) {
              ...
              <div class="flex min-w-0 items-center gap-2.5 sm:gap-3">
                ...
                <div class="min-w-0"><div class="brand-name font-serif font-black uppercase tracking-[.04em] text-white">Alperler Rent A Car</div><div class="brand-sub mt-0.5 font-black uppercase tracking-[.13em] text-slate-400">Kiralama • Satış • Tur</div></div>
              </div>
            }
          </a>
          ...
          <div class="relative flex shrink-0 items-center gap-1 sm:gap-2">
            ...
            @if (isLangMenuOpen()) {
              <div role="menu" class="absolute right-12 top-14 z-[140] max-h-96 w-40 overflow-y-auto rounded-xl border border-slate-700 bg-[#0b1526] py-1 shadow-2xl">
                @for (lang of languages; track lang) { <button type="button" role="menuitem" (click)="setLang(lang)" class="block min-h-11 w-full px-4 py-2 text-left text-sm font-semibold text-slate-200 hover:bg-white/10 focus:outline-none focus-visible:bg-white/10">{{ langName(lang) }}</button> }
              </div>
            }
            ...
            @if (!customerAuth.isLoggedIn()) {
              <a routerLink="/account/login" aria-label="Giriş yap veya kayıt ol" class="auth-entry relative flex h-11 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[.035] px-2.5 text-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400">
                <mat-icon aria-hidden="true">person_outline</mat-icon><span>Giriş / Kayıt</span>
              </a>
            } @else {
              <a routerLink="/account" aria-label="Hesabım" class="account-entry relative hidden h-11 min-w-11 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[.035] px-2 text-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 xl:flex">
    ...
    @if (navigation.mobileMenuEnabled()) {
      <nav id="mobile-navigation" aria-label="Mobil navigasyon" [class.hidden]="!isMenuOpen()" [attr.aria-hidden]="isMenuOpen() ? null : 'true'" [attr.inert]="isMenuOpen() ? null : ''" class="fixed inset-x-0 bottom-0 top-[72px] z-[95] overflow-y-auto overscroll-contain bg-[#050b16] px-4 pb-[calc(6.5rem+env(safe-area-inset-bottom))] pt-4 text-white md:top-[96px] sm:px-6 xl:hidden" style="touch-action:pan-y;-webkit-overflow-scrolling:touch">
        <div class="mx-auto w-full max-w-xl">
          <div class="mb-4 px-1"><p class="menu-kicker text-[10px] font-black uppercase tracking-[.2em]">Alperler Rent A Car</p><p class="mt-1 text-sm leading-6 text-slate-400">Kiralama, satış, tur ve diğer hizmetlere buradan ulaşın.</p></div>
          <div class="overflow-hidden rounded-2xl border border-white/10 bg-[#0b1526] shadow-2xl">
            @for (item of navigation.itemsFor('MOBILE_MENU'); track item.id; let first=$first; let last=$last) { <a [id]="first ? 'mobile-menu-first-link' : null" [routerLink]="item.route" (click)="closeMenu(false)" class="menu-row" [class.last]="last" [attr.aria-label]="item.label"><mat-icon aria-hidden="true">{{ item.icon }}</mat-icon><span>{{ item.label }}</span></a> }
          </div>
          <div class="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-[#0b1526] shadow-2xl" aria-label="Kişisel ayarlar">
            <a [routerLink]="customerAuth.isLoggedIn() ? '/account' : '/account/login'" (click)="closeMenu(false)" class="menu-row account-menu-row" [attr.aria-label]="customerAuth.isLoggedIn() ? 'Hesabım' : 'Giriş yap veya kayıt ol'">
            ...
            <button type="button" class="menu-row last" (click)="toggleMobileLanguage()" [attr.aria-expanded]="mobileLanguageOpen()" aria-controls="mobile-language-options"><mat-icon aria-hidden="true">language</mat-icon><span>Dil: {{ langName(uiService.currentLang()) }}</span><mat-icon aria-hidden="true" class="transition-transform" [class.rotate-180]="mobileLanguageOpen()">expand_more</mat-icon></button>
            @if (mobileLanguageOpen()) { <div id="mobile-language-options" class="grid grid-cols-2 gap-2 border-t border-white/10 bg-black/10 p-3 sm:grid-cols-3" aria-label="Dil seçenekleri">@for (lang of languages; track lang) { <button type="button" (click)="setMobileLang(lang)" [attr.aria-pressed]="uiService.currentLang()===lang" class="min-h-12 rounded-xl border border-white/10 bg-white/[.035] px-3 py-2 text-left text-sm font-bold text-slate-100"><span class="block text-[10px] font-black uppercase tracking-wider text-slate-400">{{ lang }}</span><span class="mt-0.5 block text-xs">{{ langName(lang) }}</span></button> }</div> }
          </div>
        </div>
      </nav>
    }
  `,
  styles: [`
    .brand-mark{background:var(--alper-gold,#c6a15b);color:#111827;box-shadow:0 10px 28px rgba(0,0,0,.28)}.brand-name{font-size:13.5px;line-height:1.05;white-space:nowrap}.brand-sub{font-size:7px;line-height:1.2;white-space:nowrap}.auth-entry span,.account-entry span{max-width:110px;overflow:hidden;text-overflow:ellipsis;font-size:10px;font-weight:900;white-space:nowrap}.auth-entry:hover,.account-entry:hover{background:rgba(255,255,255,.065)}.account-avatar,.menu-avatar{width:30px;height:30px;flex:0 0 30px;border-radius:999px;object-fit:cover;border:1px solid rgba(255,255,255,.18)}.menu-avatar{width:32px;height:32px;flex-basis:32px}.menu-kicker{color:var(--alper-gold,#d5b449)}
    .desktop-nav{min-width:0;max-width:min(62vw,760px);flex:1;align-items:center;justify-content:flex-start;gap:.72rem;overflow-x:auto;scrollbar-width:none;padding:.3rem .2rem}.desktop-nav::-webkit-scrollbar{display:none}.nav-link{flex:none;border-bottom:2px solid transparent;padding:.5rem 0;font-size:10px;font-weight:800;letter-spacing:.055em;text-transform:uppercase;color:#cbd5e1;white-space:nowrap;text-decoration:none}.nav-link:hover{color:#fff}.nav-link-active{color:#fff!important;border-color:var(--alper-blue-light,#60a5fa)!important}
    ...
    @media(max-width:390px){.auth-entry{padding-inline:.45rem}.auth-entry span{font-size:9px}}@media(min-width:390px){.brand-name{font-size:14.5px}.brand-sub{font-size:7.5px}}@media(min-width:480px){.brand-name{font-size:16px}.brand-sub{font-size:8px}}@media(min-width:1536px){.desktop-nav{gap:1rem}.nav-link{font-size:10.5px}}
  `],
})
export class NavbarComponent {
  ...
  constructor(){effect(()=>{if(!this.navigation.mobileMenuEnabled())this.closeMenu(false);});effect(()=>{if(this.customerAuth.isLoggedIn())void this.customerAccount.refreshProfileSummary();else this.customerAccount.clearLocalProfile();});}
  ...
  toggleMenu():void{this.isMenuOpen()?this.closeMenu(false):this.openMenu();}openMenu():void{if(this.isMenuOpen())return;this.isLangMenuOpen.set(false);this.mobileLanguageOpen.set(false);this.isMenuOpen.set(true);}closeMenu(restoreFocus=false):void{if(!this.isMenuOpen())return;this.isMenuOpen.set(false);this.mobileLanguageOpen.set(false);if(restoreFocus)this.focusElement("mobile-menu-trigger");}
