...
@Component({
  ...
  standalone: true,
  template: `
    @if (install.mobileBrowser() && !dismissed()) {
      @if (install.installed() && !install.runningAsApp()) {
        <aside class="install-card installed-card" role="status" aria-label="Alperler uygulaması kuruldu">
          <div class="install-copy">
            <span class="app-mark" aria-hidden="true">A</span>
            <div>
              <strong>Uygulama kuruldu</strong>
              <small>Bu Chrome sekmesi tarayıcı olarak kalır. Tarayıcı çubukları olmadan açmak için ana ekranınızdaki Alperler simgesine dokunun.</small>
            </div>
          </div>
          <div class="install-actions single-action">
            <button type="button" class="install" (click)="dismiss()" aria-label="Kurulum tamamlandı mesajını kapat">Tamam</button>
          </div>
        </aside>
      } @else if (!install.runningAsApp()) {
  ...
  async startInstall(): Promise<void> {
    ...
    try {
      ...
      if (outcome === 'dismissed') {
        ...
        this.notice.set('Chrome kurulum oturumunu yeniliyor. Kısa süre sonra tekrar deneyin.');
      }
    } finally {
