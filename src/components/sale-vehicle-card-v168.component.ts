...
export class SaleVehicleCardV168Component{
  ...
  get tramerLabel():string{const s=this.car.tramerStatus||'UNKNOWN';if(s==='UNKNOWN')return'Tramer: bilinmiyor';if(s==='DECLARED_CLEAN')return'Tramer beyanı: kayıt yok';if(s==='VERIFIED_CLEAN')return'Tramer doğrulandı: kayıt yok';const amount=Number(this.car.tramerAmount||0);const money=amount>0?new Intl.NumberFormat('tr-TR',{style:'currency',currency:'TRY',maximumFractionDigits:0}).format(amount):'tutar belirtilmedi';return`${s==='VERIFIED_RECORD'?'Tramer doğrulandı':'Tramer beyanı'}: ${money}`;}
  get ariaLabel():string{return`${this.title}, ilan ${this.car.cloudStockCode||this.car.id}, ${this.car.km??'kilometre belirtilmedi'} kilometre, fiyat ${this.car.price} Türk lirası, ${this.tramerLabel}, ${this.statusLabel}. Detayları aç`;}
