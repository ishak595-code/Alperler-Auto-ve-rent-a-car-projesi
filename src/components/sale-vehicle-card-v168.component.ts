...
@Component({
  ...
  template:`
    <a [routerLink]="['/sales',car.id]" [attr.aria-label]="ariaLabel" class="card">
      <article>
        <div class="media"><img [src]="cover" (error)="imageError($event)" [alt]="title" loading="lazy" decoding="async"/><div class="shade"></div><span class="stock">{{car.cloudStockCode||'İlan no yok'}}</span><span class="status" [class.sold]="isSold">{{statusLabel}}</span>@if(car.badge){<span class="badge">{{car.badge}}</span>}</div>
        <div class="body"><h2>{{title}}</h2><div class="facts">@if(car.year){<span>{{car.year}}</span>}@if(car.km!=null){<span>{{car.km|number}} km</span>}@if(car.fuel){<span>{{car.fuel}}</span>}@if(car.transmission){<span>{{car.transmission}}</span>}@if(car.type){<span>{{car.type}}</span>}@if(car.color){<span>{{car.color}}</span>}</div>
          <div class="trust">@if(warrantyLabel){<span>✓ {{warrantyLabel}}</span>}@if(damageLabel){<span [class.attention]="!damageFree">{{damageFree?'✓':'•'}} {{damageLabel}}</span>}</div>
          <div class="location"><div><strong>{{branchLabel||'Şube bilgisi yok'}}</strong><span>{{car.location||'Konum bilgisi girilmedi'}}</span></div><b>{{car.price|turkishCurrency}}</b></div>
          <div class="cta"><span>{{isSold?'İlanı incele':'İlan detayını ve ekspertizi incele'}}</span><span aria-hidden="true">→</span></div>
        </div>
      </article>
    </a>
  `,
  styles:[`
    :host{display:block;height:100%}.card{display:block;height:100%;overflow:hidden;border:1px solid #26354d;border-radius:18px;background:#0c1526;color:#f8fafc;text-decoration:none;box-shadow:0 12px 30px rgba(0,0,0,.2);transition:transform .2s,border-color .2s,box-shadow .2s}.card:hover{transform:translateY(-3px);border-color:#60a5fa;box-shadow:0 18px 38px rgba(0,0,0,.28)}.card:focus-visible{outline:3px solid #60a5fa;outline-offset:3px}.card article{display:flex;height:100%;flex-direction:column}.media{position:relative;aspect-ratio:16/10;overflow:hidden;background:#111827}.media img{width:100%;height:100%;object-fit:cover}.shade{position:absolute;inset:0;background:linear-gradient(to top,rgba(2,6,23,.72),transparent 55%)}.stock,.status,.badge{position:absolute;border-radius:999px;padding:5px 8px;font-size:8px;font-weight:950;letter-spacing:.05em}.stock{left:8px;top:8px;background:rgba(2,6,23,.9);color:#cbd5e1}.status{right:8px;top:8px;background:#dcfce7;color:#166534}.status.sold{background:#fee2e2;color:#991b1b}.badge{left:8px;bottom:8px;background:#fbbf24;color:#451a03}.body{display:flex;flex:1;flex-direction:column;padding:12px}.body h2{margin:0;font-size:16px;line-height:1.25}.facts{display:flex;flex-wrap:wrap;gap:5px;margin-top:9px}.facts span{border:1px solid #26354d;border-radius:7px;background:#101c30;padding:5px 7px;color:#b8c5d6;font-size:9px;font-weight:800}.trust{display:flex;flex-wrap:wrap;gap:6px;margin-top:9px}.trust span{color:#86efac;font-size:9px;font-weight:850}.trust span.attention{color:#fcd34d}.location{display:flex;align-items:end;justify-content:space-between;gap:10px;margin-top:auto;border-top:1px solid #26354d;padding-top:11px}.location div{min-width:0}.location strong,.location span{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.location strong{color:#dbeafe;font-size:9px}.location span{max-width:160px;margin-top:3px;color:#8190a6;font-size:9px}.location b{flex:none;color:#f8fafc;font-size:16px}.cta{display:flex;align-items:center;justify-content:space-between;margin-top:8px;color:#60a5fa;font-size:9px;font-weight:950;text-transform:uppercase}.card:hover .cta span:last-child{transform:translateX(2px)}@media(prefers-reduced-motion:reduce){.card{transition:none}}
  `]
})
export class SaleVehicleCardV168Component{
  ...
  get ariaLabel():string{return`${this.title}, ilan ${this.car.cloudStockCode||this.car.id}, ${this.car.km??'kilometre belirtilmedi'} kilometre, fiyat ${this.car.price} Türk lirası, ${this.statusLabel}. Detayları aç`;}
