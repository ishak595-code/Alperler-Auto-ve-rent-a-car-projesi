    ...
    @if (slides().length > 1 || hasVideo()) {
      <section class="mx-auto max-w-5xl px-4 pt-8 sm:px-6" aria-labelledby="tour-gallery-title">
        <div class="mb-4 flex items-end justify-between gap-4">
          <div>
            <p class="text-[10px] font-black uppercase tracking-[.18em] text-[#9E1B24]">Gerçek Rota Medyası</p>
          ...
          <div class="mt-3 flex snap-x gap-2 overflow-x-auto pb-2" aria-label="Tur medya küçük resimleri">
            @for (slide of slides(); track slide.key; let index = $index) {
              <button
                ...
                class="relative min-h-16 min-w-24 snap-start overflow-hidden rounded-xl border-2 bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E15A62]"
