    ...
    >
      <article class="flex h-full min-w-0 flex-col">
        <div class="relative h-[148px] w-full overflow-hidden rounded-xl bg-[#101A2E] sm:h-[168px]">
          ...
          @if (car.badge) {
            <span
              class="absolute left-2 top-2 max-w-[calc(100%-1rem)] truncate rounded-full px-2.5 py-1 text-[9px] font-black uppercase tracking-wide shadow-lg"
              [ngClass]="badgeTone(car.badge)"
            >{{ car.badge }}</span>
          }

          @if (car.isCampaign || car.discountRate) {
            <span class="absolute bottom-2 left-2 rounded-full bg-[#EABF35] px-2.5 py-1 text-[9px] font-black uppercase text-[#111827] shadow-lg">
              @if (car.discountRate) { %{{ car.discountRate }} İNDİRİM } @else { FIRSAT }
            </span>
          }
        ...
        <div class="flex flex-1 flex-col px-1 pt-3">
          ...
          <div class="mt-auto flex min-w-0 items-end justify-between gap-2 border-t border-[#24314A] pt-3">
            <div class="min-w-0">
              ...
              <span class="mt-1 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-[#60A5FA]">
                {{ variant === 'sale' ? 'İlan detayını aç' : 'Aracı ve fiyatı incele' }}
                <span aria-hidden="true">→</span>
              </span>
            </div>
            <span class="shrink-0 text-right text-sm font-black text-[#F8FAFC] sm:text-base">
              {{ car.price | turkishCurrency }}@if (variant === "rental") {<span class="ml-0.5 block text-[9px] font-semibold text-[#94A3B8]">/ gün</span>}
            </span>
          </div>
        </div>
      </article>
...
export class VehicleListItemComponent {
  ...
  badgeTone(label: string): string {
    ...
    if (/FIRSAT|İNDİRİM|INDIRIM|KAMPANYA|AVANTAJ/.test(normalized)) {
      return "bg-[#EABF35] text-[#111827]";
    }
    if (/YENİ|YENI|POPÜLER|POPULAR/.test(normalized)) {
