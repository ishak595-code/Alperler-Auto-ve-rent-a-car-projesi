    ...
    <a [routerLink]="detailRoute" [queryParams]="queryParams||undefined" [attr.aria-label]="detailAriaLabel" class="group block h-full min-w-0 cursor-pointer overflow-hidden rounded-2xl border border-[#24314A] bg-[#0D1628] p-3 text-[#F8FAFC] shadow-lg transition-all duration-200 hover:-translate-y-1 hover:border-[#C6A15B] hover:bg-[#101A2E] hover:shadow-2xl active:translate-y-0 active:scale-[.99] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C6A15B]">
      <article class="flex h-full min-w-0 flex-col">
        ...
        <div class="flex flex-1 flex-col px-1 pt-3">
          ...
          <div class="mt-2 flex flex-wrap gap-1.5 text-[10px] font-bold text-[#94A3B8] sm:text-xs" [attr.aria-label]="t().vehicleListItem.specsAria">
            ...
            @if(variant==='sale'&&car.km!=null){<span class="rounded-md border border-[#24314A] bg-[#101A2E] px-2 py-1">{{kmLabel()}}</span>}
            @if(variant==='rental'&&car.seats){<span class="rounded-md border border-[#24314A] bg-[#101A2E] px-2 py-1">{{seatsLabel()}}</span>}
            @if(variant==='rental'&&car.hourlyRentalEnabled){<span class="rounded-md border border-amber-400/25 bg-amber-400/10 px-2 py-1 text-amber-200">{{t().vehicleListItem.hourlyRentable}}</span>}
          </div>
          ...
          <div class="mt-auto flex min-w-0 items-end justify-between gap-2 border-t border-[#24314A] pt-3">
            <div class="min-w-0">
              ...
              <span class="mt-1 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-wider text-[#E7C777]">{{t().vehicleListItem.inspect}}<span aria-hidden="true">→</span></span>
            </div>
            <span class="shrink-0 text-right text-sm font-black text-[#F8FAFC] sm:text-base">{{displayPrice|turkishCurrency}}@if(variant==='rental'){<span class="ml-0.5 block text-[9px] font-semibold text-[#94A3B8]">{{hourlyContext?t().vehicleListItem.perHour:t().vehicleListItem.perDay}}</span>}</span>
          </div>
        </div>
      </article>
    </a>
...
export class VehicleListItemComponent {
  ...
  get displayTitle():string{const fallback=[this.car.year,this.car.brand,this.car.model,this.car.series].filter(Boolean).join(" ");return this.car.title?.trim()||fallback||this.t().vehicleListItem.fallbackTitle;}
  get locationLabel():string{return String(this.car.location||this.car.branchDistrict||this.car.branchCity||"").trim()||this.t().vehicleListItem.locationFallback;}
  get cardDescription():string{const description=String(this.car.description||"").trim().replace(/\s+/g," ");if(description)return description.length>160?`${description.slice(0,157).trimEnd()}...`:description;return this.variant==="sale"?this.t().vehicleListItem.saleDescFallback:"";}
  get detailAriaLabel():string{
    const V=this.t().vehicleListItem;
    const type=this.variant==="rental"?V.ariaRental:V.ariaSale;
    const kmPart=this.variant==="sale"&&this.car.km!=null?String(V.ariaKm||"").replace("{n}", String(this.car.km)):null;
    const details=[this.car.year,kmPart,this.car.transmission,this.car.fuel,this.locationLabel].filter(Boolean).join(", ");
    const unit=this.variant==="rental"?(this.hourlyContext?V.unitHourly:V.unitDaily):"";
    const detailsSegment=details?`, ${details}`:"";
    return String(V.aria||"").replace("{title}", this.displayTitle).replace("{type}", type).replace("{details}", detailsSegment).replace("{price}", String(this.displayPrice)).replace("{unit}", unit);
  }
  discountLabel():string{return String(this.t().vehicleListItem.discount||"").replace("{n}", String(this.car.discountRate));}
  seatsLabel():string{return String(this.t().vehicleListItem.seats||"").replace("{n}", String(this.car.seats||""));}
  kmLabel():string{
