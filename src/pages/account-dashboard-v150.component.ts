...
export class AccountDashboardV150Component implements OnInit {
  ...
  async reload(): Promise<void> {
    ...
    try {
      ...
      this.pageError.set('Hesap bilgileriniz şu anda yenilenemedi. Lütfen bağlantınızı kontrol edip tekrar deneyin.');
    }
  }
  ...
  typeLabel(value: string): string {
    if (value === 'RENTAL') return 'Araç Kiralama';
    if (value === 'TOUR') return 'Tur Rezervasyonu';
    if (value === 'SALE_INQUIRY') return 'Satın Alma Talebi';
    if (value === 'APPOINTMENT') return 'Randevu Talebi';
    return 'Talep';
  }

  statusLabel(value: string): string {
    if (value === 'APPROVED') return 'Onaylandı';
    if (value === 'REJECTED') return 'Onaylanmadı';
  ...
  statusDescription(booking: CustomerBooking): string {
    if (booking.status === 'APPROVED') return 'Talebiniz onaylandı. İşlem ayrıntıları için ekibimiz gerektiğinde sizinle iletişime geçecektir.';
    if (booking.status === 'REJECTED') return 'Talebiniz bu aşamada onaylanmadı. Ayrıntı veya alternatif seçenekler için destek ekibimizle iletişime geçebilirsiniz.';
    if (booking.status === 'COMPLETED') return 'Bu işlem tamamlandı. Geçmiş kayıt olarak hesabınızda saklanır.';
    if (booking.status === 'CANCELLED') return 'Bu talep iptal edildi ve artık aktif işlem olarak değerlendirilmez.';
    return 'Talebiniz başarıyla alındı ve ekibimiz tarafından inceleniyor. Durum değiştiğinde bu bölüm güncellenir.';
  }

  async confirmCancel(booking: CustomerBooking): Promise<void> {
    this.pageMessage.set('');
    this.pageError.set('');
    try {
      await this.cancelActions.cancel(booking.reference);
      this.cancelConfirm.set(null);
      this.pageMessage.set('Talebiniz başarıyla iptal edildi. Güncel durum işlem geçmişinize işlendi.');
      await this.account.refresh();
    } catch (error) {
      this.pageError.set(error instanceof Error ? error.message : 'Talep iptal edilemedi.');
    }
  }

  async openAdmin(): Promise<void> {
    if (this.adminOpening()) return;
    this.adminOpening.set(true);
    this.pageError.set('');
    try {
      await this.adminBridge.openAdmin();
    } catch (error) {
      this.pageError.set(error instanceof Error ? error.message : 'Yönetim paneli açılamadı.');
      this.adminOpening.set(false);
    }
  }
...
function accountName(fullName?: string | null, email?: string | null): string {
  const name = String(fullName || '').trim();
  if (name) return name;
  const local = String(email || '').split('@')[0].replace(/[._-]+/g, ' ').trim();
  return local ? local.replace(/\b\w/g, (letter) => letter.toLocaleUpperCase('tr-TR')) : 'Hesabım';
}
