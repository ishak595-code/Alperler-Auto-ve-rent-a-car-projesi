  ...
  async resetPassword(): Promise<void> {
    ...
    try {
      if (!await this.auth.resetPassword(email)) {
        this.error.set(this.auth.lastErrorMessage() || 'Parola yenileme isteği işlenemedi.');
        return;
      }
      this.message.set('Güvenli parola yenileme bağlantısı e-posta adresinize gönderildi.');
    } finally { this.working.set(false); }
  }
}
