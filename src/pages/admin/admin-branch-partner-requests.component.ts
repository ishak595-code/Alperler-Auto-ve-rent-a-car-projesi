...
<main class="min-h-screen bg-slate-50 text-slate-900">
  <header class="sticky top-0 z-20 border-b border-slate-200 bg-white/95 px-4 py-5 shadow-sm backdrop-blur md:px-8">
    <div class="mx-auto flex max-w-7xl flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
      <div class="flex items-center gap-3"><button type="button" (click)="router.navigate(['/admin/dashboard'])" class="grid min-h-12 min-w-12 place-items-center rounded-xl bg-slate-100 font-black" aria-label="Kontrol paneline dön">←</button><div><p class="text-[10px] font-black uppercase tracking-[.18em] text-[#9E1B24]">Şube Adayları</p><h1 class="text-2xl font-black">Bayilik ve İş Ortaklığı Başvuruları</h1><p class="mt-1 text-xs text-slate-500">Ticari doğrulama, onay, şube hazırlığı ve aktivasyonu ayrı aşamalarda yönetin.</p></div></div>
