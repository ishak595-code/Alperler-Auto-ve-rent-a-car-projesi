      ...
      <header class="sticky top-0 z-30 border-b border-slate-200 bg-white shadow-sm">
        <div class="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-3 px-4 md:px-8">
          <div class="flex min-w-0 items-center gap-3">
            ...
            <div class="min-w-0"><h1 class="truncate text-xl font-black">Şube Yönetimi</h1><p class="text-xs text-slate-500">Şube, teslimat ve iade noktalarını Supabase üzerinden yönetin</p></div>
          </div>
          <button type="button" (click)="newBranch()" class="flex min-h-11 items-center gap-2 rounded-xl bg-slate-900 px-4 font-black text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"><mat-icon>add</mat-icon><span class="hidden sm:inline">Yeni Şube</span></button>
        </div>
      </header>
      ...
      <div class="mx-auto grid max-w-7xl gap-6 px-4 py-6 md:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(360px,480px)]">
        <section class="space-y-3" aria-label="Şube listesi">
          @for (branch of branchService.managedBranches(); track branch.id) {
            ...
            <div class="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm font-bold text-slate-500">Henüz gerçek şube kaydı yok. Sağdaki formdan ilk şubeyi ekleyin.</div>
          }
        </section>
        ...
        <section class="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-24" aria-label="Şube düzenleme formu">
          <div class="mb-5 flex items-center justify-between gap-3"><div><h2 class="text-lg font-black">{{ isNew() ? 'Yeni Şube' : 'Şubeyi Düzenle' }}</h2><p class="text-xs text-slate-500">Gerçek işletme bilgilerini eksiksiz girin</p></div><mat-icon class="text-slate-400">storefront</mat-icon></div>
          <div class="space-y-4">
            ...
            <button type="button" (click)="save()" [disabled]="saving()" class="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 font-black text-white disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">@if (saving()) { <mat-icon class="animate-spin">progress_activity</mat-icon> Kaydediliyor... } @else { <mat-icon>save</mat-icon> Şubeyi Kaydet }</button>
          </div>
        </section>
      </div>
    </main>
...
export class AdminBranchesComponent implements OnInit {
  ...
  async save(): Promise<void> {
    ...
    try {
      ...
      this.toastService.show("Şube Supabase veritabanına kaydedildi.", "success");
  ...
  async toggleActive(branch: Branch): Promise<void> {
    ...
    try {
      ...
      this.toastService.show(branch.isActive ? "Şube pasife alındı." : "Şube yeniden aktifleştirildi.", "success");
    } catch (error) {
  ...
  private emptyBranch(): Branch {
    return { id: "", name: "", city: "Hakkari", district: "Yüksekova", addressLabel: "", phone: "", whatsapp: "", email: "", mapUrl: "", workingHours: [{ label: "Çalışma saatleri", value: "" }], services: ["RENTAL", "PICKUP", "RETURN"], isActive: true, isPickupPoint: true, isReturnPoint: true, priority: 10 };
  }
