...
@Component({
    ...
    <main class="min-h-screen bg-slate-100 pb-20 text-slate-900">
      ...
      <div class="mx-auto grid max-w-7xl gap-6 px-4 py-6 md:px-8 lg:grid-cols-[minmax(0,1fr)_minmax(360px,480px)]">
          ...
          @for (branch of filteredBranches(); track branch.cloudId || branch.id) {
            <article class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" [class.opacity-70]="branch.publicStatus==='SUSPENDED' || branch.publicStatus==='CLOSED'">
              <div class="flex items-start justify-between gap-4">
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-2"><span class="text-xs font-bold uppercase tracking-wider text-blue-700">{{ branch.city }} / {{ branch.district }} · {{ branch.country || 'Türkiye' }}</span><span class="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black text-slate-600">{{ networkLabel(branch) }}</span></div>
              ...
              <div class="mt-5 grid grid-cols-2 gap-2">
                ...
                @if (branch.networkType === 'FRANCHISE' || branch.networkType === 'PARTNER') {<button type="button" (click)="openNetwork(branch)" class="min-h-11 rounded-xl bg-violet-700 px-3 font-black text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500">Ağ Kontrolü</button>} @else {<button type="button" (click)="toggleActive(branch)" [disabled]="saving()" class="min-h-11 rounded-xl bg-slate-900 px-3 font-black text-white disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">{{ branch.isActive ? 'Pasife Al' : 'Aktifleştir' }}</button>}
              </div>
            </article>
          } @empty {<div class="rounded-2xl border border-dashed border-slate-300 bg-white p-8 text-center text-sm font-bold text-slate-500">Henüz gerçek şube kaydı yok. Sağdaki formdan ilk merkez şubesini ekleyin.</div>}
        </section>
        ...
        <section class="h-fit rounded-2xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-24" aria-label="Şube düzenleme formu">
          ...
          @if (!isNew() && (draft.networkType === 'FRANCHISE' || draft.networkType === 'PARTNER')) {<div class="mb-4 rounded-xl border border-violet-200 bg-violet-50 p-3 text-xs font-semibold leading-5 text-violet-900">Bu şube yetkili iş ortağıdır. Aktivasyon, fiyat, bayi erişimi ve yayın onayı bu formdan değil ayrı Ağ Kontrolü ekranından yönetilir.</div>}
          <div class="space-y-4">
            ...
            @if (draft.networkType !== 'FRANCHISE' && draft.networkType !== 'PARTNER') {<div class="grid gap-2 sm:grid-cols-3"><label class="check-row"><input type="checkbox" [(ngModel)]="draft.isActive" />Aktif</label><label class="check-row"><input type="checkbox" [(ngModel)]="draft.isPickupPoint" />Teslim Alma</label><label class="check-row"><input type="checkbox" [(ngModel)]="draft.isReturnPoint" />İade</label></div>}
            ...
            <button type="button" (click)="save()" [disabled]="saving()" class="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 font-black text-white disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2">@if (saving()) {<mat-icon class="animate-spin" aria-hidden="true">progress_activity</mat-icon> Kaydediliyor...} @else {<mat-icon aria-hidden="true">save</mat-icon> Şube Bilgilerini Kaydet}</button>
  ...
  `,
  styles: [`.field-label{display:block;margin-bottom:.4rem;font-size:.7rem;font-weight:900;text-transform:uppercase;letter-spacing:.08em;color:#64748b}.field{min-height:48px;width:100%;border:1px solid #cbd5e1;border-radius:.75rem;padding:.75rem 1rem;outline:none;background:white}.field:focus{box-shadow:0 0 0 2px #3b82f6;border-color:#3b82f6}.check-row{display:flex;min-height:48px;align-items:center;gap:.5rem;border-radius:.75rem;background:#f8fafc;padding:0 .75rem;font-size:.8rem;font-weight:800}.check-row input{width:20px;height:20px}`],
})
export class AdminBranchesComponent implements OnInit {
  readonly branchService=inject(BranchService);readonly toastService=inject(ToastService);readonly router=inject(Router);private readonly route=inject(ActivatedRoute);readonly saving=signal(false);readonly isNew=signal(true);readonly errorMessage=signal("");readonly serviceOptions:BranchServiceType[]=["RENTAL","SALES","TOUR","TRANSFER","PICKUP","RETURN"];
  draft:Branch=this.emptyBranch();searchQuery="";
  ...
  filteredBranches():Branch[]{const q=this.searchQuery.trim().toLocaleLowerCase("tr-TR");if(!q)return this.branchService.managedBranches();return this.branchService.managedBranches().filter((branch)=>`${branch.name} ${branch.city} ${branch.district} ${branch.country||""} ${branch.addressLabel||""} ${branch.phone||""}`.toLocaleLowerCase("tr-TR").includes(q));}
  ...
  async save():Promise<void>{if(this.saving())return;this.saving.set(true);this.errorMessage.set("");try{await this.branchService.save(this.clone(this.draft));this.toastService.show("Şube bilgileri Supabase veritabanına kaydedildi.","success");this.isNew.set(false);}catch(error){console.error("Branch save failed",error);this.errorMessage.set("Şube kaydedilemedi. Ülke, saat dilimi, diğer alanlar ve yönetici yetkilerini kontrol edin.");this.toastService.show("Şube kaydedilemedi.","error");}finally{this.saving.set(false);}}
  async toggleActive(branch:Branch):Promise<void>{if(branch.networkType==="FRANCHISE"||branch.networkType==="PARTNER"){this.openNetwork(branch);return;}if(this.saving())return;this.saving.set(true);try{await this.branchService.save({...this.clone(branch),isActive:!branch.isActive,publicStatus:!branch.isActive?"ACTIVE":"SUSPENDED"});this.toastService.show(branch.isActive?"Merkez şubesi pasife alındı.":"Merkez şubesi yeniden aktifleştirildi.","success");}catch(error){console.error("Branch status update failed",error);this.toastService.show("Şube durumu değiştirilemedi.","error");}finally{this.saving.set(false);}}
  ...
  serviceLabel(service:BranchServiceType):string{return({RENTAL:"Kiralama",SALES:"Satış",TOUR:"Tur",TRANSFER:"Transfer",PICKUP:"Teslim Alma",RETURN:"İade"} as Record<BranchServiceType,string>)[service];}networkLabel(branch:Branch):string{return branch.networkType==="FRANCHISE"?"Yetkili İş Ortağı":branch.networkType==="PARTNER"?"Bölgesel Partner":"Merkez Şubesi";}statusLabel(branch:Branch):string{const status=branch.publicStatus||(branch.isActive?"ACTIVE":"SUSPENDED");return({ACTIVE:"Canlı",DRAFT:"Kurulum",SUSPENDED:"Durduruldu",CLOSED:"Kapalı"} as Record<string,string>)[status]||status;}statusClass(branch:Branch):string{const status=branch.publicStatus||(branch.isActive?"ACTIVE":"SUSPENDED");if(status==="ACTIVE")return"bg-emerald-100 text-emerald-800";if(status==="DRAFT")return"bg-amber-100 text-amber-800";return"bg-rose-100 text-rose-800";}
  private emptyBranch():Branch{return{id:"",name:"",city:"Hakkari",district:"Yüksekova",country:"Türkiye",addressLabel:"",phone:"",whatsapp:"",email:"",timezone:"Europe/Istanbul",mapUrl:"",workingHours:[{label:"Çalışma saatleri",value:""}],services:["RENTAL","PICKUP","RETURN"],isActive:true,isPickupPoint:true,isReturnPoint:true,priority:10,networkType:"OWNED",publicStatus:"ACTIVE",customerGuaranteeEnabled:true,centralPricingRequired:true,listingRequiresApproval:true};}
