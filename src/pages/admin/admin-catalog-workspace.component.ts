...
export class AdminCatalogWorkspaceComponent implements OnInit, OnDestroy {
  ...
  async nextStep():Promise<void>{
    if(this.selectedVehicle()||this.selectedTour()){
      const ok=await this.saveProgress({stayOpen:true});
      if(!ok)return;
    }
    this.step.set(Math.min(4,this.step()+1) as WorkspaceStep);
  }
  ...
  async saveProgress(options?:{stayOpen?:boolean}):Promise<boolean>{if(this.selectedVehicle())return this.saveVehicleAs('DRAFT',options);if(this.selectedTour())return this.saveTourAs('DRAFT',options);return true;}
  async saveVehicleAs(status:'DRAFT'|'SCHEDULED'|'PUBLISHED'|'ARCHIVED',options?:{stayOpen?:boolean}):Promise<boolean>{const car=this.selectedVehicle();if(!car)return false;if(status==='SCHEDULED'&&!this.validFutureSchedule(car.scheduledAt)){this.toast.show('Planlı yayın için gelecekte bir tarih ve saat seçin.','error');return false;}car.publicationStatus=status;car.isActive=status==='PUBLISHED'||status==='SCHEDULED';car.publishedAt=status==='PUBLISHED'?new Date().toISOString():car.publishedAt;car.scheduledAt=status==='SCHEDULED'?car.scheduledAt:undefined;car.recordOrigin='REAL';if(this.mode==='RENTAL'&&this.meta(car,'hourlyRentalEnabled')===true){const hourly=Number(this.meta(car,'hourlyPrice')||0),minimum=Number(this.meta(car,'minimumRentalHours')||1);if(hourly<=0||!Number.isInteger(minimum)||minimum<1||minimum>23){this.toast.show('Saatlik kiralama için fiyat ve 1-23 arası minimum saat zorunlu.','error');return false;}}if(this.mode==='SALE'){const truth=this.saleTruthError(car);if(truth){this.toast.show(truth,'error');return false;}}this.saving.set(true);try{await this.editor.saveVehicle(car);await this.reloadSelection();this.toast.show(this.statusSaveMessage(status),'success');if(!options?.stayOpen)this.returnToList(car.id);return true;}catch(error){this.toast.show(this.message(error),'error');return false;}finally{this.saving.set(false);}}
  async saveTourAs(status:'DRAFT'|'SCHEDULED'|'PUBLISHED'|'ARCHIVED',options?:{stayOpen?:boolean}):Promise<boolean>{const tour=this.selectedTour();if(!tour)return false;if(status==='SCHEDULED'&&!this.validFutureSchedule(tour.scheduledAt)){this.toast.show('Planlı yayın için gelecekte bir tarih ve saat seçin.','error');return false;}tour.publicationStatus=status;tour.isActive=status==='PUBLISHED'||status==='SCHEDULED';tour.publishedAt=status==='PUBLISHED'?new Date().toISOString():tour.publishedAt;tour.scheduledAt=status==='SCHEDULED'?tour.scheduledAt:undefined;tour.recordOrigin='REAL';this.saving.set(true);try{await this.editor.saveTour(tour);await this.reloadSelection();this.toast.show(this.statusSaveMessage(status),'success');if(!options?.stayOpen)this.returnToList(tour.id);return true;}catch(error){this.toast.show(this.message(error),'error');return false;}finally{this.saving.set(false);}}
