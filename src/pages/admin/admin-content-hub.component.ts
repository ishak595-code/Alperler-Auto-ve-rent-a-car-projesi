...
import { AdminSaleIntegrityV168Component } from './admin-sale-integrity-v168.component';

type ContentSection='catalog'|'sale-integrity'|'campaigns'|'benefits'|'blog';

@Component({
  selector:'app-admin-content-hub',standalone:true,
  imports:[CommonModule,AdminCatalogEditorComponent,AdminSaleIntegrityV168Component,AdminCampaignsV167Component,AdminCommercialBenefitsComponent,AdminBlogComponent],
  template:`
    <div class="workspace"><header class="workspace-head"><div class="head-row"><button type="button" class="back" (click)="goBack()" aria-label="Önceki sayfaya dön">←</button><div><p>İçerik Yönetimi</p><h1>İçerik & Katalog</h1><span>Araçlar, satış ilanı doğruluğu, turlar, kampanyalar, fiyat kuralları ve blog burada yönetilir.</span></div></div><nav class="tabs" aria-label="İçerik ve katalog bölümleri"><button type="button" [class.active]="active()==='catalog'" (click)="select('catalog')">Araçlar & Turlar</button><button type="button" [class.active]="active()==='sale-integrity'" (click)="select('sale-integrity')">Satış Kalitesi</button><button type="button" [class.active]="active()==='campaigns'" (click)="select('campaigns')">Kampanyalar</button><button type="button" [class.active]="active()==='benefits'" (click)="select('benefits')">Fiyat & Sadakat</button><button type="button" [class.active]="active()==='blog'" (click)="select('blog')">Blog</button></nav></header><main class="content">@switch(active()){@case('sale-integrity'){<app-admin-sale-integrity-v168/>}@case('campaigns'){<app-admin-campaigns-v167/>}@case('benefits'){<app-admin-commercial-benefits/>}@case('blog'){<app-admin-blog/>}@default{<app-admin-catalog-editor/>}}</main></div>
  `,
  styles:[`
