...
@Component({
  selector: 'app-admin-legal-center',
  standalone: true,
      ...
      <div class="mx-auto max-w-6xl space-y-5">
        ...
        @if (activeField(); as field) {
          <section class="rounded-3xl border border-slate-200 bg-white shadow-sm">
            <header class="flex items-start gap-4 border-b border-slate-200 p-5 md:p-6">
              <span class="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-blue-50 text-blue-700"><mat-icon aria-hidden="true">{{ field.icon }}</mat-icon></span>
...
export class AdminLegalCenterComponent implements OnInit {
  ...
  readonly activeKey = signal<keyof SiteConfig>('rentalTermsText');
  form!: SiteConfig;
  ...
  ngOnInit(): void { this.copyConfig(); }

  activeField(): LegalField | undefined { return this.fields.find((field) => field.key === this.activeKey()); }
  fieldValue(key: keyof SiteConfig): string { const value = this.form?.[key]; return typeof value === 'string' ? value : ''; }
  setFieldValue(key: keyof SiteConfig, value: string): void { (this.form as unknown as Record<string, unknown>)[String(key)] = value; }
  ...
  private copyConfig(): void {
    ...
    this.form = { ...cfg, homeContent: cfg.homeContent ? { ...cfg.homeContent } : undefined, team: [...(cfg.team || [])], rentalExtras: cfg.rentalExtras ? [...cfg.rentalExtras] : undefined };
  }
}
