    ...
    <main class="min-h-full bg-slate-50 p-4 md:p-8">
      <div class="mx-auto max-w-7xl space-y-6">
        <header class="rounded-3xl bg-slate-950 p-6 text-white shadow-xl md:p-8">
          ...
          <p class="mt-2 max-w-3xl text-sm leading-relaxed text-slate-300">JPG, PNG, WebP, AVIF, MP4 ve WebM yükleyin. Büyük dosyalar parçalı yüklenir. Kapak görselini, sıralamayı, alt metni ve kaynak bilgisini ayrıca yönetin.</p>
        </header>
        ...
        <section class="grid gap-5 xl:grid-cols-[390px_1fr]">
          <aside class="space-y-5 xl:sticky xl:top-20 xl:self-start">
            ...
            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <p class="mt-1 text-xs text-slate-500">En fazla 150 MB. Video için MP4/WebM.</p>
              ...
              <button type="button" (click)="uploadFiles()" [disabled]="uploading() || !selectedEntityKey || !files().length" class="mt-4 min-h-12 w-full rounded-xl bg-blue-600 font-black text-white disabled:opacity-40">Dosyaları Yükle</button>
            </div>

            <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
              <h2 class="text-lg font-black text-slate-900">Kaynaklı Dış Medya</h2>
              <p class="mt-1 text-xs leading-relaxed text-slate-500">Lisansı uygun, doğrulanmış bir medya URL'sini kaynak ve atıf bilgisiyle ekleyin.</p>
              <div class="mt-4 space-y-3">
                ...
                <label class="field"><span>Kaynak adı</span><input [(ngModel)]="sourceName" placeholder="Wikimedia Commons, Renault Media…" /></label>
                <label class="field"><span>Lisans</span><input [(ngModel)]="license" placeholder="CC BY-SA 4.0, press media…" /></label>
                ...
                <button type="button" (click)="addExternal()" [disabled]="!selectedEntityKey || !externalUrl.trim() || !externalAlt.trim()" class="min-h-12 w-full rounded-xl bg-slate-950 font-black text-white disabled:opacity-40">Kaynaklı Medyayı Ekle</button>
              </div>
            </div>
          </aside>
                ...
                @for (item of currentItems(); track item.id) {
                  <article class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
                    ...
                    <div class="space-y-3 p-4">
                      ...
                      @if (item.sourceName || item.attribution) { <div class="rounded-xl bg-slate-50 p-3 text-[11px] leading-relaxed text-slate-500"><strong class="block text-slate-700">{{ item.sourceName || 'Kaynak' }}</strong>{{ item.attribution || item.license || '' }}</div> }
...
export class AdminMediaComponent implements OnInit {
  ...
  externalUrl = ""; sourceUrl = ""; sourceName = ""; license = ""; attribution = ""; externalAlt = ""; externalAsCover = false;
  ...
  entityChanged(): void { this.files.set([]); this.uploadAlt = ""; this.uploadAsCover = false; }
  ...
  async addExternal(): Promise<void> {
    const entity = this.selectedEntity(); if (!entity) return;
    try {
      await this.media.addExternal({ entityType: entity.type, entityId: entity.id, kind: this.externalKind, url: this.externalUrl.trim(), sourceUrl: this.sourceUrl.trim() || undefined, sourceName: this.sourceName.trim() || undefined, license: this.license.trim() || undefined, attribution: this.attribution.trim() || undefined, altText: this.externalAlt.trim(), isCover: this.externalAsCover, sortOrder: this.currentItems().length + 1 });
      this.externalUrl = ""; this.sourceUrl = ""; this.sourceName = ""; this.license = ""; this.attribution = ""; this.externalAlt = ""; this.externalAsCover = false; await this.refresh(); this.toast.show("Kaynaklı medya eklendi.", "success");
    } catch (error) { this.toast.show(this.message(error), "error"); }
  }
