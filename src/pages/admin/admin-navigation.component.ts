    ...
    <main class="page">
      <div class="shell">
        <header class="hero">
          ...
          <span>Butonları kod yazmadan ekle, kaldır, sırala ve canlı siteye gönder.</span>
        </header>

        <section class="master" aria-labelledby="visibility-title">
          <div><h2 id="visibility-title">Görünürlük</h2><p>İstersen mobil alt barı veya hamburger menüyü tamamen kapatıp daha sonra tekrar açabilirsin.</p></div>
          <div class="switches">
              ...
              @for (item of itemsFor(surface); track item.id; let i = $index) {
                <article class="item" [class.inactive]="!item.isActive">
                  <div class="item-summary">
                    ...
                    <span class="icon"><mat-icon aria-hidden="true">{{ item.icon }}</mat-icon></span>
                    ...
                    <div class="actions" role="group" [attr.aria-label]="item.label + ' buton işlemleri'">
                      ...
                      <button type="button" class="remove" (click)="remove(item)" [attr.aria-label]="item.label + ' butonunu kaldır'">Kaldır</button>
                    </div>
                  </div>
                ...
                <div class="empty">Bu alanda aktif veya kayıtlı buton yok. Yukarıdan yeni bir buton ekleyebilirsin.</div>
              }
            </div>
          </section>
        </ng-template>
...
export class AdminNavigationComponent implements OnInit {
  ...
  availablePresets(surface: NavigationSurface): NavigationPreset[] {
    const existingActive = new Set(this.itemsFor(surface).filter((item) => item.isActive).map((item) => item.itemKey));
    return this.navigation.presets.filter((preset) => !existingActive.has(preset.key));
  }
  ...
  async setEnabled(target: 'dock'|'menu', enabled: boolean): Promise<void> {
    const next: NavigationSettings = { ...this.settings(), mobileDockEnabled: target === 'dock' ? enabled : this.settings().mobileDockEnabled, mobileMenuEnabled: target === 'menu' ? enabled : this.settings().mobileMenuEnabled };
    try { await this.navigation.saveSettings(next); this.toast.show(`${target === 'dock' ? 'Mobil alt bar' : 'Hamburger menü'} ${enabled ? 'açıldı' : 'kapatıldı'}.`,'success'); }
    catch (error) { this.toast.show(this.message(error),'error'); }
  }

  async addPreset(surface: NavigationSurface): Promise<void> {
    const key = this.selectedPreset()[surface]; const preset = this.navigation.presets.find((item) => item.key === key); if (!preset) return;
    try { await this.navigation.addPreset(surface,preset); this.selectPreset(surface,''); this.toast.show('Buton eklendi ve canlıya gönderildi.','success'); }
    catch (error) { this.toast.show(this.message(error),'error'); }
  }

  async addCustom(surface: NavigationSurface): Promise<void> {
    try { await this.navigation.addCustom(surface,{ label:this.customLabel, icon:this.customIcon, route:this.customRoute }); this.customLabel=''; this.customIcon='link'; this.customRoute='/'; this.customSurface.set(null); this.toast.show('Özel buton eklendi.','success'); }
    catch (error) { this.toast.show(this.message(error),'error'); }
  }

  async saveItem(item: NavigationItem): Promise<void> {
    try { await this.navigation.updateItem(item); this.editingId.set(null); this.toast.show('Buton güncellendi ve canlıya gönderildi.','success'); }
    catch (error) { this.toast.show(this.message(error),'error'); }
  }

  async remove(item: NavigationItem): Promise<void> {
    if (typeof window !== 'undefined' && !window.confirm(`“${item.label}” butonunu kaldırmak istiyor musunuz?`)) return;
    try { await this.navigation.deleteItem(item.id); if (this.editingId() === item.id) this.editingId.set(null); this.toast.show('Buton kaldırıldı.','success'); }
    catch (error) { this.toast.show(this.message(error),'error'); }
  }

  async move(surface: NavigationSurface, index: number, delta: number): Promise<void> {
    const list = [...this.itemsFor(surface)]; const target = index + delta; if (target < 0 || target >= list.length) return;
    [list[index],list[target]] = [list[target],list[index]];
    try { await this.navigation.reorder(surface,list.map((item) => item.id)); }
    catch (error) { this.toast.show(this.message(error),'error'); }
  }
