...
type OperationsSection = 'reservations' | 'vehicles' | 'branches-requests' | 'messages' | 'newsletter' | 'branches';

interface OperationsTab { id: OperationsSection; label: string; area: AdminArea; }

@Component({
  selector: 'app-admin-operations-hub',
  standalone: true,
  imports: [CommonModule, AdminReservationsComponent, AdminPartnerRequestsComponent, AdminBranchPartnerRequestsComponent, AdminFeedbackComponent, AdminSubscribersComponent, AdminBranchesComponent],
  template: `
    <div class="workspace">
      <header class="workspace-head">
        <div class="head-row">
          <button type="button" class="back" (click)="goBack()" aria-label="Önceki sayfaya dön">←</button>
          <div><p>Müşteri İşlemleri</p><h1>Operasyonlar</h1><span>Rezervasyonları, başvuruları, mesajları, bülteni ve şubeleri tek alanda yönetin.</span></div>
        </div>
        <nav class="tabs" aria-label="Operasyon bölümleri">
      ...
      <main class="content">
        @switch (active()) {
          ...
          @case ('messages') { <app-admin-feedback /> }
...
export class AdminOperationsHubComponent implements OnInit {
  private readonly route = inject(ActivatedRoute);
  private readonly router = inject(Router);
  private readonly location = inject(Location);
  private readonly access = inject(AdminAccessService);
  readonly active = signal<OperationsSection>('reservations');
  readonly ready = signal(false);

  readonly tabs: OperationsTab[] = [
    { id:'reservations', label:'Rezervasyonlar', area:'operations' },
    { id:'vehicles', label:'Araç Başvuruları', area:'operations' },
    { id:'branches-requests', label:'Bayilik Başvuruları', area:'operations' },
    { id:'messages', label:'Mesajlar', area:'operations' },
    { id:'newsletter', label:'Bülten', area:'operations' },
    { id:'branches', label:'Şubeler', area:'settings' },
  ];
  ...
  async ngOnInit(): Promise<void> {
    ...
    const selected = this.visibleTabs().some((tab) => tab.id === requested) ? requested : (this.visibleTabs()[0]?.id || 'reservations');
    this.active.set(selected);
    if (!this.router.url.startsWith('/admin/operations')) {
      await this.router.navigate(['/admin/operations'], { queryParams: { section: selected }, replaceUrl: true });
    }
  }

  select(section: OperationsSection): void {
  ...
  private resolveSection(): OperationsSection {
    const query = this.route.snapshot.queryParamMap.get('section') as OperationsSection | null;
    if (query && this.tabs.some((tab) => tab.id === query)) return query;
    const data = this.route.snapshot.data['operationsSection'] as OperationsSection | undefined;
    return data && this.tabs.some((tab) => tab.id === data) ? data : 'reservations';
  }
}
