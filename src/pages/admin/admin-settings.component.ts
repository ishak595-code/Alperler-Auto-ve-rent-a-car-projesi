    ...
    <main class="min-h-screen bg-slate-50 text-slate-950">
      <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/95 px-4 py-4 shadow-sm backdrop-blur md:px-8">
        <div class="mx-auto flex max-w-6xl flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <button type="button" (click)="save()" [disabled]="loading() || saving()" class="min-h-11 rounded-xl bg-[#9E1B24] px-5 text-sm font-black text-white shadow-lg shadow-blue-600/20 disabled:opacity-50">{{ saving() ? 'Kaydediliyor…' : 'Kaydet ve Uygula' }}</button>
          </div>
        </div>
      </header>
      ...
      <form class="mx-auto max-w-6xl space-y-6 p-4 pb-12 md:p-8" (submit)="$event.preventDefault(); save()">
          ...
          <div class="grid gap-5 p-5 md:grid-cols-2 md:p-6">
            <div class="md:col-span-2 grid gap-5 rounded-2xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-[150px_1fr]">
              ...
              <div class="grid content-start gap-3">
                <label class="grid gap-1.5"><span class="text-xs font-black uppercase tracking-wide text-slate-500">Logo URL</span><input [(ngModel)]="formConfig.logoUrl" name="logoUrl" class="min-h-12 rounded-xl border border-slate-300 bg-white px-4 outline-none focus:border-[#E15A62]" placeholder="https://…" /></label>
            ...
            <label class="grid gap-1.5"><span class="text-xs font-black uppercase tracking-wide text-slate-500">Şirket Adı</span><input [(ngModel)]="formConfig.companyName" name="companyName" required class="min-h-12 rounded-xl border border-slate-300 bg-slate-50 px-4 font-bold outline-none focus:border-[#E15A62] focus:bg-white" /></label>
            <label class="grid gap-1.5"><span class="text-xs font-black uppercase tracking-wide text-slate-500">Slogan</span><input [(ngModel)]="formConfig.tagline" name="tagline" class="min-h-12 rounded-xl border border-slate-300 bg-slate-50 px-4 outline-none focus:border-[#E15A62] focus:bg-white" /></label>
            <label class="grid gap-1.5"><span class="text-xs font-black uppercase tracking-wide text-slate-500">Masaüstü Logo Genişliği</span><input [(ngModel)]="formConfig.logoWidthDesktop" name="logoWidthDesktop" type="number" min="40" max="500" class="min-h-12 rounded-xl border border-slate-300 bg-slate-50 px-4 outline-none focus:border-[#E15A62] focus:bg-white" /></label>
        ...
        <section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          <div class="grid gap-5 p-5 md:grid-cols-2 md:p-6">
            <label class="grid gap-1.5"><span class="text-xs font-black uppercase tracking-wide text-slate-500">Telefon</span><input [(ngModel)]="formConfig.phone" name="phone" autocomplete="tel" class="min-h-12 rounded-xl border border-slate-300 bg-slate-50 px-4 outline-none focus:border-[#E15A62] focus:bg-white" /></label>
            <label class="grid gap-1.5"><span class="text-xs font-black uppercase tracking-wide text-slate-500">E-posta</span><input [(ngModel)]="formConfig.email" name="email" type="email" autocomplete="email" class="min-h-12 rounded-xl border border-slate-300 bg-slate-50 px-4 outline-none focus:border-[#E15A62] focus:bg-white" /></label>
            <label class="grid gap-1.5 md:col-span-2"><span class="text-xs font-black uppercase tracking-wide text-slate-500">Adres</span><textarea [(ngModel)]="formConfig.address" name="address" rows="3" class="rounded-xl border border-slate-300 bg-slate-50 p-4 outline-none focus:border-[#E15A62] focus:bg-white"></textarea></label>
          </div>
        </section>
