                  ...
                  <article class="rounded-2xl border border-slate-200 p-4" [class.opacity-50]="!admin.isActive">
                    <div class="flex flex-col gap-4 md:flex-row md:items-center">
                      ...
                      <div class="grid gap-2 sm:grid-cols-3 md:w-[430px]">
                        <select [(ngModel)]="admin.role" (change)="saveAdmin(admin)" [attr.aria-label]="(admin.displayName || admin.email) + ' yönetici rolünü seç'" class="min-h-11 rounded-xl border border-slate-200 px-3 text-sm font-bold"><option value="owner">Owner</option><option value="admin">Admin</option><option value="editor">Editör</option><option value="support">Destek</option></select>
                        <select [(ngModel)]="admin.primaryBranchId" (change)="saveAdmin(admin)" [attr.aria-label]="(admin.displayName || admin.email) + ' ana şubesini seç'" class="min-h-11 rounded-xl border border-slate-200 px-3 text-sm"><option [ngValue]="undefined">Şubesiz</option>@for (branch of branches(); track branch.id) { <option [ngValue]="branch.id">{{ branch.name }}</option> }</select>
