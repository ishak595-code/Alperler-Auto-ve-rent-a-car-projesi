...
@Component({
  ...
  template: `
    <main class="workspace">
      <header class="hero"><p>TUR KALİTESİ</p><h1>Tur Programı & Yayın Hazırlığı</h1><span>Tur kartı ve detay sayfasındaki kısa anlatım, süre, kapasite, buluşma noktası, rota programı ve kapsam aynı canonical tur kaydından yönetilir.</span></header>
        ...
        <section class="editor">
          @if(selected();as tour){
            ...
            <div class="grid">
              ...
              <section class="panel"><header><h3>Öne çıkanlar</h3><p>Detay sayfasındaki karar destek alanları.</p></header><label class="textarea"><span>Öne çıkanlar, satır başına bir</span><textarea [ngModel]="highlights(tour).join('\n')" (ngModelChange)="setHighlights(tour,$event)" rows="7"></textarea></label></section>
            </div>
...
export class AdminTourIntegrityV169Component implements OnInit {
  ...
  shortDescriptionLength(tour: TourAdminRecord): number { return String(tour.shortDescription || "").trim().length; }
  publicationReady(tour: TourAdminRecord): boolean { return this.missingFields(tour).length === 0; }
  readinessText(tour: TourAdminRecord): string { const missing = this.missingFields(tour); return missing.length ? `${missing.length} yayın alanı tamamlanmalı` : "Tur içeriği yayına hazır"; }
  missingFields(tour: TourAdminRecord): string[] {
    ...
    const shortLength = this.shortDescriptionLength(tour);
    if (shortLength < 40 || shortLength > 240) missing.push("Kısa açıklama 40–240 karakter");
