...
@Component({selector:"app-admin-vehicle-registry-v171",standalone:true,imports:[CommonModule,FormsModule],template:`
  ...
  <section class="mx-auto max-w-7xl">
    ...
    <section class="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" aria-label="Araç sicili filtreleri">
      <div class="grid gap-3 md:grid-cols-5">
        ...
        <label class="grid gap-1 text-xs font-black text-slate-600">Kayıt başlangıcı
          <input [(ngModel)]="from" type="date" class="field" aria-label="Araç kayıt başlangıç tarihi" />
        </label>
        <label class="grid gap-1 text-xs font-black text-slate-600">Kayıt bitişi
          <input [(ngModel)]="to" type="date" class="field" aria-label="Araç kayıt bitiş tarihi" />
        </label>
      </div>
