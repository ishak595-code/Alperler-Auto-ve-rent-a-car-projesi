...
<main class="min-h-screen bg-slate-100 p-4 text-slate-900 md:p-6">
  <section class="mx-auto max-w-7xl">
    ...
    <section class="mt-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" aria-label="Araç sicili filtreleri">
      ...
      <div class="mt-4 flex flex-wrap gap-2"><button type="button" (click)="search()" [disabled]="loading()" class="min-h-11 rounded-xl bg-[#9E1B24] px-5 font-black text-white disabled:opacity-50">{{loading()?'Aranıyor…':'Ara'}}</button><button type="button" (click)="clear()" class="min-h-11 rounded-xl border border-slate-300 px-5 font-black">Filtreleri Temizle</button></div>
    </section>
