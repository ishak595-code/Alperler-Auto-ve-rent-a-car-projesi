      ...
      <div class="sticky top-[72px] z-40 border-b border-slate-800 bg-slate-900 shadow-lg md:top-[96px]">
        <div class="mx-auto max-w-7xl px-4">
          <div class="flex min-h-16 items-center gap-2 py-2 sm:gap-3">
            <button type="button" (click)="goBack()" class="-ml-2 flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-slate-400 transition-colors hover:bg-slate-800 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E15A62]" aria-label="Geri Dön">
          ...
          <div class="p-4 sm:p-8">
            @if (submitSuccess()) {
              <div class="rounded-xl bg-emerald-50 p-8 text-center text-emerald-900" role="status">
                ...
                <button type="button" (click)="goHome()" class="mt-8 min-h-12 rounded-xl bg-slate-900 px-8 py-3 font-bold text-white transition-colors hover:bg-[#9E1B24] hover:text-slate-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E15A62]">Ana Sayfaya Dön</button>
              </div>
            } @else {
              <form data-analytics-form="booking_conversion" [formGroup]="appointmentForm" (ngSubmit)="onSubmit()" class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-6 pt-4 md:grid-cols-2">
                  <label class="block">
                    ...
                    <input type="text" autocomplete="name" formControlName="name" class="min-h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-[#E15A62]" placeholder="Adınız Soyadınız" />
                  </label>
                  <label class="block">
                    <span class="mb-2 block text-sm font-bold uppercase tracking-wider text-slate-700">Telefon *</span>
                    <input type="tel" inputmode="tel" autocomplete="tel" formControlName="phone" class="min-h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-[#E15A62]" placeholder="05XX XXX XX XX" />
                  </label>
                  <label class="block md:col-span-2">
                    <span class="mb-2 block text-sm font-bold uppercase tracking-wider text-slate-700">E-posta *</span>
                    <input type="email" inputmode="email" autocomplete="email" formControlName="email" class="min-h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-[#E15A62]" placeholder="ornek@email.com" />
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                  <label class="block">
                    ...
                    <input type="date" [min]="minDate" formControlName="date" aria-label="Randevu tarihi seç" title="Randevu tarihi seç" class="min-h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-[#E15A62]" />
                  </label>
                  <label class="block">
                    <span class="mb-2 block text-sm font-bold uppercase tracking-wider text-slate-700">Saat *</span>
                    <input type="time" formControlName="time" aria-label="Randevu saati seç" title="Randevu saati seç" class="min-h-12 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-[#E15A62]" />
                  </label>
                </div>

                <label class="block">
                  <span class="mb-2 block text-sm font-bold uppercase tracking-wider text-slate-700">Mesajınız / Notunuz</span>
                  <textarea formControlName="message" rows="4" class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-[#E15A62]" placeholder="Eklemek istediğiniz detaylar..."></textarea>
                </label>
