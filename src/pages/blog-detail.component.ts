...
@Component({
  ...
  template: `
    <main class="page">
      <header class="topbar">
        <div class="bar">
          <button type="button" (click)="goBack()" aria-label="Rehber yazısından geri dön"><mat-icon aria-hidden="true">arrow_back</mat-icon></button>
          <div><p>ALPERLER YOL REHBERİ</p><strong>{{ post()?.title || 'Rehber Yazısı' }}</strong></div>
        </div>
      </header>
      ...
      @if(post(); as article){
          ...
          <section class="gallery" [attr.aria-label]="article.title + ' fotoğraf ve video galerisi'" (touchstart)="touchStart($event)" (touchend)="touchEnd($event)">
            <div class="frame">
              @if(media.kind==='IMAGE'){
                ...
                <button type="button" class="expand" (click)="openFullscreen()" aria-label="Videoyu tam ekran galeride aç"><mat-icon aria-hidden="true">fullscreen</mat-icon></button>
              }
              <div class="shade"></div>
        ...
        <article class="article">
          ...
          <footer class="article-footer">
            ...
            <div class="share"><button type="button" (click)="sharePost()" aria-label="Bu yazıyı paylaş"><mat-icon aria-hidden="true">share</mat-icon>Paylaş</button><button type="button" (click)="copyLink()" aria-label="Yazı bağlantısını kopyala"><mat-icon aria-hidden="true">link</mat-icon>Bağlantıyı Kopyala</button></div>
            @if(showCopyMsg()){<p class="copied" role="status">Bağlantı kopyalandı.</p>}
            <div class="bottom"><a routerLink="/blog"><mat-icon aria-hidden="true">arrow_back</mat-icon>Tüm Rehberler</a><button type="button" (click)="openContact()">Bize Ulaşın</button></div>
          </footer>
        </article>
        ...
        <section class="state" role="status"><div class="spinner"></div><strong>Yazı hazırlanıyor</strong></section>
      }@else{
        <section class="state" role="alert"><mat-icon aria-hidden="true">error_outline</mat-icon><strong>Bu yazıya şu anda ulaşılamıyor</strong><span>Lütfen kısa bir süre sonra yeniden deneyin veya diğer rehberlere göz atın.</span><a routerLink="/blog">Tüm Rehberlere Dön</a></section>
      }
    </main>
  `,
  styles: [`
...
export class BlogDetailComponent implements OnInit {
  ...
  sharePost(): void {
    const title = this.post()?.title || "Alperler Yol Rehberi";
    const url = window.location.href;
    if (navigator.share) {
      void navigator.share({ title, text: this.post()?.summary || "Alperler Yol Rehberi'ndeki bu yazıyı inceleyin.", url }).catch(() => undefined);
      return;
    }
