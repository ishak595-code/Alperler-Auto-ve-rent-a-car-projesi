  ...
  template: `
    <main class="page">
      ...
      @if(post(); as article){
            ...
            <div class="frame">
              @if(media.kind==='IMAGE'){
                <button type="button" class="media-button" (click)="openFullscreen()" [attr.aria-label]="media.title + ' tam ekran aç'">
                  <img [src]="media.url" [alt]="media.title" decoding="async" />
                </button>
              }@else{
        ...
        @if(fullscreenOpen() && activeMedia(); as full){
          <div class="fullscreen" role="dialog" aria-modal="true" aria-label="Medya tam ekran" (touchstart)="touchStart($event)" (touchend)="touchEnd($event)">
            <button type="button" class="close" (click)="closeFullscreen()" aria-label="Tam ekranı kapat"><mat-icon aria-hidden="true">close</mat-icon></button>
            @if(full.kind==='IMAGE'){<img [src]="full.url" [alt]="full.title"/>}@else{<video [src]="full.url" [poster]="full.posterUrl" controls autoplay playsinline [attr.aria-label]="full.title"></video>}
            @if(article.media.length>1){<button type="button" class="prev" (click)="previous()" aria-label="Önceki medya"><mat-icon aria-hidden="true">chevron_left</mat-icon></button><button type="button" class="next" (click)="next()" aria-label="Sonraki medya"><mat-icon aria-hidden="true">chevron_right</mat-icon></button><b class="count">{{currentMedia()+1}} / {{article.media.length}}</b>}
          </div>
        }
      }@else if(loading()){
