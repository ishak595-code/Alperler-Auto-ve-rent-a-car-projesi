...
@Component({
  ...
  template: `
    <main class="page">
      <header class="topbar">
        <div class="bar">
          <button type="button" (click)="goBack()" [attr.aria-label]="t().blogDetail.backAria"><mat-icon aria-hidden="true">arrow_back</mat-icon></button>
          <div><p>{{ t().blogDetail.kicker }}</p><strong>{{ post()?.title || t().blogDetail.titleFallback }}</strong></div>
        </div>
      </header>

      @if(post(); as article){
        @if(activeMedia(); as media){
          <section class="gallery" [attr.aria-label]="galleryAria(article.title)" (touchstart)="touchStart($event)" (touchend)="touchEnd($event)">
            <div class="frame">
              @if(media.kind==='IMAGE'){
                ...
                <button type="button" class="expand" (click)="openFullscreen()" [attr.aria-label]="t().blogDetail.openVideoFullscreenAria"><mat-icon aria-hidden="true">fullscreen</mat-icon></button>
              }
              <div class="shade"></div>
              <div class="hero-copy"><p>{{article.authorName || t().blogDetail.brandFallback}}</p><h1>{{article.title}}</h1><span>{{article.date}} · {{article.readTime}}</span></div>
              @if(article.media.length>1){<div class="nav"><button type="button" (click)="previous()" [attr.aria-label]="t().blogDetail.prevMediaAria"><mat-icon aria-hidden="true">chevron_left</mat-icon></button><b>{{currentMedia()+1}} / {{article.media.length}}</b><button type="button" (click)="next()" [attr.aria-label]="t().blogDetail.nextMediaAria"><mat-icon aria-hidden="true">chevron_right</mat-icon></button></div>}
            </div>
          </section>
        }@else{
          <section class="title-hero"><div><p>{{article.authorName || t().blogDetail.brandFallback}}</p><h1>{{article.title}}</h1><span>{{article.date}} · {{article.readTime}}</span></div></section>
        }
...
export class BlogDetailComponent implements OnInit {
  ...
  galleryAria(title: string): string {
    return String(this.t().blogDetail.galleryAria || "").replace("{title}", String(title || ""));
  }
  fullscreenAria(title: string): string {
    return String(this.t().blogDetail.openFullscreenAria || "").replace("{title}", String(title || ""));
  }
  sharePost(): void {
    const title = this.post()?.title || this.t().blogDetail.shareTitleFallback;
    const url = window.location.href;
    if (navigator.share) {
      void navigator.share({ title, text: this.post()?.summary || this.t().blogDetail.shareTextFallback, url }).catch(() => undefined);
      return;
    }
  ...
  private async loadPost(id: string): Promise<void> {
    ...
    try {
      ...
      if (sequence !== this.requestSequence) return;
      this.error.set("BLOG_DETAIL_UNAVAILABLE");
    } finally {
