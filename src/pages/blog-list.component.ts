  ...
  template: `
    <main class="min-h-screen bg-[#050A18] pb-20 font-sans text-[#94A3B8]">
      ...
      <section class="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8" aria-label="Blog yazıları">
        <div class="mb-8 max-w-3xl sm:mb-10"><p class="text-[10px] font-black uppercase tracking-[.16em] text-[#60A5FA]">Rehber ve İçerikler</p><h2 class="mt-1 text-2xl font-black text-[#F8FAFC] sm:text-3xl">Yola çıkmadan önce bilmeniz gerekenler</h2><p class="mt-3 text-sm leading-6 text-[#94A3B8] sm:text-base">Araç seçimi, kiralama süreci, yolculuk planlama, bölgesel rotalar ve güvenli sürüş hakkında karar vermenizi kolaylaştıracak yayınlanmış içerikler.</p></div>

        @if (loading()) {
          <div class="rounded-3xl border border-[#24314A] bg-[#0B1224] px-6 py-12 text-center" role="status"><mat-icon aria-hidden="true" class="!h-12 !w-12 !text-5xl text-[#60A5FA]">sync</mat-icon><h2 class="mt-3 text-lg font-black text-[#F8FAFC]">Yazılar yükleniyor</h2></div>
        } @else if (error()) {
          <div class="rounded-3xl border border-[#7F1D1D] bg-[#1F0B12] px-6 py-12 text-center" role="alert"><mat-icon aria-hidden="true" class="!h-12 !w-12 !text-5xl text-[#FCA5A5]">error_outline</mat-icon><h2 class="mt-3 text-lg font-black text-[#F8FAFC]">Yazılar yüklenemedi</h2><p class="mt-2 text-sm text-[#FCA5A5]">{{error()}}</p><button type="button" (click)="load()" class="mt-5 min-h-12 rounded-xl bg-[#2563EB] px-5 font-black text-white">Tekrar Dene</button></div>
...
export class BlogListComponent implements OnInit {
  ...
  async load(): Promise<void> {
    ...
    catch (error) { this.blogPosts.set([]); this.error.set(error instanceof Error ? error.message : "Blog verisi alınamadı."); }
