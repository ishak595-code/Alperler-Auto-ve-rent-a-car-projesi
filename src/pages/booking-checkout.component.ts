      ...
      <div class="mx-auto max-w-4xl space-y-5 px-4 py-6">
        @if (request(); as booking) {
          ...
          @if (isRental()) {
            <section class="rounded-2xl border border-slate-800 bg-slate-900 p-4 sm:p-6" aria-labelledby="rental-details-title">
              ...
              <button
                type="button"
                (click)="openExtras()"
                class="mt-5 flex min-h-14 w-full items-center justify-between gap-3 rounded-xl border border-slate-700 bg-slate-950 px-4 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400"
                aria-label="Teslimat ve ek hizmetleri seç"
              >
                <span class="flex min-w-0 items-center gap-3">
                  ...
                  <span class="min-w-0">
                    <strong class="block text-sm text-white">Teslimat ve Ek Hizmetler</strong>
              ...
              <div class="mt-5 space-y-2 rounded-xl border border-blue-500/20 bg-blue-500/10 p-4" aria-live="polite" aria-atomic="true">
                ...
                @if (extrasTotal() > 0) {
                  <div class="flex items-center justify-between gap-3 text-sm">
                    <span class="text-slate-300">Seçilen ek hizmetler</span>
          ...
          <section class="rounded-2xl border border-slate-800 bg-slate-900 p-4 sm:p-6" aria-labelledby="contact-details-title">
            ...
            <div class="grid gap-4 sm:grid-cols-2">
              <label for="booking-first-name" class="block">
                <span class="mb-2 block text-xs font-bold uppercase text-slate-400">Ad</span>
                <input id="booking-first-name" autocomplete="given-name" [(ngModel)]="firstName" aria-label="Ad" class="min-h-12 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </label>
              <label for="booking-last-name" class="block">
                <span class="mb-2 block text-xs font-bold uppercase text-slate-400">Soyad</span>
                <input id="booking-last-name" autocomplete="family-name" [(ngModel)]="lastName" aria-label="Soyad" class="min-h-12 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </label>
              <label for="booking-phone" class="block">
                <span class="mb-2 block text-xs font-bold uppercase text-slate-400">Telefon</span>
                <input id="booking-phone" type="tel" autocomplete="tel" inputmode="tel" [(ngModel)]="phone" aria-label="Telefon" class="min-h-12 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </label>
              <label for="booking-email" class="block">
                <span class="mb-2 block text-xs font-bold uppercase text-slate-400">E-posta</span>
                <input id="booking-email" type="email" autocomplete="email" inputmode="email" [(ngModel)]="email" aria-label="E-posta" class="min-h-12 w-full rounded-xl border border-slate-700 bg-slate-950 px-4 text-white outline-none focus:ring-2 focus:ring-blue-500" />
              </label>
            </div>
            <label for="booking-note" class="mt-4 block">
              <span class="mb-2 block text-xs font-bold uppercase text-slate-400">Not</span>
              <textarea id="booking-note" rows="3" [(ngModel)]="notes" aria-label="Rezervasyon notu" class="w-full rounded-xl border border-slate-700 bg-slate-950 p-4 text-white outline-none focus:ring-2 focus:ring-blue-500" placeholder="Özel istek veya açıklama"></textarea>
            </label>
          </section>
          ...
          @if (isRental()) {
            <section class="rounded-2xl border border-slate-800 bg-slate-900 p-4 sm:p-6" aria-labelledby="payment-title">
              <div class="mb-5 flex items-center justify-between gap-3">
                <h2 id="payment-title" class="text-lg font-black text-white">3. Ödeme Yöntemi</h2>
              ...
              <div class="grid gap-3 sm:grid-cols-3" role="radiogroup" aria-label="Ödeme yöntemi">
                <button type="button" (click)="selectPayment('CARD')" [attr.aria-pressed]="paymentMethod() === 'CARD'" aria-label="Kart ile ödeme" class="min-h-24 rounded-xl border p-4 text-left transition" [class.border-blue-400]="paymentMethod() === 'CARD'" [class.bg-blue-500]="paymentMethod() === 'CARD'" [class.text-slate-950]="paymentMethod() === 'CARD'" [class.border-slate-700]="paymentMethod() !== 'CARD'">
                  <mat-icon aria-hidden="true">credit_card</mat-icon>
                  <div class="mt-2 font-black">Kart</div>
                  <div class="mt-1 text-xs" [class.text-slate-400]="paymentMethod() !== 'CARD'">{{ paymentService.cardReady() ? 'Güvenli ödeme hazır' : 'Sağlayıcı bekleniyor' }}</div>
                </button>
                ...
                <button type="button" (click)="selectPayment('OFFICE')" [attr.aria-pressed]="paymentMethod() === 'OFFICE'" aria-label="Araç tesliminde ödeme" class="min-h-24 rounded-xl border p-4 text-left transition" [class.border-blue-400]="paymentMethod() === 'OFFICE'" [class.bg-blue-500]="paymentMethod() === 'OFFICE'" [class.text-slate-950]="paymentMethod() === 'OFFICE'" [class.border-slate-700]="paymentMethod() !== 'OFFICE'">
                  ...
                  <div class="mt-2 font-black">Teslimde</div>
                  <div class="mt-1 text-xs" [class.text-slate-400]="paymentMethod() !== 'OFFICE'">Araç tesliminde ödeme</div>
                </button>

                <button type="button" (click)="selectPayment('EFT')" [attr.aria-pressed]="paymentMethod() === 'EFT'" aria-label="Havale veya EFT ile ödeme" class="min-h-24 rounded-xl border p-4 text-left transition" [class.border-blue-400]="paymentMethod() === 'EFT'" [class.bg-blue-500]="paymentMethod() === 'EFT'" [class.text-slate-950]="paymentMethod() === 'EFT'" [class.border-slate-700]="paymentMethod() !== 'EFT'">
                  <mat-icon aria-hidden="true">account_balance</mat-icon>
                  <div class="mt-2 font-black">Havale / EFT</div>
                  <div class="mt-1 text-xs" [class.text-slate-400]="paymentMethod() !== 'EFT'">Bilgiler onay sonrası iletilir</div>
                </button>
              </div>

              @if (paymentMethod() === 'CARD' && !paymentService.cardReady()) {
                <div role="status" class="mt-4 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-100">Kart ödeme sağlayıcısı henüz aktif değil. Şimdilik teslimde ödeme veya EFT seçebilirsiniz.</div>
              }
            </section>
          }
          ...
          @if (successMessage()) {
            <section class="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center" role="status" aria-live="polite">
              <mat-icon aria-hidden="true" class="!h-12 !w-12 !text-[48px] text-emerald-400">check_circle</mat-icon>
              ...
              <button type="button" (click)="finish()" aria-label="Ana sayfaya dön" class="mt-5 min-h-12 rounded-xl bg-white px-6 font-black text-slate-950">Ana Sayfaya Dön</button>
            </section>
          } @else {
            <button type="button" (click)="submit()" [disabled]="isSubmitting() || !isFormValid()" aria-label="Rezervasyon talebini kaydet" class="flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-blue-500 px-6 font-black text-slate-950 disabled:cursor-not-allowed disabled:opacity-50">
              @if (isSubmitting()) { <mat-icon aria-hidden="true" class="animate-spin">progress_activity</mat-icon> İşleniyor... }
        ...
        <div class="fixed inset-0 z-[120] flex items-end justify-center sm:items-center" role="presentation">
          ...
          <section class="relative z-10 max-h-[88vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl border border-slate-700 bg-slate-900 p-4 pb-[max(1rem,env(safe-area-inset-bottom))] text-slate-100 shadow-2xl sm:rounded-3xl sm:p-6" role="dialog" aria-modal="true" aria-labelledby="extras-title">
            <div class="flex items-start justify-between gap-3">
              <div>
                <p class="text-xs font-black uppercase tracking-wider text-blue-400">Rezervasyonu Özelleştir</p>
                <h2 id="extras-title" class="mt-1 text-xl font-black text-white">Teslimat ve Ek Hizmetler</h2>
              </div>
              <button type="button" (click)="closeExtras()" aria-label="Kapat" class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-400"><mat-icon aria-hidden="true">close</mat-icon></button>
            </div>

            <div class="mt-5 grid gap-4 sm:grid-cols-2">
              <label for="pickup-location" class="block">
                <span class="mb-2 block text-xs font-bold uppercase text-slate-400">Teslim Alma Noktası</span>
                <select id="pickup-location" [ngModel]="pickupLocation()" (ngModelChange)="pickupLocation.set($event)" aria-label="Teslim alma noktası" class="min-h-12 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 text-white">
                  @for (choice of pickupChoices(); track choice.key) { <option [value]="choice.label">{{ choice.label }}</option> }
                </select>
              </label>

              <label for="dropoff-location" class="block">
                <span class="mb-2 block text-xs font-bold uppercase text-slate-400">İade Noktası</span>
                <select id="dropoff-location" [ngModel]="dropoffLocation()" (ngModelChange)="dropoffLocation.set($event)" aria-label="İade noktası" class="min-h-12 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 text-white">
                  @for (choice of returnChoices(); track choice.key) { <option [value]="choice.label">{{ choice.label }}</option> }
                </select>
              </label>
            </div>

            <div class="mt-6 space-y-3">
              <h3 class="text-sm font-black text-white">Ek hizmetler</h3>
              @for (extra of rentalExtras(); track extra.id) {
                <label class="flex min-h-16 items-center gap-3 rounded-xl border border-slate-700 bg-slate-950 p-3" [class.opacity-50]="extraUnavailable(extra)">
                  ...
                  <span class="min-w-0 flex-1">
                    <strong class="block text-sm text-white">{{ extra.label }}</strong>
                    @if (extra.description) { <small class="mt-1 block text-xs text-slate-400">{{ extra.description }}</small> }
                  </span>
            ...
            <div class="sticky bottom-0 mt-6 rounded-2xl border border-slate-700 bg-slate-900/95 p-3 backdrop-blur">
              <div class="mb-3 flex items-center justify-between text-sm"><span>Ek hizmet toplamı</span><strong class="text-blue-300">{{ extrasTotal() | number }} ₺</strong></div>
...
export class BookingCheckoutComponent implements OnInit {
  ...
  extraLocked(extra: RentalExtraOption): boolean {
    return extra.id === "driver" && this.request()?.item?.driverOption === "WITH_DRIVER";
  }

  extraUnavailable(extra: RentalExtraOption): boolean {
    return extra.id === "driver" && this.request()?.item?.driverOption === "WITHOUT_DRIVER";
  }
  ...
  extrasButtonSummary(): string {
    ...
    return `${pickup || "Teslim noktası seç"} · ${count ? count + " ek hizmet" : "Ek hizmet seçilmedi"}`;
  }
  ...
  calculatePrice(): void {
    ...
    this.totalPrice.set(rental + extras);
  }
  ...
  async submit(): Promise<void> {
    ...
    if (this.isRental() && this.paymentMethod() === "CARD" && !this.paymentService.cardReady()) {
      this.errorMessage.set("Kart ödeme sağlayıcısı henüz aktif değil. Şimdilik teslimde ödeme veya EFT seçebilirsiniz.");
      return;
    }
    ...
    try {
      ...
      this.successMessage.set(
        this.paymentMethod() === "EFT" && this.isRental()
          ? "Rezervasyon talebiniz ve EFT tercihiniz kaydedildi. Banka bilgileri onay sürecinde iletilecektir."
          : this.isRental()
            ? "Rezervasyon talebiniz kaydedildi. Ekibimiz uygunluğu doğruladıktan sonra sizinle iletişime geçecektir."
            : "Talebiniz kaydedildi. Ekibimiz sizinle iletişime geçecektir.",
      );
    } catch (error) {
  ...
  private buildLocationChoices(branches: Branch[], serviceRuleKey: string): LocationChoice[] {
    ...
    for (const branch of branches) {
      ...
      if (locations.length) {
        locations.forEach((label, index) => values.push({ key: `${branch.id}:${serviceRuleKey}:${index}`, label }));
      } else {
        values.push({ key: `${branch.id}:${serviceRuleKey}:main`, label: `${branch.name} · ${branch.district || branch.city}` });
      }
    }
