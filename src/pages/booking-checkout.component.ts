          ...
          @if (successMessage()) {
            ...
            @if (checkoutStep() === 1) {
              <section class="step-card" aria-labelledby="rental-step-title">
                <p class="step-kicker">1. Planlama</p><h2 id="rental-step-title">Kiralama planınızı belirleyin</h2><p class="step-copy">Tarih, teslim noktası ve sürücü tercihini seçin. Ek hizmetler isteğe bağlıdır ve seçildiğinde toplam fiyata ayrı olarak eklenir.</p>
                ...
                <div class="driver-block" aria-labelledby="driver-choice-title">
                  <div class="driver-title"><span id="driver-choice-title">Sürücü Tercihi</span><small>{{ driverAvailabilityNote() }}</small></div>
                  <div class="choice-grid">
                    <button type="button" (click)="setDriver(false)" [disabled]="!driverAllowed(false)" [attr.aria-pressed]="driverAllowed(false) && !withDriver()" [attr.aria-label]="driverAllowed(false) ? 'Şoförsüz kiralamayı seç' : 'Şoförsüz kiralama bu araçta kullanılamıyor'" [class.active]="driverAllowed(false) && !withDriver()">
                      <mat-icon aria-hidden="true">key</mat-icon><strong>Şoförsüz</strong><small>{{ driverAllowed(false) ? 'Aracı siz kullanırsınız' : 'Bu araçta sunulmuyor' }}</small>
                    </button>
                    <button type="button" (click)="setDriver(true)" [disabled]="!driverAllowed(true)" [attr.aria-pressed]="driverAllowed(true) && withDriver()" [attr.aria-label]="driverAllowed(true) ? 'Şoförlü kiralamayı seç' : 'Şoförlü kiralama bu araçta kullanılamıyor'" [class.active]="driverAllowed(true) && withDriver()">
                      <mat-icon aria-hidden="true">person_pin</mat-icon><strong>Şoförlü</strong><small>{{ driverAllowed(true) ? driverPriceLabel() : 'Bu araçta sunulmuyor' }}</small>
                    </button>
                  </div>
                </div>
                ...
                @if (additionalExtras().length) {
                  <details class="extras-details"><summary><span><strong>Ek Hizmetler</strong><small>{{ selectedAdditionalCount() ? selectedAdditionalCount() + ' hizmet seçildi' : 'İsteğe bağlı' }}</small></span><mat-icon aria-hidden="true">expand_more</mat-icon></summary><div class="extras-list">@for (extra of additionalExtras(); track extra.id) {<label [class.disabled]="extraUnavailable(extra)"><input type="checkbox" [checked]="isExtraSelected(extra.id)" [disabled]="extraUnavailable(extra)" (change)="toggleExtra(extra)" [attr.aria-label]="extra.label + ' ek hizmetini seç'" /><span><strong>{{ extra.label }}</strong>@if (extra.description) {<small class="extra-desc">{{ extra.description }}</small>}<small>{{ extraPriceLabel(extra) }}</small></span></label>}</div></details>
                }

                <div class="price-summary" aria-live="polite">
                  <div><span>Araç · {{ totalDays() }} gün</span><strong>{{ baseRentalTotal() | number }} ₺</strong></div>
                  @if (driverTotal() > 0) {<div><span>Şoför hizmeti</span><strong>{{ driverTotal() | number }} ₺</strong></div>}
                  @if (otherExtrasTotal() > 0) {<div><span>Diğer ek hizmetler</span><strong>{{ otherExtrasTotal() | number }} ₺</strong></div>}
                  @if (matchedDistanceKm() > 0) {<div><span>Mesafe / yakıt · {{ matchedDistanceKm() | number:'1.0-1' }} km</span><strong>{{ routeFuelTotal() | number }} ₺</strong></div>}
                  <div class="total"><span>Tahmini Toplam</span><strong>{{ totalPrice() | number }} ₺</strong></div>
                </div>
...
export class BookingCheckoutComponent implements OnInit {
  ...
  readonly driverTotal = computed(() => { const extra = this.rentalExtras().find((item) => item.id === "driver"); return extra && this.isExtraSelected("driver") && !this.extraUnavailable(extra) ? this.extraAmount(extra) : 0; });
  readonly otherExtrasTotal = computed(() => this.additionalExtras().filter((extra) => this.isExtraSelected(extra.id) && !this.extraUnavailable(extra)).reduce((sum, extra) => sum + this.extraAmount(extra), 0));
  ...
  setDriver(value: boolean): void {
    if (!this.driverAllowed(value)) return;
    this.selectedExtraIds.update((items) => value ? (items.includes("driver") ? items : ["driver", ...items]) : items.filter((id) => id !== "driver"));
    this.calculatePrice();
  }

  driverPriceLabel(): string {
    const extra = this.rentalExtras().find((item) => item.id === "driver");
    return extra ? this.extraPriceLabel(extra) : "Şoför hizmeti";
  }
  ...
  extraUnavailable(extra: RentalExtraOption): boolean {
    if (extra.id !== "driver") return false;
    return !this.driverAllowed(true);
  }

  toggleExtra(extra: RentalExtraOption): void {
    if (this.extraUnavailable(extra)) return;
  ...
  extraAmount(extra: RentalExtraOption): number {
    return Math.round(Number(extra.pricePerDay || 0) * Math.max(1, this.totalDays()) + Number(extra.flatPrice || 0));
  }
  ...
  extraPriceLabel(extra: RentalExtraOption): string {
    ...
    if (!parts.length && extra.pricePerHour) return "Fiyat kiralama süresine göre doğrulanır";
    return parts.join(" + ") || "Ücretsiz";
  }

  continueFromPlan(): void {
    if (!this.startDate || !this.endDate) { this.errorMessage.set("Alış ve iade tarihlerini seçin."); return; }
    if (this.startDate < this.today || this.endDate <= this.startDate) { this.errorMessage.set("İade tarihi alış tarihinden sonra olmalıdır."); return; }
    if (!this.pickupLocation()) { this.errorMessage.set("Teslim alma noktasını seçin."); return; }
    if (!this.driverAllowed(this.withDriver())) { this.errorMessage.set("Bu araç için geçerli sürücü tercihini seçin."); return; }
    if (!this.dropoffLocation()) this.dropoffLocation.set(this.pickupLocation());
  ...
  continueFromContact(): void {
    if (!this.validContact()) { this.errorMessage.set("Ad, soyad, telefon ve geçerli e-posta bilgilerini tamamlayın."); return; }
    this.errorMessage.set(""); this.calculatePrice(); this.checkoutStep.set(3); window.scrollTo({ top: 0, behavior: "smooth" });
  }
  ...
  calculatePrice(): void {
    ...
    const rental = Math.round(Number(booking.basePrice || 0) * days);
  ...
  async submit(): Promise<void> {
    ...
    if (this.isRental() && !this.driverAllowed(this.withDriver())) { this.errorMessage.set("Araç için geçerli sürücü tercihini seçin."); this.checkoutStep.set(1); return; }
    ...
    try {
      ...
      const detailNotes = this.isRental() ? [
        ...
        `Sürücü tercihi: ${this.withDriver() ? "Şoförlü" : "Şoförsüz"}`,
        extrasText ? `Ek hizmetler: ${extrasText}` : "Ek hizmetler: Yok",
        `Araç bedeli: ${this.baseRentalTotal()} TRY`,
        `Şoför bedeli: ${this.driverTotal()} TRY`,
        `Diğer ek hizmetler: ${this.otherExtrasTotal()} TRY`,
      ...
      if (this.isRental() && this.paymentMethod() === "CARD") {
        ...
        this.successMessage.set("Rezervasyon kaydedildi ancak kart ödeme oturumu başlatılamadı. Talebiniz kaybolmadı.");
        return;
      }
      ...
      this.errorMessage.set(detail.includes("INVALID_RENTAL_VEHICLE") ? "Araç kaydı doğrulanamadı. Araç sayfasına dönüp tekrar deneyin." : "Talep kaydedilemedi. Bilgilerinizi kontrol edip tekrar deneyin.");
