...
<main class="min-h-screen bg-slate-950 text-white pb-20">
  <section class="border-b border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950">
    <div class="mx-auto max-w-7xl px-4 py-14 md:px-8 md:py-20">
      <p class="text-xs font-black uppercase tracking-[.18em] text-[#E15A62]">Türkiye Geneli Alperler Şube Ağı</p>
      ...
      <div class="mt-8 grid gap-3 rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur md:grid-cols-[1fr_1fr_.7fr_auto]">
        ...
        <button type="button" (click)="search()" [disabled]="loading()||geo.loading()" class="self-end min-h-12 rounded-xl bg-[#9E1B24] px-6 font-black text-white disabled:opacity-50">{{loading()?'Aranıyor…':'Sonuçları Getir'}}</button>
      </div>
    </div>
  </section>
  ...
  <section class="mx-auto max-w-7xl px-4 py-8 md:px-8">
    ...
    @if(result()){
      ...
      <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        @for(item of result()!.records;track item.cloudId){<article class="overflow-hidden rounded-3xl border border-white/10 bg-slate-900 shadow-2xl"><div class="aspect-[16/9] bg-slate-800">@if(item.image){<img [src]="item.image" [alt]="title(item)" class="h-full w-full object-cover"/>}@else{<div class="grid h-full place-items-center text-slate-500"><mat-icon aria-hidden="true">directions_car</mat-icon></div>}</div><div class="p-5"><div class="flex items-start justify-between gap-3"><div><span class="rounded-full bg-[#9E1B24]/15 px-2 py-1 text-[10px] font-black uppercase tracking-wider text-[#E15A62]">{{kindLabel(item.category)}}</span><h2 class="mt-2 text-xl font-black">{{title(item)}}</h2></div><strong class="text-lg">{{item.price|number:'1.0-0'}} ₺</strong></div><div class="mt-4 rounded-2xl bg-white/5 p-4"><div class="flex items-center gap-2 text-sm font-black"><mat-icon aria-hidden="true">storefront</mat-icon>{{item.operatorName}}</div><p class="mt-1 text-xs text-slate-400">{{item.city}} / {{item.district}}</p><p class="mt-3 text-xs leading-5 text-slate-400">{{item.branch.platformDisclaimer}}</p></div><div class="mt-4 grid grid-cols-2 gap-2"><a [routerLink]="detailLink(item)" class="grid min-h-11 place-items-center rounded-xl bg-[#9E1B24] font-black text-white no-underline">Detay</a><a [routerLink]="['/branches',item.branch.slug]" class="grid min-h-11 place-items-center rounded-xl border border-white/15 font-black text-white no-underline">Şubeyi Gör</a></div></div></article>}
