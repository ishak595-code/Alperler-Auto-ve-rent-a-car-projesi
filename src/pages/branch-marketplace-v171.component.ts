...
<main class="min-h-screen bg-slate-950 text-white pb-20">
  <section class="border-b border-white/10 bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950">
    <div class="mx-auto max-w-7xl px-4 py-14 md:px-8 md:py-20">
      <p class="text-xs font-black uppercase tracking-[.18em] text-blue-300">Türkiye geneli doğrulanmış şube ağı</p>
      <h1 class="mt-3 max-w-4xl text-4xl font-black leading-tight md:text-6xl">İlinizi ve ilçenizi seçin, bölgenizdeki gerçek araç ve turları görün.</h1>
      <p class="mt-5 max-w-3xl text-base leading-7 text-slate-300">Sonuçlar şubenin kayıtlı il ve ilçe koduna göre gelir. Her ilanda yerel işletmenin adı ve Alperler Auto'nun platform rolü açıkça gösterilir.</p>
  ...
  <section class="mx-auto max-w-7xl px-4 py-8 md:px-8">
    ...
    @if(result()){
      ...
      <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        ...
        @empty{<div class="col-span-full rounded-3xl border-2 border-dashed border-white/15 p-12 text-center"><mat-icon class="text-slate-500" aria-hidden="true">travel_explore</mat-icon><h2 class="mt-3 text-xl font-black">Bu konumda henüz yayınlanmış sonuç yok</h2><p class="mt-2 text-sm text-slate-400">Farklı bir ilçe seçebilir veya tüm ili arayabilirsiniz.</p></div>}
      </div>
    }
  </section>
...
export class BranchMarketplaceV171Component implements OnInit{
  ...
  ngOnInit():void{void this.geo.ensureLoaded().then(()=>this.search()).catch(()=>this.error.set("Konum dizini yüklenemedi."));}
  provinceChanged(value:string):void{this.provinceCode=value;this.districtCode="";}
  async search():Promise<void>{if(this.loading())return;this.loading.set(true);this.error.set("");try{this.result.set(await this.marketplace.search(this.provinceCode,this.districtCode,this.kind));}catch(error){this.error.set(error instanceof Error?error.message:"Arama yapılamadı.");}finally{this.loading.set(false);}}
