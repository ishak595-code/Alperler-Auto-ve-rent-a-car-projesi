...
<main class="min-h-screen bg-slate-950 pb-20 text-white">
  <section class="border-b border-white/10 bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950">
    <div class="mx-auto max-w-6xl px-4 py-16 text-center md:px-8 md:py-24">
      <p class="text-xs font-black uppercase tracking-[.2em] text-[#E15A62]">Alperler Auto Şube Ağı</p>
  ...
  <section class="mx-auto max-w-6xl px-4 py-12 md:px-8">
    ...
    <div class="grid gap-5 lg:grid-cols-3">
      @for(plan of plans();track plan.id){<article [class.ring-2]="plan.code==='PRO'" [class.ring-blue-500]="plan.code==='PRO'" class="relative flex flex-col rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl">@if(plan.code==='PRO'){<span class="absolute -top-3 left-6 rounded-full bg-[#9E1B24] px-3 py-1 text-xs font-black">ÖNERİLEN</span>}<p class="text-xs font-black uppercase tracking-[.16em] text-[#E15A62]">{{plan.code}}</p><h2 class="mt-2 text-2xl font-black">{{plan.name}}</h2><p class="mt-3 min-h-16 text-sm leading-6 text-slate-400">{{plan.short_description}}</p><div class="mt-6"><strong class="text-4xl font-black">{{plan.monthly_fee|number:'1.0-0'}} ₺</strong><span class="ml-2 text-sm text-slate-400">/ {{plan.billing_interval==='YEARLY'?'yıl':'ay'}}</span></div><p class="mt-3 text-sm font-bold text-[#E15A62]">{{headline(plan)}}</p><ul class="mt-5 flex-1 space-y-3">@for(item of benefits(plan);track item){<li class="flex gap-2 text-sm leading-6 text-slate-300"><mat-icon class="mt-0.5 text-emerald-400" aria-hidden="true">check_circle</mat-icon><span>{{item}}</span></li>}</ul><a routerLink="/branch-partner" [queryParams]="{plan:plan.code}" class="mt-7 grid min-h-12 place-items-center rounded-xl bg-white font-black text-slate-950 no-underline">Bu Paketle Başvur</a></article>}
    </div>
...
</main>`,styles:[`:host{display:block}.value{display:flex;min-height:140px;flex-direction:column;gap:.45rem;border:1px solid rgba(255,255,255,.1);border-radius:20px;background:rgba(255,255,255,.05);padding:1rem}.value mat-icon{color:#E15A62}.value strong{font-weight:900}.value span{color:#94a3b8;font-size:.75rem;line-height:1.5}`]})
