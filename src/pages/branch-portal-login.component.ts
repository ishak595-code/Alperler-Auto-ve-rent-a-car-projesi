...
@Component({
    ...
    <main class="min-h-screen bg-slate-950 px-4 py-10 text-slate-200">
      <div class="mx-auto grid min-h-[calc(100vh-5rem)] max-w-5xl items-center gap-8 lg:grid-cols-[1.05fr_.95fr]">
        <section class="hidden lg:block">
          <p class="text-xs font-black uppercase tracking-[.18em] text-[#E15A62]">Alperler Auto Şube Ağı</p>
        ...
        <section class="rounded-3xl border border-slate-800 bg-slate-900 p-5 shadow-2xl sm:p-8" aria-labelledby="branch-login-title">
          ...
          @if (inviteMode()) {
            ...
            <div class="mt-5"><p class="text-xs font-black uppercase tracking-wider text-[#E15A62]">Yetkili Şube Girişi</p><h2 id="branch-login-title" class="mt-2 text-2xl font-black text-white">Şube Portalı</h2><p class="mt-2 text-sm leading-6 text-slate-400">Merkez tarafından yetkilendirilen e-posta ve şifrenizle giriş yapın.</p></div>
  ...
  `,
  styles: [`
    .benefit{display:flex;min-height:150px;flex-direction:column;gap:.45rem;border:1px solid #1e293b;border-radius:18px;background:#0f172a;padding:1rem}.benefit mat-icon{color:#E15A62}.benefit strong{color:white}.benefit span{font-size:.72rem;line-height:1.25rem;color:#94a3b8}.label{display:block;margin-bottom:.4rem;font-size:.65rem;font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:#94a3b8}.field{min-height:50px;width:100%;border:1px solid #334155;border-radius:13px;background:#020617;padding:0 .9rem;color:white;outline:none}.field:focus{border-color:#E15A62;box-shadow:0 0 0 3px rgba(96,165,250,.13)}.action{min-height:52px;width:100%;border:0;border-radius:13px;background:#9E1B24;padding:0 1rem;font-weight:900;color:white}.action:disabled{opacity:.55}.error{margin:0;border:1px solid rgba(244,63,94,.25);border-radius:12px;background:rgba(244,63,94,.1);padding:.75rem;color:#fecdd3;font-size:.78rem;font-weight:800;line-height:1.3rem}
  `],
})
