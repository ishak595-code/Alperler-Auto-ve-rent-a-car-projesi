...
@Component({
      ...
      <header class="sticky top-0 z-40 border-b border-slate-200 bg-white/95 shadow-sm backdrop-blur">
        <div class="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-3 px-4 md:px-8">
          <div class="flex min-w-0 items-center gap-3">
            ...
            <div class="min-w-0"><p class="text-[10px] font-black uppercase tracking-[.16em] text-prestige-red">Alperler Auto Şube Portalı</p><h1 class="truncate text-lg font-black">{{ currentBranch()?.name || 'Şube çalışma alanı' }}</h1></div>
          </div>
          ...
          @if (tab()==='OVERVIEW') {
            <section class="grid gap-5 lg:grid-cols-[1.15fr_.85fr]">
              ...
              <aside class="space-y-5">
                ...
                @if (currentBranch()?.slug && currentBranch()?.publicStatus==='ACTIVE') {<a [routerLink]="['/branches',currentBranch()?.slug]" class="flex min-h-14 items-center justify-between rounded-2xl bg-prestige-red px-5 font-black text-white">Canlı Şube Sayfasını Gör <mat-icon aria-hidden="true">open_in_new</mat-icon></a>}
              </aside>
            </section>
          }
        ...
        <div class="fixed inset-0 z-[100] overflow-y-auto bg-slate-950/75 p-3 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="listing-editor-title">
          <div class="mx-auto my-4 max-w-3xl rounded-3xl bg-white shadow-2xl">
            ...
            <div class="space-y-5 p-5 sm:p-6">
              ...
              <fieldset class="rounded-2xl border border-slate-200 p-4"><legend class="px-1 text-xs font-black uppercase tracking-wider text-slate-500">Gerçek Araç Fotoğrafları</legend><p class="mt-1 text-xs leading-5 text-slate-500">JPG, PNG, WEBP veya AVIF. Fotoğraflar yalnızca bu şubenin klasörüne yüklenir. İlk fotoğraf kapak olur.</p><label class="mt-3 flex min-h-12 cursor-pointer items-center justify-center gap-2 rounded-xl border border-dashed border-slate-300 bg-slate-50 font-black"><mat-icon aria-hidden="true">add_photo_alternate</mat-icon>{{ uploading() ? 'Fotoğraflar yükleniyor...' : 'Fotoğraf Seç' }}<input type="file" class="sr-only" multiple accept="image/*" (change)="uploadImages($event)" [disabled]="uploading()" /></label>@if(draft.images?.length){<div class="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-5">@for(image of draft.images; track image; let i=$index){<div class="relative aspect-square overflow-hidden rounded-xl bg-slate-100"><img [src]="image" [alt]="'Araç fotoğrafı ' + (i+1)" class="h-full w-full object-cover"/><button type="button" (click)="removeImage(i)" class="absolute right-1 top-1 grid h-8 w-8 place-items-center rounded-full bg-slate-950/80 text-white" [attr.aria-label]="'Fotoğraf ' + (i+1) + ' kaldır'"><mat-icon class="!text-[18px]" aria-hidden="true">close</mat-icon></button></div>}</div>}</fieldset>
  ...
  `,
  styles: [`
    .metric{display:flex;min-height:84px;flex-direction:column;align-items:center;justify-content:center;border:1px solid #1e293b;border-radius:14px;background:#020617}.metric strong{font-size:1.45rem}.metric span{margin-top:2px;font-size:.65rem;font-weight:800;color:#94a3b8}.card{border:1px solid #e2e8f0;border-radius:18px;background:white;padding:1.15rem;box-shadow:0 5px 18px rgba(15,23,42,.04)}.card-head{display:flex;align-items:start;justify-content:space-between;gap:1rem}.card h2{margin-top:.15rem;font-size:1.1rem;font-weight:900}.card-head>strong{font-size:1.2rem}.kicker{font-size:.62rem;font-weight:900;letter-spacing:.1em;text-transform:uppercase;color:var(--alper-blue)}.check-item{display:grid;grid-template-columns:auto 1fr auto;align-items:start;gap:.65rem;border-radius:12px;background:#f8fafc;padding:.75rem}.check-item strong{display:block;font-size:.78rem}.check-item p{margin-top:.15rem;font-size:.68rem;line-height:1.15rem;color:#64748b}.check-item>span{font-size:.65rem;font-weight:900;color:#64748b}.summary{display:flex;min-height:72px;flex-direction:column;align-items:center;justify-content:center;border-radius:12px;background:#f8fafc}.summary strong{font-size:1.2rem}.summary span{font-size:.65rem;font-weight:800;color:#64748b}.guard{display:flex;gap:.7rem}.guard mat-icon{flex:none;color:var(--alper-blue)}.guard strong{display:block;font-size:.8rem}.guard p{margin-top:.15rem;font-size:.7rem;line-height:1.2rem;color:#64748b}.field-label{display:block;margin-bottom:.4rem;font-size:.65rem;font-weight:900;letter-spacing:.07em;text-transform:uppercase;color:#64748b}.field{min-height:48px;width:100%;border:1px solid #cbd5e1;border-radius:12px;background:white;padding:.7rem .85rem;outline:none}.field:focus{border-color:#3b82f6;box-shadow:0 0 0 3px rgba(59,130,246,.12)}.primary,.primary-small,.secondary,.secondary-danger{display:flex;align-items:center;justify-content:center;gap:.3rem;border-radius:12px;font-weight:900}.primary{min-height:48px;background:var(--alper-blue);padding:0 1rem;color:white}.primary-small{min-height:42px;background:#0f172a;padding:0 .8rem;color:white;font-size:.75rem}.secondary{min-height:44px;border:1px solid #cbd5e1;background:white;padding:0 .8rem;color:#0f172a}.secondary-danger{min-height:44px;border:1px solid #fecdd3;background:#fff1f2;padding:0 .8rem;color:#be123c}.msg{margin-bottom:1rem;border-radius:12px;padding:.8rem 1rem;font-size:.78rem;font-weight:800}.msg.success{background:#ecfdf5;color:#065f46}.msg.error{background:#fff1f2;color:#9f1239}
  `],
})
