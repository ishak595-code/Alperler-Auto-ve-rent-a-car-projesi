      ...
      @if (vehicle(); as car) {
        ...
        <div class="detail-layout">
          ...
          <section class="panel all-details-panel" aria-labelledby="all-details-title">
            <button type="button" class="all-details-toggle" (click)="detailsOpen.update(v=>!v)" [attr.aria-expanded]="detailsOpen()" aria-controls="rental-all-details"><span><small>{{ t().carDetail.aboutToggleSmall }}</small><strong id="all-details-title">Konfor, kiralama koşulları ve araç bilgileri</strong></span><mat-icon aria-hidden="true">{{detailsOpen()?'expand_less':'expand_more'}}</mat-icon></button>
            @if(detailsOpen()){
        ...
        <nav class="fixed-actions" [attr.aria-label]="t().carDetail.actionsAria">
          @if(phoneHref()){<a class="phone" [href]="phoneHref()" [attr.aria-label]="t().carDetail.callAria"><span class="phone-symbol" aria-hidden="true">☎</span><span>{{ t().carDetail.call }}</span></a>}@else{<button type="button" class="phone" disabled aria-label="Telefonla arama şu anda kullanılamıyor"><span class="phone-symbol" aria-hidden="true">☎</span><span>{{ t().carDetail.call }}</span></button>}
...
export class CarDetailComponent implements OnInit {
  ...
  driverOptionLabel(option:Car["driverOption"]):string{return option==="WITH_DRIVER"?this.t().carDetail.withDriver:option==="WITHOUT_DRIVER"?this.t().carDetail.withoutDriver:this.t().carDetail.driverBoth;}galleryAria(car:Car){return String(this.t().carDetail.galleryAria||'').replace('{brand}',String(car.brand||'')).replace('{model}',String(car.model||''));}fullscreenAria(car:Car){return String(this.t().carDetail.fullscreenAria||'').replace('{brand}',String(car.brand||'')).replace('{model}',String(car.model||''));}stockLabel(car:Car){return String(this.t().carDetail.stockNo||'').replace('{id}', String(car.cloudStockCode||car.id));}peopleLabel(n:number){return String(this.t().carDetail.people||'').replace('{n}', String(n));}
