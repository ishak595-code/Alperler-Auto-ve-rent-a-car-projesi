      ...
      @if (vehicle(); as car) {
        <header class="detail-header">
          <div class="header-inner">
            <div class="header-left">
              <button type="button" class="icon-button" (click)="goBack()" aria-label="Kiralık araçlardan geri dön"><mat-icon aria-hidden="true">arrow_back</mat-icon></button>
              <div class="header-copy"><span>KİRALIK ARAÇ · CANLI KATALOG</span><h1>{{car.brand}} {{car.model}}</h1></div>
            </div>
        ...
        <section class="gallery" [attr.aria-label]="car.brand+' '+car.model+' fotoğraf ve video galerisi'" (touchstart)="touchStart($event)" (touchend)="touchEnd($event)">
          @if(activeMedia();as media){
            ...
            <div class="gallery-empty"><mat-icon aria-hidden="true">directions_car</mat-icon><strong>Araç medyası yüklenemedi</strong></div>
          }
        </section>
        ...
        @if(activeCampaign();as offer){
          <section class="campaign-strip" aria-label="Bu araca ait aktif kampanya">
            <div class="campaign-copy">
              <div class="campaign-badges"><span class="campaign-kicker">{{offer.badge||'KAMPANYA'}}</span>@if(discountLabel(offer)){<span class="discount-badge">{{discountLabel(offer)}}</span>}</div>
              <h2>{{offer.title}}</h2>
        ...
        <div class="detail-layout">
          <div class="detail-main">
            <section class="panel summary-panel" aria-labelledby="rental-summary-title">
              <div class="summary-head">
                <div><p class="stock-code">{{car.cloudStockCode||('Araç No '+car.id)}}</p><h2 id="rental-summary-title">{{car.brand}} @if(car.series){<span>{{car.series}}</span>} {{car.model}}</h2><p class="summary-meta">{{summaryMeta(car)}}</p></div>
              ...
              <dl class="facts" aria-label="Temel araç bilgileri">
                ...
                <div><dt>Gövde</dt><dd>{{display(car.type)}}</dd></div>
            ...
            <section class="panel all-details-panel" aria-labelledby="all-details-title">
              <button type="button" class="all-details-toggle" (click)="detailsOpen.update(v=>!v)" [attr.aria-expanded]="detailsOpen()" aria-controls="rental-all-details"><span><small>ARAÇ DETAYLARI</small><strong id="all-details-title">Özellikler, koşullar ve açıklama</strong></span><mat-icon aria-hidden="true">{{detailsOpen()?'expand_less':'expand_more'}}</mat-icon></button>
              @if(detailsOpen()){
                <div id="rental-all-details" class="all-details-content">
                  <section class="details-group"><h3>Kiralama Bilgileri</h3><dl class="detail-list"><div><dt>Konum</dt><dd>{{display(car.location)}}</dd></div><div><dt>Sürücü seçeneği</dt><dd>{{driverOptionLabel(car.driverOption)}}</dd></div>@if(car.dailyMileageLimit){<div><dt>Günlük kilometre</dt><dd>{{car.dailyMileageLimit}} km</dd></div>}@if(car.hourlyRentalEnabled){<div><dt>Saatlik kiralama</dt><dd>Minimum {{car.minimumRentalHours||1}} saat</dd></div>}@if(car.hourlyMileageLimit){<div><dt>Saatlik kilometre</dt><dd>{{car.hourlyMileageLimit}} km</dd></div>}@if(car.deposit!==undefined){<div><dt>Depozito</dt><dd>{{car.deposit|turkishCurrency}}</dd></div>}@if(car.minAge){<div><dt>Minimum yaş</dt><dd>{{car.minAge}}</dd></div>}@if(car.minLicenseYears){<div><dt>Minimum ehliyet</dt><dd>{{car.minLicenseYears}} yıl</dd></div>}@if(car.luggage){<div><dt>Bagaj</dt><dd>{{car.luggage}}</dd></div>}@if(car.group){<div><dt>Araç grubu</dt><dd>{{car.group}}</dd></div>}</dl></section>
                  @if(features().length){<section class="details-group"><h3>Donanım ve Konfor</h3><ul class="feature-list">@for(feature of features();track feature){<li><mat-icon aria-hidden="true">check_circle</mat-icon><span>{{feature}}</span></li>}</ul></section>}
                  @if(technicalRows().length){<section class="details-group"><h3>Teknik Veriler</h3><dl class="tech-grid">@for(row of technicalRows();track row.label){<div><dt>{{row.label}}</dt><dd>{{row.value}}</dd></div>}</dl></section>}
                  @if(car.description){<section class="details-group"><h3>Açıklama</h3><p class="description">{{car.description}}</p></section>}
                </div>
              }
            </section>
          </div>
          ...
          <aside class="reservation-panel" aria-labelledby="reservation-title">
            <p>Rezervasyon Özeti</p><h2 id="reservation-title">Planınıza uygun aracı ayırtın</h2>
            <div class="reservation-price"><small>{{presetDuration==='hourly'?'Saatlik başlangıç':'Günlük başlangıç'}}</small><strong>{{selectedUnitPrice(car)|turkishCurrency}}</strong>@if(activeCampaign()){<span class="campaign-price-note">Aktif kampanya rezervasyonda otomatik doğrulanır.</span>}</div>
            @if(presetDuration==='hourly'&&!car.hourlyRentalEnabled){<p class="unavailable-note">Bu araç saatlik kiralamaya açık değil.</p>}@else if(!selectedPeriodAvailable()){<p class="unavailable-note">Bu araç seçtiğiniz zaman aralığında müsait değil.</p>}@else{<p class="availability-note"><mat-icon aria-hidden="true">verified</mat-icon>Seçiminiz rezervasyon aşamasında canlı olarak yeniden doğrulanır.</p>}
          </aside>
        </div>

        <nav class="fixed-actions" aria-label="Araç hızlı işlemleri">
          @if(phoneHref()){<a class="phone" [href]="phoneHref()" aria-label="Telefonla ara"><span class="phone-symbol" aria-hidden="true">☎</span><span>Ara</span></a>}@else{<button type="button" class="phone" disabled aria-label="Telefon numarası tanımlı değil"><span class="phone-symbol" aria-hidden="true">☎</span><span>Ara</span></button>}
          <button type="button" class="whatsapp" (click)="whatsapp()"><mat-icon aria-hidden="true">chat</mat-icon><span>WhatsApp</span></button>
        ...
        <section class="state-panel error" role="alert"><mat-icon aria-hidden="true">error_outline</mat-icon><strong>Araç kaydı yüklenemedi</strong><span>{{loadError()}}</span><button type="button" (click)="reload()">Tekrar Dene</button></section>
      }
    </main>
  `,
  styles: [`
    :host{display:block;background:#050914;color:#f8fafc}.detail-page{min-height:100dvh;padding-bottom:92px;background:#050914;font-family:Inter,system-ui,sans-serif}.detail-header{position:sticky;top:0;z-index:70;border-bottom:1px solid #1e293b;background:rgba(7,16,32,.98);backdrop-filter:blur(14px)}.header-inner{width:min(100% - 24px,1180px);min-height:70px;margin:auto;display:flex;align-items:center;justify-content:space-between;gap:12px}.header-left,.header-actions{display:flex;align-items:center;gap:8px}.header-copy{min-width:0}.header-copy span{display:block;color:#93c5fd;font-size:9px;font-weight:950;text-transform:uppercase;letter-spacing:.09em}.header-copy h1{margin:3px 0 0;max-width:62vw;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:19px}.icon-button{display:grid;width:46px;height:46px;place-items:center;border:0;border-radius:14px;background:#0d1729;color:#fff}.favorite-active{color:#fb7185}.gallery{background:#020617}.gallery-frame{position:relative;width:min(100%,1180px);margin:auto;aspect-ratio:16/10;overflow:hidden}.media-open,.media-open img,.gallery-frame>video{display:block;width:100%;height:100%}.media-open{border:0;background:#020617;padding:0}.media-open img,.gallery-frame>video{object-fit:cover}.video-expand{position:absolute;z-index:4;top:12px;right:12px;display:grid;width:44px;height:44px;place-items:center;border:1px solid #334155;border-radius:13px;background:rgba(2,6,23,.8);color:#fff}.gallery-toolbar{position:absolute;left:14px;right:14px;bottom:13px;display:flex;align-items:center;justify-content:space-between}.gallery-toolbar>span{border-radius:999px;background:rgba(0,0,0,.72);padding:7px 11px;font-size:11px;font-weight:900}.gallery-toolbar>div{display:flex;gap:7px}.gallery-toolbar button{display:grid;width:44px;height:44px;place-items:center;border:0;border-radius:50%;background:rgba(0,0,0,.72);color:#fff}.gallery-empty{min-height:340px;display:grid;place-content:center;gap:8px;text-align:center;color:#94a3b8}.campaign-strip{width:min(100% - 24px,1180px);margin:14px auto 0;display:grid;gap:14px;border:1px solid rgba(251,191,36,.4);border-radius:20px;background:radial-gradient(circle at 95% 0,rgba(251,191,36,.16),transparent 38%),linear-gradient(135deg,#451a03,#7c2d12 55%,#111827);padding:17px;box-shadow:0 14px 34px rgba(0,0,0,.2)}.campaign-badges{display:flex;gap:7px;flex-wrap:wrap}.campaign-kicker,.discount-badge{display:inline-flex;min-height:26px;align-items:center;border-radius:999px;padding:0 9px;font-size:9px;font-weight:950}.campaign-kicker{background:#fbbf24;color:#451a03}.discount-badge{background:#fff;color:#7f1d1d}.campaign-strip h2{margin:8px 0 0;font:900 23px Georgia,serif}.campaign-strip p{margin:6px 0 0;color:#ffedd5;line-height:1.6}.campaign-copy ul{list-style:none;margin:10px 0 0;padding:0;display:grid;gap:5px}.campaign-copy li{display:flex;gap:6px;color:#fff7ed;font-size:11px}.campaign-copy li mat-icon{width:17px;height:17px;font-size:17px;color:#fcd34d}.campaign-value{display:flex;flex-wrap:wrap;align-items:center;gap:8px}.campaign-value>span{display:flex;align-items:center;gap:5px;border:1px solid rgba(255,255,255,.14);border-radius:999px;background:rgba(15,23,42,.42);padding:7px 10px;font-size:10px;font-weight:900}.campaign-value .proof.hot{background:rgba(124,45,18,.66)}.live-dot{width:7px;height:7px;border-radius:50%;background:#22c55e;box-shadow:0 0 0 4px rgba(34,197,94,.13)}.campaign-value mat-icon{width:16px;height:16px;font-size:16px;color:#fcd34d}.price-offer{border-radius:13px;background:rgba(2,6,23,.5);padding:10px 12px}.price-offer small,.price-offer strong,.price-offer em{display:block}.price-offer small{color:#fdba74;text-decoration:line-through}.price-offer strong{font-size:22px}.price-offer em{margin-top:2px;color:#fde68a;font-size:9px;font-style:normal}.detail-layout{width:min(100% - 24px,1180px);margin:auto;padding:18px 0 22px;display:grid;gap:18px}.detail-main{display:grid;gap:14px}.panel,.reservation-panel{border:1px solid #253149;border-radius:20px;background:#0b1220;padding:18px;box-shadow:0 14px 34px rgba(0,0,0,.18)}.summary-head{display:grid;gap:16px}.stock-code{margin:0;color:#fbbf24;font-size:11px;font-weight:950;text-transform:uppercase}.summary-head h2,.reservation-panel h2{margin:5px 0 0;font:900 clamp(29px,7vw,45px)/1.08 Georgia,serif}.summary-head h2 span{color:#94a3b8}.summary-meta{margin:8px 0 0;color:#cbd5e1}.price-pair{display:flex;flex-wrap:wrap;gap:12px}.price-block span,.reservation-price small{color:#94a3b8;font-size:10px;font-weight:900;text-transform:uppercase}.price-block strong,.reservation-price strong{display:block;margin-top:4px;color:#93c5fd;font-size:29px}.price-block.hourly strong{color:#fcd34d}.facts{display:grid;grid-template-columns:1fr 1fr;gap:9px;margin:18px 0 0}.facts>div{border:1px solid #27344f;border-radius:13px;background:#050b18;padding:12px}.facts dt{color:#94a3b8;font-size:10px;font-weight:900;text-transform:uppercase}.facts dd{margin:6px 0 0;font-size:13px;font-weight:900}.available{color:#86efac}.unavailable{color:#fda4af}.plan-summary{display:flex;gap:9px;margin-top:12px;border:1px solid #233453;border-radius:13px;background:#071020;padding:12px}.plan-summary mat-icon{color:#60a5fa}.plan-summary strong,.plan-summary span{display:block}.plan-summary span{margin-top:3px;color:#94a3b8;font-size:11px}.all-details-panel{padding:0;overflow:hidden}.all-details-toggle{display:flex;width:100%;min-height:76px;align-items:center;justify-content:space-between;border:0;background:transparent;padding:16px 18px;color:#fff;text-align:left}.all-details-toggle span{display:grid;gap:3px}.all-details-toggle small{color:#60a5fa;font-size:9px;font-weight:950;letter-spacing:.12em}.all-details-toggle strong{font-size:18px}.all-details-content{border-top:1px solid #1e293b;padding:0 18px 18px}.details-group{padding-top:16px}.details-group+.details-group{margin-top:16px;border-top:1px solid #1e293b}.details-group h3{margin:0 0 10px}.detail-list{margin:0}.detail-list>div{display:flex;justify-content:space-between;gap:18px;border-top:1px solid #1e293b;padding:11px 0}.detail-list>div:first-child{border-top:0}.detail-list dt{color:#94a3b8}.detail-list dd{margin:0;text-align:right;font-weight:850}.feature-list{list-style:none;margin:0;padding:0;display:grid;gap:8px}.feature-list li{display:flex;gap:8px;border:1px solid #1e293b;border-radius:12px;background:#050b18;padding:11px}.feature-list mat-icon{color:#34d399}.tech-grid{display:grid;gap:8px;margin:0}.tech-grid>div{border:1px solid #1e293b;border-radius:12px;background:#050b18;padding:11px}.tech-grid dt{color:#94a3b8;font-size:10px;font-weight:900;text-transform:uppercase}.tech-grid dd{margin:4px 0 0;font-weight:900}.description{white-space:pre-line;color:#cbd5e1;line-height:1.75}.reservation-panel>p{margin:0;color:#93c5fd;font-size:10px;font-weight:950;text-transform:uppercase}.reservation-price{margin-top:15px;border-radius:13px;background:#050b18;padding:13px}.campaign-price-note{display:block;margin-top:5px;color:#fbbf24;font-size:10px;font-weight:900}.unavailable-note{border:1px solid #7f1d1d;border-radius:12px;background:#2b0d13;padding:11px;color:#fecaca}.availability-note{display:flex;align-items:flex-start;gap:7px;margin:13px 0 0;color:#94a3b8;font-size:11px;line-height:1.55}.availability-note mat-icon{color:#34d399}.fixed-actions{position:fixed;z-index:90;left:0;right:0;bottom:0;display:grid;grid-template-columns:.72fr 1fr 1.22fr;gap:8px;border-top:1px solid #334155;background:rgba(5,9,20,.98);padding:9px 10px calc(9px + env(safe-area-inset-bottom));backdrop-filter:blur(16px)}.fixed-actions a,.fixed-actions button{display:flex;min-width:0;min-height:56px;align-items:center;justify-content:center;gap:6px;border:0;border-radius:13px;color:#fff;text-decoration:none;font-weight:950;font-size:13px}.fixed-actions .phone{background:#0f766e}.phone-symbol{font-size:22px;line-height:1}.fixed-actions .whatsapp{background:#059669}.fixed-actions .reserve{background:#2563eb}.fixed-actions button:disabled{opacity:.45}.state-panel{min-height:68vh;display:grid;place-content:center;gap:10px;text-align:center;padding:24px}.state-panel button{min-height:46px;border:0;border-radius:10px;background:#2563eb;color:#fff;padding:0 18px;font-weight:900}.spinner{width:40px;height:40px;margin:auto;border:4px solid #334155;border-top-color:#60a5fa;border-radius:50%;animation:spin .8s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}@media(min-width:700px){.gallery-frame{aspect-ratio:16/9}.campaign-strip{grid-template-columns:1fr auto;align-items:start}.campaign-value{justify-content:flex-end}.summary-head{grid-template-columns:1fr auto;align-items:end}.price-pair{text-align:right;justify-content:flex-end}.tech-grid,.feature-list{grid-template-columns:1fr 1fr}.facts{grid-template-columns:repeat(3,1fr)}}@media(min-width:1024px){.gallery-frame{aspect-ratio:21/9}.detail-layout{grid-template-columns:minmax(0,1fr) 350px;align-items:start}.reservation-panel{position:sticky;top:90px}.fixed-actions{left:50%;right:auto;width:min(760px,calc(100% - 28px));transform:translateX(-50%);border:1px solid #334155;border-bottom:0;border-radius:16px 16px 0 0}}@media(max-width:410px){.fixed-actions{grid-template-columns:64px 1fr 1.15fr}.fixed-actions .phone span:last-child{display:none}.fixed-actions a,.fixed-actions button{font-size:12px}}@media(prefers-reduced-motion:reduce){*{scroll-behavior:auto!important}}
  `],
})
export class CarDetailComponent implements OnInit {
  ...
  async reload(): Promise<void> {
    ...
    try {
      ...
      this.seo.updateSeoTags({title:`${car.brand||"Araç"} ${car.model||""} Kiralama | ${config.companyName}`,description:`${car.brand||"Araç"} ${car.model||""} günlük/saatlik fiyat, donanım ve rezervasyon bilgileri.`,image:car.image||config.seoOgImage});
    } catch(error){this.vehicle.set(null);this.loadError.set(error instanceof Error?error.message:"Araç verisi alınamadı.");}
  ...
  campaignPriceSuffix(offer:CampaignRecord):string{return String(offer.metadata?.["priceSuffix"]||offer.metadata?.["priceLabel"]||"kampanya paketi");}
