    ...
    <main class="detail-page">
      @if (vehicle(); as car) {
        ...
        <section class="gallery" [attr.aria-label]="car.brand + ' ' + car.model + ' araç medyası'">
          ...
          } @else {<div class="gallery-empty" role="status"><mat-icon aria-hidden="true">directions_car</mat-icon><strong>Araç görseli yüklenemedi</strong><span>Medya kaydı canlı katalogdan doğrulanıyor.</span></div>}
        </section>
        ...
        <div class="detail-layout">
          <div class="detail-main">
            ...
            <section class="panel all-details-panel" aria-labelledby="all-details-title">
              <button type="button" class="all-details-toggle" (click)="detailsOpen.update(v => !v)" [attr.aria-expanded]="detailsOpen()" aria-controls="rental-all-details"><span><strong id="all-details-title">Tüm Özellikler ve Açıklama</strong><small>Araç bilgileri, donanım, açıklama ve teknik özellikler</small></span><mat-icon aria-hidden="true">{{ detailsOpen() ? 'expand_less' : 'expand_more' }}</mat-icon></button>
              @if (detailsOpen()) {
          ...
          <aside class="reservation-panel" aria-labelledby="reservation-title"><p>Rezervasyon</p><h2 id="reservation-title">Bu aracı rezerve edin</h2><span>Tarih, nereden alınacağı, iade noktası, şoför tercihi ve ek hizmetler sonraki adımlarda ayrı ayrı seçilir.</span><div class="reservation-price"><small>Günlük başlangıç fiyatı</small><strong>{{ car.price | turkishCurrency }}</strong></div><button type="button" class="primary-action" (click)="reserve(car)" [disabled]="car.isAvailable === false">Rezervasyon Oluştur</button><button type="button" class="whatsapp-action" (click)="whatsapp()">WhatsApp ile Sor</button></aside>
        </div>
...
export class CarDetailComponent implements OnInit {
  ...
  reserve(car: Car): void { if (car.isAvailable === false) return; const days = this.rentalDays(this.presetStartDate, this.presetEndDate); this.carService.setBookingRequest({ type: "RENTAL", item: car, itemName: `${car.brand || ""} ${car.model || ""}`.trim(), image: car.image || car.images?.[0], basePrice: Number(car.price || 0), totalPrice: days > 0 ? days * Number(car.price || 0) : Number(car.price || 0), startDate: days > 0 ? this.presetStartDate : undefined, endDate: days > 0 ? this.presetEndDate : undefined, days: days > 0 ? days : undefined, rentalDuration: "daily", withDriver: car.driverOption === "WITH_DRIVER" }); void this.router.navigate(["/contact"]); }
  ...
  async share(car: Car): Promise<void> { const payload = { title: `${car.brand || ""} ${car.model || ""} | Alperler Auto`.trim(), text: "Bu kiralık aracı inceleyin.", url: window.location.href }; try { if (navigator.share) await navigator.share(payload); else await navigator.clipboard?.writeText(window.location.href); } catch { /* cancelled */ } }
