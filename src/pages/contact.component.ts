...
@Component({
    ...
    <main class="min-h-screen bg-slate-950 pb-20 text-slate-200">
      <header class="sticky top-0 z-40 border-b border-slate-800 bg-slate-900/95 shadow-lg backdrop-blur">
        <div class="mx-auto flex min-h-16 max-w-7xl items-center gap-3 px-4">
          <button type="button" (click)="goBack()" data-analytics-key="contact-back" class="flex h-11 w-11 items-center justify-center rounded-xl text-slate-300 hover:bg-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E15A62]" aria-label="Geri dön">
      ...
      <div class="mx-auto grid max-w-6xl gap-6 px-4 py-8 lg:grid-cols-[0.82fr_1.18fr] lg:py-12">
        <section class="space-y-5">
          <div>
            <span class="text-xs font-black uppercase tracking-[.18em] text-[#E15A62]">Bize Ulaşın</span>
        ...
        <section class="rounded-3xl bg-white p-5 text-slate-900 shadow-2xl sm:p-8">
          @if (sent()) {
            <div role="status" aria-live="polite" class="flex min-h-[28rem] flex-col items-center justify-center text-center">
              ...
              <button type="button" (click)="reset()" data-analytics-key="contact-new-message" class="mt-6 min-h-12 rounded-xl bg-slate-900 px-6 font-black text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E15A62]">Yeni Mesaj Gönder</button>
            </div>
          } @else {
            ...
            <form (ngSubmit)="submit()" data-analytics-form="contact" class="space-y-5" novalidate>
              ...
              <button type="submit" data-analytics-key="contact-submit" [disabled]="submitting() || !isValid()" class="flex min-h-14 w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-5 font-black text-white transition hover:bg-[#9E1B24] disabled:cursor-not-allowed disabled:opacity-45 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E15A62]">
  ...
  `,
  styles: [`
    .contact-card{display:flex;min-height:72px;align-items:center;gap:14px;border:1px solid rgb(51 65 85);border-radius:16px;background:rgb(15 23 42);padding:14px 16px;color:rgb(226 232 240);text-decoration:none}.contact-card mat-icon{color:#E15A62}.contact-card span{display:flex;min-width:0;flex-direction:column}.contact-card strong{font-size:.9rem}.contact-card small{margin-top:2px;color:rgb(148 163 184);overflow-wrap:anywhere}.field{display:flex;flex-direction:column;gap:7px}.field>span{font-size:.72rem;font-weight:900;text-transform:uppercase;letter-spacing:.08em;color:rgb(71 85 105)}.field input,.field textarea{width:100%;border:1px solid rgb(203 213 225);border-radius:12px;background:rgb(248 250 252);padding:12px 14px;color:rgb(15 23 42);outline:none}.field input{min-height:48px}.field input:focus,.field textarea:focus{border-color:#E15A62;box-shadow:0 0 0 2px rgba(225,90,98,.18)}
  `],
})
