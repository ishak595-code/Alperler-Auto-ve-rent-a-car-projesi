      ...
      <section class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-12" aria-labelledby="faq-title">
        ...
        @if (loading()) {
          ...
          <div class="rounded-xl border border-red-900/60 bg-slate-900 p-6 text-center" role="alert">
            ...
            <button type="button" (click)="reload()" class="mt-4 rounded-lg bg-[#9E1B24] px-4 py-2 font-bold text-white">Tekrar dene</button>
          </div>
          ...
          <div class="space-y-4">
            @for (faq of faqs(); track faq.id) {
              <article class="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-sm transition-all duration-300 hover:shadow-md group hover:border-[#E15A62]">
                <h3>
                  <button
                    type="button"
                    (click)="toggleFaq(faq.id)"
                    class="w-full flex justify-between items-center p-6 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E15A62] bg-slate-900 hover:bg-slate-800 transition-colors"
                    [attr.aria-expanded]="isOpen(faq.id)"
                    [attr.aria-controls]="'faq-answer-' + faq.id"
                  >
                    <span class="font-bold text-slate-100 text-lg group-hover:text-[#E15A62] transition-colors">{{ faq.question }}</span>
