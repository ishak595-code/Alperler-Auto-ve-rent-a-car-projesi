    ...
    <main class="min-h-screen bg-slate-950 text-white">
      <header class="sticky top-0 z-30 border-b border-white/10 bg-slate-950/95 px-4 py-4 backdrop-blur-xl md:px-8">
        <div class="mx-auto flex max-w-6xl items-center justify-between gap-4">
          ...
          <div class="hidden rounded-full border border-emerald-400/20 bg-emerald-400/10 px-4 py-2 text-xs font-black text-emerald-300 sm:block">{{ t().listYourCar.secureBadge }}</div>
        </div>
      </header>
      ...
      <section class="border-b border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,.16),transparent_35%),radial-gradient(circle_at_top_left,rgba(59,130,246,.12),transparent_35%)] px-4 py-10 md:px-8 md:py-16">
        <div class="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
          <div>
            <span class="inline-flex rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1 text-[11px] font-black uppercase tracking-[0.16em] text-emerald-300">{{ t().listYourCar.heroEyebrow }}</span>
            <h2 class="mt-5 max-w-3xl text-4xl font-black leading-tight tracking-tight md:text-6xl">{{ t().listYourCar.heroTitle }}</h2>
            <p class="mt-5 max-w-2xl text-sm font-medium leading-7 text-slate-300 md:text-base">{{ t().listYourCar.heroCopy }}</p>
          ...
          <aside class="rounded-[2rem] border border-amber-300/20 bg-amber-300/10 p-6 shadow-2xl shadow-black/20">
            <p class="text-xs font-black uppercase tracking-[0.16em] text-amber-300">{{ t().listYourCar.noticeEyebrow }}</p>
            <h3 class="mt-2 text-xl font-black">{{ t().listYourCar.noticeTitle }}</h3>
            <p class="mt-3 text-sm leading-6 text-amber-50/80">{{ t().listYourCar.noticeCopy }}</p>
          </aside>
        </div>
      </section>
      ...
      <section class="mx-auto max-w-6xl px-4 py-8 md:px-8 md:py-12">
        @if (success()) {
          <div role="status" aria-live="polite" class="mx-auto max-w-3xl rounded-[2rem] border border-emerald-300/30 bg-emerald-300/10 p-7 text-center md:p-10">
            ...
            <p class="mt-5 text-xs font-black uppercase tracking-[0.18em] text-emerald-300">{{ t().listYourCar.successEyebrow }}</p>
            <h2 class="mt-2 text-3xl font-black">{{ t().listYourCar.successTitle }}</h2>
            <p class="mt-3 text-sm leading-6 text-slate-300">{{ t().listYourCar.successCopy }}</p>
            ...
            <button type="button" (click)="reset()" class="mt-6 min-h-12 rounded-2xl bg-white px-6 text-sm font-black text-slate-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400">{{ t().listYourCar.newApplication }}</button>
          </div>
        } @else {
          @if (geo.error()) {
            <div role="alert" class="mb-5 rounded-2xl border border-amber-300/30 bg-amber-300/10 p-4 text-sm font-bold text-amber-100">{{ t().listYourCar.geoError }}</div>
          }
          @if (errorMessage()) {
          ...
          <div class="grid gap-6 lg:grid-cols-[1fr_340px] lg:items-start">
            <div class="space-y-6">
              ...
              <section class="panel">
                <div class="section-head"><span>03</span><div><h2>{{ t().listYourCar.sectionLocationTitle }}</h2><p>{{ t().listYourCar.sectionLocationHint }}</p></div></div>
                <div class="form-grid mt-5">
              ...
              <section class="panel">
                <div class="section-head"><span>06</span><div><h2>{{ t().listYourCar.sectionMediaTitle }}</h2><p>{{ t().listYourCar.sectionMediaHint }}</p></div></div>
                <label class="mt-5 flex min-h-28 cursor-pointer flex-col items-center justify-center rounded-2xl border-2 border-dashed border-white/15 bg-white/[.03] p-5 text-center transition hover:border-emerald-400/50 hover:bg-emerald-400/5">
                  <strong class="text-sm">{{ t().listYourCar.chooseFiles }}</strong><span class="mt-1 text-xs text-slate-400">{{ t().listYourCar.filesHint }}</span>
                ...
                @if (submitting() && files().length) {
                  <div class="mt-4" aria-live="polite"><div class="mb-2 flex justify-between text-xs font-black"><span>{{ t().listYourCar.uploading }}</span><span>{{ overallProgress() }}%</span></div><div class="h-2 overflow-hidden rounded-full bg-white/10"><div class="h-full rounded-full bg-emerald-400 transition-all" [style.width.%]="overallProgress()"></div></div></div>
                }
              </section>

              <section class="panel">
                <div class="section-head"><span>07</span><div><h2>{{ t().listYourCar.sectionContactTitle }}</h2><p>{{ t().listYourCar.sectionContactHint }}</p></div></div>
                <div class="form-grid mt-5">
              ...
              <section class="panel">
                <div class="space-y-4">
                  <label class="consent"><input type="checkbox" [ngModel]="termsAccepted()" (ngModelChange)="termsAccepted.set($event)" /><span>{{ t().listYourCar.terms }}</span></label>
                  <label class="consent"><input type="checkbox" [ngModel]="privacyAccepted()" (ngModelChange)="privacyAccepted.set($event)" /><span>{{ t().listYourCar.privacy }}</span></label>
                </div>
                <button type="button" (click)="submit()" [disabled]="!formValid() || submitting()" class="mt-6 min-h-14 w-full rounded-2xl bg-emerald-400 px-5 text-sm font-black text-slate-950 shadow-xl shadow-emerald-950/20 transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400">{{ submitting() ? t().listYourCar.submitting : t().listYourCar.submit }}</button>
                @if (!formValid()) {<p class="mt-3 text-center text-xs font-semibold text-slate-400">{{ t().listYourCar.formIncomplete }}</p>}
              </section>
            </div>
...
export class ListYourCarV172Component implements OnInit {
  ...
  private readonly router=inject(Router);private readonly ui=inject(UiService);readonly t=this.ui.translations;
  ...
  onFilesSelected(event:Event){
    ...
    if(valid.length!==selected.length||total>200*1024*1024){this.errorMessage.set(this.t().listYourCar.fileReject);this.files.set(total>200*1024*1024?[]:valid);return;}
