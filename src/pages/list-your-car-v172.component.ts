    ...
    <main class="min-h-screen bg-slate-950 text-white">
      <header class="sticky top-0 z-30 border-b border-white/10 bg-slate-950/95 px-4 py-4 backdrop-blur-xl md:px-8">
        <div class="mx-auto flex max-w-6xl items-center justify-between gap-4">
          ...
          <div class="min-w-0 flex-1">
            <p class="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-400">Alperler Auto V172</p>
      ...
      <section class="border-b border-white/10 bg-[radial-gradient(circle_at_top_right,rgba(16,185,129,.16),transparent_35%),radial-gradient(circle_at_top_left,rgba(59,130,246,.12),transparent_35%)] px-4 py-10 md:px-8 md:py-16">
        <div class="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.15fr_.85fr] lg:items-center">
          <div>
            ...
            <p class="mt-5 max-w-2xl text-sm font-medium leading-7 text-slate-300 md:text-base">Araç bilgilerini, konumunu ve varsa ekspertiz belgelerini güvenli şekilde gönder. Ekibimiz piyasa bandını, kondisyon sınıfını ve uygun ticari modeli ayrı bir resmi değerleme olarak oluşturur.</p>
      ...
      <section class="mx-auto max-w-6xl px-4 py-8 md:px-8 md:py-12">
        @if (success()) {
          ...
          @if (geo.error()) {
            <div role="alert" class="mb-5 rounded-2xl border border-amber-300/30 bg-amber-300/10 p-4 text-sm font-bold text-amber-100">İl ve ilçe dizini şu anda yüklenemedi. Konum alanları hazır olmadan başvuru gönderilemez.</div>
          }
          @if (errorMessage()) {
          ...
          <div class="grid gap-6 lg:grid-cols-[1fr_340px] lg:items-start">
            <div class="space-y-6">
              ...
              <section class="panel">
                <div class="section-head"><span>03</span><div><h2>Konum ve tercih edilen şube</h2><p>Türkiye'nin doğrulanmış il ve ilçe dizini kullanılır.</p></div></div>
                <div class="form-grid mt-5">
                  <label><span>İl *</span><select [ngModel]="provinceCode()" (ngModelChange)="onProvinceChange($event)" [disabled]="geo.loading()"><option value="">{{ geo.loading() ? 'İller yükleniyor...' : 'İl seç' }}</option>@for(province of geo.provinces();track province.code){<option [value]="province.code">{{ province.name }}</option>}</select></label>
              ...
              <section class="panel">
                <div class="section-head"><span>05</span><div><h2>Özel araç kimliği</h2><p>Plaka, VIN ve ruhsat referansı public başvuru kaydına yazılmaz. Ayrı private güvenlik alanında tutulur.</p></div></div>
              ...
              <section class="panel">
                ...
                @if (submitting() && files().length) {
                  <div class="mt-4" aria-live="polite"><div class="mb-2 flex justify-between text-xs font-black"><span>Güvenli yükleme</span><span>{{ overallProgress() }}%</span></div><div class="h-2 overflow-hidden rounded-full bg-white/10"><div class="h-full rounded-full bg-emerald-400 transition-all" [style.width.%]="overallProgress()"></div></div></div>
                }
              </section>
              ...
              <section class="panel">
                ...
                <button type="button" (click)="submit()" [disabled]="!formValid() || submitting()" class="mt-6 min-h-14 w-full rounded-2xl bg-emerald-400 px-5 text-sm font-black text-slate-950 shadow-xl shadow-emerald-950/20 transition hover:bg-emerald-300 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400">{{ submitting() ? 'Başvuru güvenli şekilde gönderiliyor...' : 'Profesyonel Değerleme Başvurusu Gönder' }}</button>
                @if (!formValid()) {<p class="mt-3 text-center text-xs font-semibold text-slate-400">Yıldızlı alanları, gerçek il-ilçe bilgisini ve onay kutularını tamamla.</p>}
              </section>
            </div>
            ...
            <aside class="space-y-4 lg:sticky lg:top-24">
              ...
              <section class="rounded-[1.75rem] border border-blue-300/20 bg-blue-300/10 p-5"><strong class="text-sm text-blue-200">Süreç nasıl ilerler?</strong><ol class="mt-4 space-y-3 text-xs leading-5 text-blue-50/80"><li><b>1.</b> Başvuru ve belgeler güvenli alınır.</li><li><b>2.</b> Uzman ekip piyasa bandı ve A-E kondisyon sınıfı oluşturur.</li><li><b>3.</b> Gerekirse ekspertiz veya görüşme randevusu atanır.</li><li><b>4.</b> FINAL değerleme olmadan resmi teklif statüsüne geçilemez.</li></ol></section>
            </aside>
          </div>
        }
      </section>
