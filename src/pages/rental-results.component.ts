    ...
    <main class="min-h-screen bg-slate-950 pb-28 text-slate-300 md:pb-10">
      <header class="sticky top-0 z-40 border-b border-slate-800 bg-slate-900/95 shadow-lg backdrop-blur-xl">
        <div class="mx-auto flex min-h-16 max-w-7xl items-center gap-2 px-2 py-2 sm:px-4">
          ...
          <label for="rental-search" class="relative min-w-0 flex-1"><span class="sr-only">Kiralık araçlarda ilan no, marka veya model ara</span><mat-icon aria-hidden="true" class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">search</mat-icon><input id="rental-search" type="search" inputmode="search" autocomplete="off" [ngModel]="searchQuery()" (ngModelChange)="searchQuery.set($event)" placeholder="İlan no, marka veya model ara" class="min-h-12 w-full rounded-xl border border-slate-700 bg-slate-800 pl-11 pr-4 text-sm font-semibold text-white outline-none placeholder:text-slate-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40" /></label>
        </div>
      </header>
      ...
      <section class="border-b border-slate-800 bg-[#020617] px-4 py-6 sm:py-8">
        <div class="mx-auto max-w-7xl">
          ...
          @if (hasPlannerContext()) {
            ...
            <p class="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Araçları karşılaştırın veya filtreleri kullanarak ihtiyacınıza uygun seçenekleri daraltın.</p>
          }
        </div>
      </section>
        ...
        <section class="sticky top-16 z-30 border-b border-slate-200 bg-white/95 px-4 py-3 text-slate-900 shadow-sm backdrop-blur" aria-label="Kiralık araç sonuç araçları">
          <div class="mx-auto flex max-w-7xl items-center justify-between gap-3">
            ...
            <div class="flex shrink-0 items-center gap-2">
              @if (activeFilterCount()) { <button type="button" (click)="clearFilters()" class="min-h-11 rounded-xl px-3 text-xs font-black text-slate-600 hover:bg-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">Temizle</button> }
              <button type="button" (click)="filterOpen.set(true)" class="inline-flex min-h-11 items-center gap-2 rounded-xl bg-slate-950 px-4 text-xs font-black text-white shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" [attr.aria-expanded]="filterOpen()" aria-controls="rental-filter-sheet"><mat-icon aria-hidden="true">tune</mat-icon> Filtrele @if (activeFilterCount()) { <span class="count-badge">{{ activeFilterCount() }}</span> }</button>
            </div>
          </div>
        </section>
      }
...
export class RentalResultsComponent {
  ...
  readonly filteredVehicles = computed(() => {
    let vehicles: Car[] = this.showFavoritesOnly() ? (this.allVehicles().filter((vehicle) => vehicle.category === "RENTAL" && this.carService.isFavorite(vehicle.id)) as Car[]) : [...this.allCars()];
    ...
    result = [...result];
    if (this.sortBy() === "priceAsc") result.sort((a,b)=>Number(a.price||0)-Number(b.price||0));
    else if (this.sortBy() === "priceDesc") result.sort((a,b)=>Number(b.price||0)-Number(a.price||0));
    else if (this.sortBy() === "yearDesc") result.sort((a,b)=>(b.year||0)-(a.year||0));
    else result.sort((a,b)=>(Number(Boolean(b.isFeatured))-Number(Boolean(a.isFeatured))) || Number(b.displayPriority||0)-Number(a.displayPriority||0) || Number(a.price||0)-Number(b.price||0));
    return result;
  });
  ...
  constructor() {
    ...
    const explicitDriverMode = params.get("driverMode");
    if (explicitDriverMode === "with" || explicitDriverMode === "without") this.driverMode.set(explicitDriverMode);
  ...
  detailQueryParams(): Params {
    return {
      ...
      ...(this.pickupBranchId() ? { pickup: this.pickupBranchId() } : {}),
      ...(this.pickupLocation() ? { pickupLocation: this.pickupLocation() } : {}),
      ...(this.driverMode() !== "any" ? { driverMode: this.driverMode() } : {}),
      ...(this.occasion() ? { occasion: this.occasion() } : {}),
      ...(this.campaignId() ? { campaign: this.campaignId() } : {}),
    };
  }
  ...
  private isAvailableForSelectedPeriod(car: Car): boolean {
    ...
    const start=this.parseLocalDate(this.startDate()); const end=this.parseLocalDate(this.endDate());
    if(!start||!end) return car.isAvailable !== false;
    return !(car.bookedDates || []).some((booking)=>{ const bookedStart=new Date(booking.start); const bookedEnd=new Date(booking.end); if(Number.isNaN(bookedStart.getTime())||Number.isNaN(bookedEnd.getTime())) return false; return start < bookedEnd && end > bookedStart; });
  }
