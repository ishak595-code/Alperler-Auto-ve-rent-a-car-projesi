    ...
    <main class="sale-page">
      @if (car(); as item) {
        <header class="sale-header">
          <div class="header-left"><button type="button" class="header-button" (click)="goBack()" aria-label="Satılık araçlardan geri dön"><mat-icon aria-hidden="true">arrow_back</mat-icon></button><div><span>SATILIK ARAÇ · CANLI KATALOG</span><h1>{{ item.brand }} {{ item.model }}</h1></div></div>
        ...
        <section class="listing-head" aria-labelledby="listing-title">
          <div><p class="listing-no">İlan No {{ item.cloudStockCode || item.id }}</p><h2 id="listing-title">{{ item.year || '' }} {{ item.brand }} @if(item.series){<span>{{item.series}}</span>} {{ item.model }}</h2><p class="summary">{{ summaryMeta(item) }}</p></div>
          <strong class="listing-price">{{ item.price | turkishCurrency }}</strong>
          <dl class="core-facts" aria-label="Temel ilan bilgileri">
            <div><dt>Model Yılı</dt><dd>{{ item.year || 'Belirtilmedi' }}</dd></div>
            <div><dt>Kilometre</dt><dd>{{ item.km != null ? ((item.km | number:'1.0-0') + ' km') : 'Belirtilmedi' }}</dd></div>
            <div><dt>Yakıt</dt><dd>{{ display(item.fuel) }}</dd></div>
            <div><dt>Vites</dt><dd>{{ display(item.transmission) }}</dd></div>
          </dl>
          @if ((item.viewers || 0) > 0 || (item.favCount || 0) > 0) {<div class="social-proof" aria-label="İlan etkileşim bilgileri">@if((item.viewers || 0) > 0){<span><mat-icon aria-hidden="true">visibility</mat-icon>{{item.viewers}} görüntülenme</span>}@if((item.favCount || 0) > 0){<span><mat-icon aria-hidden="true">favorite</mat-icon>{{item.favCount}} favori</span>}</div>}
        </section>
          ...
          @if (activeTab() === 'info') {
            <div class="info-panel">
              <dl class="listing-table">
                <div><dt>İlan No</dt><dd class="important">{{ item.cloudStockCode || item.id }}</dd></div><div><dt>İlan Tarihi</dt><dd>{{ listingDate(item) }}</dd></div><div><dt>Marka</dt><dd>{{ display(item.brand) }}</dd></div><div><dt>Seri / Model</dt><dd>{{ [item.series,item.model].filter(Boolean).join(' ') || 'Belirtilmedi' }}</dd></div><div><dt>Yıl</dt><dd class="important">{{ display(item.year) }}</dd></div><div><dt>Kilometre</dt><dd class="important">{{ item.km != null ? ((item.km | number:'1.0-0') + ' km') : 'Belirtilmedi' }}</dd></div><div><dt>Yakıt</dt><dd>{{ display(item.fuel) }}</dd></div><div><dt>Vites</dt><dd>{{ display(item.transmission) }}</dd></div><div><dt>Kasa Tipi</dt><dd>{{ display(item.type) }}</dd></div><div><dt>Renk</dt><dd>{{ display(item.color) }}</dd></div><div><dt>Koltuk</dt><dd>{{ item.seats ? item.seats + ' kişi' : 'Belirtilmedi' }}</dd></div><div><dt>Kapı</dt><dd>{{ display(item.doors) }}</dd></div><div><dt>Çekiş</dt><dd>{{ display(item.drivetrain) }}</dd></div><div><dt>Motor Gücü</dt><dd>{{ display(item.enginePower) }}</dd></div><div><dt>Motor Hacmi</dt><dd>{{ display(item.engineVolume) }}</dd></div><div><dt>Garanti</dt><dd>{{ display(item.warranty || (item.hasWarranty ? 'Var' : 'Belirtilmedi')) }}</dd></div><div><dt>Durum</dt><dd>{{ display(item.availability || 'Satışta') }}</dd></div>
              </dl>

              @if (technicalRows().length) {
                <section class="expand-section"><button type="button" class="expand-button" (click)="techOpen.update(v => !v)" [attr.aria-expanded]="techOpen()" aria-controls="sale-tech-specs"><span><mat-icon aria-hidden="true">settings_suggest</mat-icon>{{ techOpen() ? 'Teknik Verileri Gizle' : 'Teknik Verileri Gör' }}</span><mat-icon aria-hidden="true">{{ techOpen() ? 'expand_less' : 'expand_more' }}</mat-icon></button>@if (techOpen()) {<dl id="sale-tech-specs" class="spec-grid">@for (row of technicalRows(); track row.label) {<div><dt>{{row.label}}</dt><dd>{{row.value}}</dd></div>}</dl>}</section>
              }

              @if (features().length) {
                <section class="expand-section"><button type="button" class="expand-button blue" (click)="featuresOpen.update(v => !v)" [attr.aria-expanded]="featuresOpen()" aria-controls="sale-features"><span><mat-icon aria-hidden="true">checklist</mat-icon>{{ featuresOpen() ? 'Donanımı Gizle' : 'Donanım ve Özellikleri Gör' }}</span><mat-icon aria-hidden="true">{{ featuresOpen() ? 'expand_less' : 'expand_more' }}</mat-icon></button>@if (featuresOpen()) {<ul id="sale-features" class="feature-grid">@for (feature of features(); track feature) {<li><mat-icon aria-hidden="true">check_circle</mat-icon>{{feature}}</li>}</ul>}</section>
              }
              ...
              <section class="expertise" aria-labelledby="expertise-title">
                ...
                <div class="truth-grid"><div><span>Hasar Durumu</span><strong>{{ item.damageStatus || (item.isDamageFree ? 'Hatasız & Boyasız' : 'Belirtilmedi') }}</strong></div><div><span>Tramer Durumu</span><strong>{{ tramerStatusLabel(item) }}</strong></div>@if(item.tramerAmount != null){<div><span>Tramer Tutarı</span><strong>{{item.tramerAmount|turkishCurrency}}</strong></div>}@if(item.tramerVerifiedAt){<div><span>Doğrulama Tarihi</span><strong>{{formatDate(item.tramerVerifiedAt)}}</strong></div>}</div>
...
export class SaleCarDetailComponent implements OnInit {
  ...
  async reload(): Promise<void> {
    ...
    try {
      ...
      this.seo.updateSeoTags({ title: `${item.brand || "Araç"} ${item.model || ""} Satılık | ${config.companyName}`, description: `${item.year || ""} ${item.brand || ""} ${item.model || ""} satılık araç ilanı. ${item.km != null ? Number(item.km).toLocaleString('tr-TR') + ' km.' : ''} Fiyat, ekspertiz, açıklama ve konum bilgileri.`, image: item.image || config.seoOgImage });
