...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, MatIconModule, AccessibleNativeDateComponent, TurkishCurrencyPipe, DetailMediaLightboxComponent],
  template: `
    <main class="page">
      @if(tour();as item){
        ...
        <section class="gallery" [attr.aria-label]="item.title + ' fotoğraf ve video galerisi'" (touchstart)="touchStart($event)" (touchend)="touchEnd($event)">
          @if(activeMedia();as media){
            <div class="frame">
              @if(media.kind==='IMAGE'){<button type="button" class="media-open" (click)="openLightbox()" [attr.aria-label]="media.title + ' tam ekran aç'"><img [src]="media.url" [alt]="media.title" (error)="mediaFailed(media.url)" loading="eager" decoding="async" /></button>}
              @else{<video [src]="media.url" [poster]="media.posterUrl||item.image" controls playsinline preload="metadata" [attr.aria-label]="media.title" (error)="mediaFailed(media.url)"></video><button type="button" class="expand" (click)="openLightbox()" aria-label="Tur videosunu tam ekran galeride aç"><mat-icon aria-hidden="true">fullscreen</mat-icon></button>}
        ...
        <app-detail-media-lightbox [open]="lightboxOpen()" [items]="mediaItems()" [index]="currentMedia()" [title]="item.title+' fotoğraf ve video galerisi'" (closed)="lightboxOpen.set(false)" (indexChange)="currentMedia.set($event)" />
...
export class TourDetailV170Component implements OnInit {
  ...
  async reload(): Promise<void>{this.loading.set(true);this.loadError.set("");try{const id=this.route.snapshot.paramMap.get("id")||"";const item=await this.data.load(id);this.tour.set(item);this.currentMedia.set(0);this.failedMedia.set([]);this.lightboxOpen.set(false);}catch(error){this.tour.set(null);this.loadError.set(error instanceof Error?error.message:"Tur yüklenemedi.");}finally{this.loading.set(false);}}
  ...
  touchEnd(event:TouchEvent):void{const end=event.changedTouches[0]?.clientX||0,delta=end-this.touchX;if(Math.abs(delta)<45)return;delta<0?this.next():this.previous();}
