      ...
      @if (tour(); as item) {
        ...
        <section class="gallery" [attr.aria-label]="item.title + ' tur görselleri'">
          @if (activeImage(); as image) {
            ...
            <div class="gallery-empty" role="status"><mat-icon aria-hidden="true">landscape</mat-icon><strong>Tur görseli henüz eklenmedi</strong></div>
          }
        </section>
        ...
        <div class="detail-layout">
          <div class="detail-main">
            <section class="panel summary-panel" aria-labelledby="tour-summary-title">
              <div class="summary-head">
                ...
                <div class="price"><span>Kişi başı</span><strong>{{ item.price | turkishCurrency }}</strong></div>
              </div>
              <dl class="facts">
                <div><dt>Süre</dt><dd>{{ display(item.duration) }}</dd></div>
                <div><dt>Kapasite</dt><dd>{{ item.capacity ? item.capacity + ' kişi' : 'Belirtilmedi' }}</dd></div>
          ...
          <aside class="reserve-panel" aria-labelledby="tour-reserve-title">
            ...
            <h2 id="tour-reserve-title">Tarihinizi seçin, talebinizi oluşturun</h2>
            <span>Rezervasyon akışında tarih, kişi sayısı ve iletişim bilgileri adım adım alınır. Kesin müsaitlik ekip doğrulamasından sonra oluşur.</span>
            <div class="reserve-price"><small>Kişi başı başlangıç</small><strong>{{ item.price | turkishCurrency }}</strong></div>
            <div class="reserve-note"><mat-icon aria-hidden="true">verified_user</mat-icon><span>Talep göndermek rezervasyonu otomatik kesinleştirmez.</span></div>
          </aside>
        </div>
...
export class TourDetailComponent implements OnInit {
  ...
  async reload(): Promise<void> {
    ...
    try {
      ...
      this.seo.updateSeoTags({
        title: `${item.title || "Tur"} | Alperler Rent A Car`,
        description: `${item.title || "Tur"} için rota, süre, kişi başı fiyat ve rezervasyon bilgileri.`,
  ...
  async submitReservation(): Promise<void> {
    ...
    try {
      const record = await this.bookingService.create({
        ...
        totalPrice: this.totalTourPrice(),
        currency: "TRY",
        personCount: this.personCount(),
        startDate: this.tourDate,
        endDate: this.tourDate,
      ...
      this.reservationError.set("Rezervasyon kaydedilemedi. Lütfen tekrar deneyin.");
    } finally {
      this.submitting.set(false);
    }
  }

  formattedTourDate(): string {
    if (!this.tourDate) return "Seçilmedi";
    const date = new Date(`${this.tourDate}T12:00:00`);
    return Number.isNaN(date.getTime()) ? this.tourDate : new Intl.DateTimeFormat("tr-TR", { day: "2-digit", month: "long", year: "numeric" }).format(date);
  }
  ...
  whatsapp(): void {
    ...
    if (!phone) return;
    const message = `Merhaba, ${item.title || "tur"} hakkında bilgi almak istiyorum. ${window.location.href}`;
    window.open(`https://wa.me/${phone}?text=${encodeURIComponent(message)}`, "_blank", "noopener,noreferrer");
  }

  goBack(): void { if (window.history.length > 1) this.location.back(); else void this.router.navigate(["/tours"]); }

  private itineraryText(value: unknown, index: number): string {
    if (typeof value === "string") return value.trim();
    if (value && typeof value === "object") {
      const row = value as Record<string, unknown>;
      return String(row["title"] || row["name"] || row["description"] || row["label"] || `Program adımı ${index + 1}`).trim();
    }
    return "";
  }
}
