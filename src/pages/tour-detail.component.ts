  ...
  template: `
    @if (tour()) {
      <div class="min-h-screen bg-white animate-fade-in flex flex-col pb-24 lg:pb-0 relative">
        <!-- Header / Back Button -->
        <div
          class="sticky top-0 bg-white/90 backdrop-blur-md border-b border-slate-100 px-4 py-4 flex items-center justify-between shadow-sm z-50"
        >
          <button
            (click)="goBack()"
            class="flex items-center text-slate-900 hover:text-blue-600 font-bold transition-colors text-lg"
          >
        ...
        <div class="flex-grow pb-24">
          <div class="relative h-[40vh] sm:h-[50vh] min-h-[300px] w-full">
            ...
            <div class="absolute bottom-6 left-6 sm:left-12 right-6">
              <span
                class="bg-blue-500 text-slate-900 font-bold text-xs px-3 py-1 rounded-full shadow-lg mb-4 inline-block"
              >
                {{ tour()!.duration }}
              </span>
              <h2
                class="text-3xl sm:text-5xl font-serif font-bold text-white leading-tight"
              >
                {{ tour()!.title }}
              </h2>
            </div>
          ...
          <div class="max-w-5xl mx-auto px-4 sm:px-6 py-12">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
              <div class="lg:col-span-2 space-y-10">
                <section>
                  <h3
                    class="text-2xl font-bold text-slate-900 mb-4 flex items-center"
                  >
                    <mat-icon class="text-blue-500 mr-2">explore</mat-icon>
                    Tur Hakkında Özel Detaylar
                  </h3>
                  <p class="text-slate-600 leading-relaxed text-lg bg-slate-50 p-6 rounded-2xl border-l-4 border-blue-500 shadow-sm">
                    {{ tour()!.description }}
                  </p>
                </section>
                ...
                <section>
                  <h3
                    class="text-2xl font-bold text-slate-900 mb-6 flex items-center"
                  >
                  ...
                  <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @for (highlight of tour()!.highlights; track highlight) {
                      <div
                        class="flex items-start bg-white p-4 rounded-2xl shadow-sm border border-slate-100 hover:border-blue-400 hover:shadow-md hover:-translate-y-1 transition-all duration-300"
                      >
                        <mat-icon class="text-blue-500 mr-3 flex-shrink-0 mt-0.5">check_circle</mat-icon>
                        <span class="text-slate-700 font-bold tracking-wide">{{
                          highlight
                        }}</span>
                      </div>
                    }
              ...
              <div class="lg:col-span-1">
                <div
                  class="bg-white rounded-t-2xl sm:rounded-3xl p-4 sm:p-8 border-t sm:border border-slate-200 fixed bottom-0 left-0 right-0 sm:relative sm:top-24 sm:sticky shadow-[0_-10px_40px_rgba(0,0,0,0.15)] sm:shadow-xl z-40"
                >
                  <div class="mb-3 sm:mb-8 flex sm:block items-center justify-between gap-4">
                    ...
                    <div class="flex sm:hidden flex-1 justify-end items-center gap-3">
                       <a
                        [href]="'https://wa.me/905379594851?text=' + tour()!.title + ' turunuz hakkında bilgi almak istiyorum.'"
                        target="_blank"
                        class="bg-[#25D366] text-white w-12 h-12 rounded-xl hover:bg-[#1ebe57] transition-all shadow-md active:scale-95 flex items-center justify-center shrink-0"
                      ...
                      >
                       ...
                       <button
                        ...
                        class="bg-slate-900 text-white px-2 h-12 rounded-xl font-bold text-sm hover:bg-blue-600 transition-all shadow-md active:scale-95 flex-1 max-w-[180px] text-center flex items-center justify-center uppercase tracking-wide"
                      >
                        Rezervasyon Yap
                      </button>
                    </div>
                  ...
                  <div class="hidden sm:block space-y-4 mb-8">
                    <div
                      class="flex items-center text-slate-700 bg-slate-50 p-4 rounded-xl border border-slate-100"
                    >
                  ...
                  <div class="hidden sm:block space-y-3">
                    <button
                      (click)="openReservationModal()"
                      class="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-lg hover:bg-blue-600 hover:text-white transition-all shadow-lg hover:shadow-blue-500/30 active:scale-95 flex items-center justify-center transform"
                    >
                    ...
                    <a
                      [href]="'https://wa.me/905379594851?text=' + tour()!.title + ' turunuz hakkında bilgi almak istiyorum.'"
                      target="_blank"
                      class="w-full bg-[#25D366] text-white py-4 rounded-2xl font-bold text-lg hover:bg-[#1ebe57] transition-all shadow-lg active:scale-95 flex items-center justify-center"
                      aria-label="WhatsApp'tan Sor"
                    >
            ...
            <!-- Çapraz Satış: Önerilen Kiralık Araçlar -->
            @if (recommendedCars().length > 0) {
              <div class="mt-20 border-t border-slate-100 pt-16">
                <div class="flex justify-between items-end mb-8">
                  <div>
                    <h3 class="text-2xl font-bold text-slate-900">
                      Bu Tura Uygun Araçlar
                    </h3>
                    <p class="text-slate-500 mt-2">
                      Kendi aracınızla özgürce keşfetmek isterseniz size özel
                      lüks kiralık araç listemiz.
                    </p>
                  </div>
                  <button
                    (click)="router.navigate(['/fleet'])"
                    class="hidden sm:flex text-blue-600 font-bold hover:text-blue-800 transition-colors items-center text-sm"
                  >
                    Tüm Araçları Gör
                    <mat-icon class="ml-1 w-4 h-4 text-[16px]"
        ...
        <!-- Full-Screen Reservation Overlay Modal -->
        @if (isReservationModalOpen()) {
           <div class="fixed inset-0 z-[999] bg-white flex flex-col animate-fade-in pb-16 lg:pb-0 overflow-y-auto">
              ...
              <div class="sticky top-0 bg-white/90 backdrop-blur-md border-b border-slate-100 px-4 py-4 flex items-center justify-between shadow-sm z-50">
                 <button (click)="closeReservationModal()" class="flex items-center text-slate-600 hover:text-slate-900 font-bold transition-colors">
                    <mat-icon class="mr-1">close</mat-icon>
                    Kapat
                 </button>
                 <div class="font-bold text-slate-900 truncate px-4 flex-1 text-center font-serif text-xl">
                   {{ reservationStep() === 1 ? 'Tur Rezervasyonu' : 'Güvenli Ödeme Aşaması' }}
                 </div>
              ...
              <div class="flex-1 w-full max-w-3xl mx-auto px-4 py-8">
                 @if (reservationStep() === 1) {
                    ...
                    <div class="bg-white border text-center p-6 rounded-3xl mb-8 flex flex-col items-center shadow-[0_10px_40px_rgba(0,0,0,0.03)] border-slate-100">
                        ...
                        <div class="flex items-center gap-6 bg-slate-50 p-3 rounded-2xl border border-slate-200">
                           <button (click)="decreasePerson()" aria-label="Kişi Sayısını Azalt" type="button" class="w-12 h-12 rounded-xl bg-white shadow-sm flex items-center justify-center text-slate-700 active:scale-95 border border-slate-100">
                              <mat-icon>remove</mat-icon>
                           </button>
                           <span class="font-black text-3xl w-12 text-center text-slate-900">{{ personCount() }}</span>
                           <button (click)="increasePerson()" aria-label="Kişi Sayısını Artır" type="button" class="w-12 h-12 rounded-xl bg-slate-900 shadow-sm flex items-center justify-center text-white active:scale-95 hover:bg-blue-600 transition-colors">
                    ...
                    <form [formGroup]="reservationForm" (ngSubmit)="proceedToPayment()" class="space-y-6">
                       <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                          <div>
                            ...
                            <input id="tour_first_name" type="text" formControlName="firstName" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium text-slate-900 placeholder-slate-400" placeholder="Adınız">
                          </div>
                          <div>
                            <label for="tour_last_name" class="block text-sm font-bold text-slate-700 mb-2">Soyadınız</label>
                            <input id="tour_last_name" type="text" formControlName="lastName" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium text-slate-900 placeholder-slate-400" placeholder="Soyadınız">
                          </div>
                          <div>
                            <label for="tour_phone" class="block text-sm font-bold text-slate-700 mb-2">Telefon Numaranız</label>
                            <input id="tour_phone" type="tel" formControlName="phone" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium text-slate-900 placeholder-slate-400" placeholder="05XX XXX XX XX">
                          </div>
                          <div>
                            <label for="tour_email" class="block text-sm font-bold text-slate-700 mb-2">E-posta Adresiniz</label>
                            <input id="tour_email" type="email" formControlName="email" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium text-slate-900 placeholder-slate-400" placeholder="ornek@email.com">
                          </div>
                       </div>
                       <div>
                       ...
                       <div class="pt-6 border-t border-slate-100 flex justify-end">
                         <button type="submit" [disabled]="reservationForm.invalid" class="bg-blue-600 disabled:opacity-50 text-white px-8 py-4 rounded-xl font-bold text-lg hover:bg-blue-700 transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)] active:scale-95 w-full sm:w-auto flex items-center justify-center">
                           Rezervasyonu İleri
                 ...
                 @if (reservationStep() === 2) {
                    ...
                    <div class="text-center py-12 flex flex-col items-center">
                       ...
                       <h2 class="text-3xl font-serif font-bold text-slate-900 mb-4">Kaydınız Alındı!</h2>
                       <p class="text-lg text-slate-600 mb-8 max-w-md mx-auto">
                         Ödeme işlemleri için tarafınıza <b>e-posta</b> aracılığıyla bir ödeme bağlantısı ve tur biniş detayları gönderilecektir. Bizi tercih ettiğiniz için teşekkür ederiz.
                       </p>
                       ...
                       <button (click)="finishReservation()" class="bg-slate-900 text-white px-10 py-4 rounded-xl font-bold text-lg hover:bg-slate-800 transition-all active:scale-95 shadow-md">
                         Ana Sayfaya Dön
                       </button>
                    </div>
                 }
      ...
      <div class="min-h-screen flex items-center justify-center bg-slate-50">
        <div class="text-center">
          <mat-icon class="text-6xl text-slate-300 mb-4"
          ...
          <button
            (click)="goBack()"
            class="bg-blue-500 text-slate-900 px-6 py-3 rounded-full font-bold hover:bg-slate-900 hover:text-white transition-colors"
          >
            Geri Dön
          </button>
        </div>
...
export class TourDetailComponent implements OnInit {
  ...
  constructor() {
    effect(() => {
      ...
      if (id) {
        const foundTour = this.carService
          .getTours()()
          .find((t) => t.id == id);
        if (foundTour) {
          this.tour.set(foundTour);
          // Suggest up to 4 rental SUV/Pickup
          const allCars = this.carService.getAllVehicles()();
          let suggestions = allCars
            .filter((c) => c.category === "RENTAL" && (c.type === "SUV" || c.type === "Pickup"));
            
          if (suggestions.length < 4) {
             const others = allCars.filter((c) => c.category === "RENTAL" && !suggestions.includes(c));
  ...
  proceedToPayment() {
    if (this.reservationForm.valid) {
      ...
      const tourReq: any = {
        ...
        notes: `Tur Rezervasyonu: ${t?.title} | Kişi Sayısı: ${this.personCount()} | Not: ${val.notes}`,
        status: 'PENDING',
        totalPrice: (t?.price || 0) * this.personCount(),
        itemName: t?.title
      };
      
      this.carService.addReservation(tourReq as any);
  ...
  increasePerson() {
    if (this.personCount() < 20) {
      this.personCount.update(c => c + 1);
    }
  }
