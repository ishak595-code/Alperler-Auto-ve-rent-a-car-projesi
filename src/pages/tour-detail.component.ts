    ...
    <main class="tour-page">
      @if (tour(); as item) {
        <header class="topbar" [attr.inert]="reservationOpen() ? '' : null" [attr.aria-hidden]="reservationOpen() ? 'true' : null">
          <div class="topbar-inner"><button type="button" class="icon-button" (click)="goBack()" aria-label="Turlardan geri dön"><mat-icon aria-hidden="true">arrow_back</mat-icon></button><div class="topbar-copy"><span>ALPERLER TUR</span><h1>{{ item.title }}</h1></div></div>
        </header>
        ...
        <section class="gallery" [attr.aria-label]="item.title + ' fotoğraf ve video galerisi'" [attr.inert]="reservationOpen() ? '' : null" [attr.aria-hidden]="reservationOpen() ? 'true' : null" (touchstart)="touchStart($event)" (touchend)="touchEnd($event)">
          @if (activeMedia(); as media) {
            <div class="gallery-frame">
              @if (media.kind === 'IMAGE') {<button type="button" class="media-open" (click)="openLightbox()" [attr.aria-label]="media.title + ' tam ekran aç'"><img [src]="media.url" [alt]="media.title || item.title" loading="eager" decoding="async" (error)="mediaFailed(media.url)" /></button>} @else {<video [src]="media.url" [poster]="media.posterUrl || item.image || ''" controls playsinline preload="metadata" [attr.aria-label]="media.title || item.title" (error)="mediaFailed(media.url)"></video><button type="button" class="video-expand" (click)="openLightbox()" aria-label="Videoyu tam ekran galeride aç"><mat-icon aria-hidden="true">fullscreen</mat-icon></button>}
              <div class="gallery-shade" aria-hidden="true"></div><div class="hero-copy"><p>{{ item.duration || 'Tur deneyimi' }}</p><h2>{{ item.title }}</h2><span>{{ item.location || item.meetingPoint }}</span></div><div class="gallery-toolbar"><span>{{ currentSlide() + 1 }} / {{ mediaItems().length }}</span>@if (mediaItems().length > 1) {<div><button type="button" (click)="previousMedia()" aria-label="Önceki medya"><mat-icon aria-hidden="true">chevron_left</mat-icon></button><button type="button" (click)="nextMedia()" aria-label="Sonraki medya"><mat-icon aria-hidden="true">chevron_right</mat-icon></button></div>}</div>
            </div>
          } @else {<div class="gallery-empty" role="status"><mat-icon aria-hidden="true">perm_media</mat-icon><strong>Bu tur için görsel yüklenmedi</strong><p>Fotoğraf ve videolar yüklendikçe burada gösterilecektir</p></div>}
        </section>
        ...
        <div class="detail-layout" [attr.inert]="reservationOpen() ? '' : null" [attr.aria-hidden]="reservationOpen() ? 'true' : null">
          <div class="detail-main">
            <section class="panel summary-panel" aria-labelledby="tour-summary-title">
              <div class="summary-head"><div><p class="eyebrow">Rota Bilgisi</p><h2 id="tour-summary-title">{{ item.title }}</h2><p>{{ summaryMeta(item) }}</p></div><div class="price"><span>{{ activeCampaign() ? 'Kampanya · kişi başı' : 'Kişi başı' }}</span>@if(activeCampaign()?.oldPrice){<small>{{ activeCampaign()!.oldPrice! | turkishCurrency }}</small>}<strong>{{ displayUnitPrice() | turkishCurrency }}</strong></div></div>
            ...
            @if (itineraryRows().length) {<section class="panel accordion-panel"><button type="button" class="accordion-toggle" (click)="routeOpen.update(v => !v)" [attr.aria-expanded]="routeOpen()" aria-controls="tour-route"><span><strong>Tur Programı</strong><small>{{ itineraryRows().length }} durak ve deneyim</small></span><mat-icon aria-hidden="true">{{ routeOpen() ? 'expand_less' : 'expand_more' }}</mat-icon></button>@if (routeOpen()) {<ol id="tour-route" class="route-list">@for (row of itineraryRows(); track $index) {<li><span>{{ $index + 1 }}</span><p>{{ row }}</p></li>}</ol>}</section>}
            ...
            @if (mapHref(item)) {<section class="panel map-panel" aria-labelledby="tour-map-title"><p class="eyebrow">Konum</p><h2 id="tour-map-title">Buluşma ve rota</h2><span>{{ display(item.meetingPoint || item.location) }}</span><a [href]="mapHref(item)" target="_blank" rel="noopener noreferrer"><mat-icon aria-hidden="true">map</mat-icon>Haritada aç</a></section>}
          </div>
        </div>
        ...
        @if (reservationOpen()) {
          <div class="reservation-overlay" role="dialog" aria-modal="true" aria-labelledby="tour-booking-title" tabindex="-1">
            ...
            <div class="reservation-content">
              @if (reservationSuccess()) {<section class="success" role="status"><mat-icon aria-hidden="true">check_circle</mat-icon><h3>Rezervasyon talebiniz alındı</h3><p>Referans: {{ reservationReference() }}</p><button type="button" (click)="closeReservation()" aria-label="Tur detayına dön">Tura Dön</button></section>}
