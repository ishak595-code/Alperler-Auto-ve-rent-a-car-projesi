    ...
    <main class="tour-page">
      @if (tour(); as item) {
        ...
        <section class="gallery" [attr.aria-label]="item.title + ' fotoğraf ve video galerisi'" [attr.inert]="reservationOpen() ? '' : null" [attr.aria-hidden]="reservationOpen() ? 'true' : null" (touchstart)="touchStart($event)" (touchend)="touchEnd($event)">
          ...
          } @else {<div class="gallery-empty" role="status"><mat-icon aria-hidden="true">perm_media</mat-icon><strong>Tur medyası henüz eklenmedi</strong></div>}
        </section>
        ...
        @if (reservationOpen()) {
          <div class="reservation-overlay" role="dialog" aria-modal="true" aria-labelledby="tour-booking-title" tabindex="-1">
            ...
            <div class="reservation-content">
              @if (reservationSuccess()) {<section class="success" role="status"><mat-icon aria-hidden="true">check_circle</mat-icon><h3>Rezervasyon talebiniz kaydedildi</h3><p>Referans: {{ reservationReference() }}</p><button type="button" (click)="closeReservation()" aria-label="Tur detayına dön">Tura Dön</button></section>}
...
export class TourDetailComponent implements OnInit {
  ...
  async reload(): Promise<void> {
    ...
    catch (error) { this.tour.set(null); this.loadError.set(error instanceof Error ? error.message : "Tur verisi alınamadı."); }
