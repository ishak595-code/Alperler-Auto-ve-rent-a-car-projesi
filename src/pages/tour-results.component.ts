  ...
  template: `
    <main class="min-h-screen bg-slate-950 pb-20 text-slate-300">
      <header class="sticky top-0 z-40 border-b border-slate-800 bg-slate-900 shadow-lg">
        <div class="mx-auto flex min-h-16 max-w-7xl items-center gap-2 px-3 py-2 sm:gap-3 sm:px-4">
          <button type="button" (click)="goBack()" class="grid min-h-12 min-w-12 shrink-0 place-items-center rounded-xl text-slate-300 hover:bg-slate-800 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" aria-label="Geri dön">
          ...
          <label class="relative min-w-0 flex-1">
            ...
            <input [(ngModel)]="searchQuery" type="search" inputmode="search" autocomplete="off" placeholder="Tur ara..." class="min-h-12 w-full rounded-xl border border-slate-700 bg-slate-800 pl-11 pr-3 text-sm text-white outline-none focus:border-blue-500" />
          </label>
          <button #filterTrigger type="button" (click)="openFilter()" class="relative grid min-h-12 min-w-12 shrink-0 place-items-center rounded-xl border border-slate-700 bg-slate-800 text-slate-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" aria-haspopup="dialog" [attr.aria-expanded]="filterOpen()" aria-controls="tour-filter-dialog" aria-label="Tur filtrelerini aç">
            <mat-icon aria-hidden="true">tune</mat-icon>
            @if (activeFilterCount() > 0) { <span class="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-blue-500 px-1 text-[10px] font-bold text-white">{{ activeFilterCount() }}</span> }
          </button>
          <button #sortTrigger type="button" (click)="openSort()" class="grid min-h-12 min-w-12 shrink-0 place-items-center rounded-xl border border-slate-700 bg-slate-800 text-slate-300 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" aria-haspopup="dialog" [attr.aria-expanded]="sortOpen()" aria-controls="tour-sort-dialog" aria-label="Tur sıralamasını aç">
            <mat-icon aria-hidden="true">sort</mat-icon>
          </button>
        </div>
      </header>

      <section class="mx-auto max-w-7xl px-4 py-7 sm:py-8">
        <div class="mb-7 sm:mb-8">
          <h1 class="text-3xl font-black text-white md:text-5xl">Turlarımız <span class="text-lg font-semibold text-slate-500 md:text-3xl">({{ filteredTours().length }} tur)</span></h1>
          <p class="mt-3 max-w-3xl text-sm leading-6 text-slate-400 sm:text-base sm:leading-relaxed">Hakkâri ve Yüksekova'nın gerçek rotalarını güncel tur kayıtlarıyla inceleyin. Arama, filtre ve sıralama seçenekleriyle planınıza uygun turu bulun.</p>
        </div>

        @if (filteredTours().length > 0) {
          <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-6">
            @for (tour of filteredTours(); track tour.id) {
              <a [routerLink]="['/tour', tour.cloudSlug || tour.cloudId || tour.id]" class="group flex min-w-0 flex-col overflow-hidden rounded-2xl border border-slate-800 bg-white text-slate-900 shadow-sm transition hover:-translate-y-1 hover:shadow-xl focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
                <div class="relative aspect-[16/10] overflow-hidden bg-slate-100">
                  @if (tour.image) {
                    <img [src]="tour.image" [alt]="tour.title + ' turu kapak görseli'" loading="lazy" decoding="async" class="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  } @else {
                    <div class="grid h-full w-full place-items-center bg-gradient-to-br from-slate-100 to-slate-200 text-slate-400" aria-label="Bu tur için kapak görseli eklenmedi"><mat-icon aria-hidden="true" class="!h-14 !w-14 !text-[56px]">landscape</mat-icon></div>
                  }
                ...
                <div class="flex flex-1 flex-col p-4">
                  ...
                  <p class="mt-2 line-clamp-3 text-sm leading-relaxed text-slate-500">{{ tour.description }}</p>
                  <div class="mt-auto flex items-end justify-between gap-3 border-t border-slate-100 pt-4">
                    <div class="min-w-0"><span class="block text-[10px] font-black uppercase tracking-wider text-slate-400">Kişi başı</span><span class="block truncate text-xl font-black">{{ tour.price | number:'1.0-0' }} ₺</span></div>
                    <span class="shrink-0 rounded-xl bg-blue-50 px-3 py-2 text-sm font-black text-blue-700">İncele</span>
                  </div>
                </div>
              </a>
            }
          ...
          <div class="rounded-3xl border-2 border-dashed border-slate-800 bg-slate-900 p-8 text-center sm:p-12">
            <mat-icon aria-hidden="true" class="!h-14 !w-14 !text-6xl text-slate-600">explore_off</mat-icon>
      ...
      <dialog #filterDialog id="tour-filter-dialog" (close)="onFilterClosed()" (cancel)="onFilterCancelled($event)" class="m-auto w-[min(92vw,32rem)] rounded-3xl bg-white p-0 text-slate-900 shadow-2xl backdrop:bg-black/70">
        <form method="dialog" class="overflow-hidden rounded-3xl">
          <div class="flex items-center justify-between border-b border-slate-100 p-5">
            ...
            <button type="button" (click)="closeFilter()" class="grid min-h-12 min-w-12 place-items-center rounded-xl hover:bg-slate-100" aria-label="Filtreyi kapat"><mat-icon aria-hidden="true">close</mat-icon></button>
          </div>
          ...
          <div class="grid grid-cols-1 gap-3 border-t border-slate-100 p-5 min-[390px]:grid-cols-2">
      ...
      <dialog #sortDialog id="tour-sort-dialog" (close)="onSortClosed()" (cancel)="onSortCancelled($event)" class="m-auto w-[min(92vw,26rem)] rounded-3xl bg-white p-0 text-slate-900 shadow-2xl backdrop:bg-black/70">
        <div class="overflow-hidden rounded-3xl">
          <div class="flex items-center justify-between border-b border-slate-100 p-5"><h2 class="text-xl font-black">Sırala</h2><button type="button" (click)="closeSort()" class="grid min-h-12 min-w-12 place-items-center rounded-xl hover:bg-slate-100" aria-label="Sıralamayı kapat"><mat-icon aria-hidden="true">close</mat-icon></button></div>
          <div class="space-y-2 p-4">
            @for (option of sortOptions; track option.id) {
              <button type="button" (click)="applySort(option.id)" class="flex min-h-12 w-full items-center justify-between rounded-xl px-4 text-left font-bold hover:bg-slate-50" [class.bg-blue-50]="sortOption() === option.id" [class.text-blue-700]="sortOption() === option.id">{{ option.label }} @if (sortOption() === option.id) { <mat-icon aria-hidden="true">check</mat-icon> }</button>
            }
          </div>
        </div>
      </dialog>
...
export class TourResultsComponent {
  ...
  readonly filteredTours = computed(() => {
    ...
    if (query) tours = tours.filter((tour) => `${tour.title || ''} ${tour.description || ''} ${tour.category || ''}`.toLocaleLowerCase('tr-TR').includes(query));
  ...
  constructor() {
    this.route.queryParams.subscribe((params) => { if (params['search']) this.searchQuery.set(String(params['search'])); });
  }
