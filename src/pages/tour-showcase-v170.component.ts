...
@Component({
  ...
  template: `
    <main class="page">
      ...
      <section class="hero">
        <div><p>ALPERLER TUR</p><h1>Rotanı seç, tarihini belirle</h1><span>Fiyat, süre, konum, program, medya ve tur özellikleri canlı yönetim verisinden gelir.</span></div>
        @if(bounds();as range){<aside><small>Canlı fiyat aralığı</small><strong>{{range.min|number:'1.0-0'}} - {{range.max|number:'1.0-0'}} TL</strong><em>{{tours().length}} yayınlanmış tur</em></aside>}
      </section>

      @if(filterOpen()){
        <section id="tour-filter-v170" class="filters" aria-label="Tur filtreleri">
          @if(bounds();as range){
            <label><span>Minimum fiyat</span><input type="number" [min]="range.min" [max]="range.max" [ngModel]="minPrice()" (ngModelChange)="setMin($event)" /></label>
            <label><span>Maksimum fiyat</span><input type="number" [min]="range.min" [max]="range.max" [ngModel]="maxPrice()" (ngModelChange)="setMax($event)" /></label>
          }
          ...
          <label><span>Kategori</span><select [ngModel]="category()" (ngModelChange)="category.set($event)"><option value="ALL">Tümü</option>@for(value of categories();track value){<option [value]="value">{{value}}</option>}</select></label>
          ...
          <label><span>Sıralama</span><select [ngModel]="sort()" (ngModelChange)="sort.set($event)"><option value="featured">Öne çıkanlar</option><option value="priceAsc">Fiyat artan</option><option value="priceDesc">Fiyat azalan</option><option value="title">Ada göre</option></select></label>
          <button type="button" (click)="reset()">Filtreleri temizle</button>
        </section>
      }

      <section class="content">
        <div class="head"><div><p>{{results().length}} tur bulundu</p><h2>Canlı tur vitrini</h2></div>@if(activeFilters()){<button type="button" (click)="reset()">Temizle</button>}</div>
        @if(loading()){
          <div class="state"><div class="spinner"></div><strong>Turlar yükleniyor</strong></div>
        }@else if(error()){
          <div class="state"><mat-icon aria-hidden="true">error_outline</mat-icon><strong>Turlar yüklenemedi</strong><span>{{error()}}</span><button type="button" (click)="reload()">Tekrar dene</button></div>
          ...
          <div class="grid">
            @for(tour of results();track tour.cloudId||tour.id){
              <a class="card" [routerLink]="['/tour',tour.cloudSlug||tour.cloudId||tour.id]" [attr.aria-label]="tour.title + ' tur detayını aç'">
                <div class="media">
                  @if(tour.image){<img [src]="tour.image" [alt]="tour.title + ' kapak görseli'" loading="lazy" decoding="async" />}@else{<div class="media-empty"><mat-icon aria-hidden="true">landscape</mat-icon><span>Kapak görseli yok</span></div>}
                  <div class="shade"></div>
                  <div class="badges">
                ...
                <div class="body">
                  ...
                  <div class="facts">
                    @if(tour.capacity){<span><mat-icon aria-hidden="true">groups</mat-icon>Önerilen grup: {{tour.capacity}} kişi</span>}
...
export class TourShowcaseV170Component {
  ...
  async reload(): Promise<void> { this.loading.set(true); this.error.set(""); try { this.tours.set(await this.data.list()); } catch(error) { this.error.set(error instanceof Error ? error.message : "Tur verisi alınamadı."); } finally { this.loading.set(false); } }
