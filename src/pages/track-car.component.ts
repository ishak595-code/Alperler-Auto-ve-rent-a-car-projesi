...
@Component({
  ...
  template: `
    <main class="tracking-page">
      <header class="topbar">
        <button type="button" class="back" (click)="goBack()" [attr.aria-label]="t().trackCar.backAria">
          <mat-icon aria-hidden="true">arrow_back</mat-icon>
        </button>
        <div class="head-copy">
          <p>{{ t().trackCar.kicker }}</p>
          <h1>{{ bookingId() || t().trackCar.bookingFallback }}</h1>
          <span>{{ t().trackCar.liveHint }}</span>
        </div>
      </header>
      ...
      <section class="shell">
        @if (authorizing() || !bookingService.isAdminLoaded()) {
          <div class="state-card" role="status" aria-live="polite">
            ...
            <h2>{{ t().trackCar.loadingTitle }}</h2>
            <p>{{ t().trackCar.loadingHint }}</p>
          </div>
        } @else if (accessError()) {
          <div class="state-card error" role="alert">
            <mat-icon aria-hidden="true">lock</mat-icon>
            <h2>{{ t().trackCar.accessDeniedTitle }}</h2>
            <p>{{ accessError() }}</p>
          </div>
        } @else if (bookingService.lastAdminError()) {
          <div class="state-card error" role="alert">
            <mat-icon aria-hidden="true">cloud_off</mat-icon>
            <h2>{{ t().trackCar.dataErrorTitle }}</h2>
            <p>{{ bookingService.lastAdminError() }}</p>
          </div>
        } @else if (!booking()) {
          <div class="state-card" role="status">
            <mat-icon aria-hidden="true">search_off</mat-icon>
            <h2>{{ t().trackCar.notFoundTitle }}</h2>
            <p>{{ notFoundHint() }}</p>
            <button type="button" class="primary" (click)="openReservations()">{{ t().trackCar.backToReservations }}</button>
          </div>
        } @else {
          @let record = booking()!;
          <section class="hero-card" aria-labelledby="tracking-item-title">
            ...
            <div class="hero-copy">
              ...
              <p class="reference">{{ t().trackCar.reference }} <strong>{{ record.id }}</strong></p>
              <div class="hero-actions">
                @if (record.customerPhone) {
                  <a [href]="phoneHref(record.customerPhone)" class="primary"><mat-icon aria-hidden="true">call</mat-icon>{{ t().trackCar.callCustomer }}</a>
                  <a [href]="whatsappHref(record.customerPhone, record.id)" target="_blank" rel="noopener noreferrer" class="secondary"><mat-icon aria-hidden="true">chat</mat-icon>{{ t().trackCar.whatsapp }}</a>
                }
                @if (record.itemId && record.type === 'RENTAL') {
                  <button type="button" class="secondary" (click)="openVehicle(record)"><mat-icon aria-hidden="true">open_in_new</mat-icon>{{ t().trackCar.openVehicle }}</button>
                }
              </div>
            </div>
          </section>
          ...
          <div class="grid">
            <section class="panel" aria-labelledby="customer-title">
              <div class="panel-head"><mat-icon aria-hidden="true">person</mat-icon><h2 id="customer-title">{{ t().trackCar.customerTitle }}</h2></div>
              <dl>
                <div><dt>{{ t().trackCar.fullName }}</dt><dd>{{ record.customerName }}</dd></div>
                <div><dt>{{ t().trackCar.phone }}</dt><dd><a [href]="phoneHref(record.customerPhone)">{{ record.customerPhone }}</a></dd></div>
                <div><dt>{{ t().trackCar.email }}</dt><dd>{{ record.customerEmail || t().trackCar.notSpecified }}</dd></div>
                <div><dt>{{ t().trackCar.source }}</dt><dd>{{ record.source || 'WEB' }}</dd></div>
              </dl>
            </section>

            <section class="panel" aria-labelledby="rental-title">
              <div class="panel-head"><mat-icon aria-hidden="true">event</mat-icon><h2 id="rental-title">{{ t().trackCar.detailTitle }}</h2></div>
              <dl>
                <div><dt>{{ t().trackCar.start }}</dt><dd>{{ dateLabel(record.startDate) }}</dd></div>
                <div><dt>{{ t().trackCar.end }}</dt><dd>{{ dateLabel(record.endDate) }}</dd></div>
                <div><dt>{{ t().trackCar.duration }}</dt><dd>{{ durationLabel(record) }}</dd></div>
                <div><dt>{{ t().trackCar.driver }}</dt><dd>{{ record.withDriver ? t().trackCar.withDriver : t().trackCar.withoutDriver }}</dd></div>
              </dl>
            </section>

            <section class="panel" aria-labelledby="route-title">
              <div class="panel-head"><mat-icon aria-hidden="true">route</mat-icon><h2 id="route-title">{{ t().trackCar.routeTitle }}</h2></div>
              <div class="route-block">
                <span class="route-dot start" aria-hidden="true"></span>
                <div><small>{{ t().trackCar.pickup }}</small><strong>{{ record.pickupLocation || t().trackCar.notSpecified }}</strong></div>
              </div>
              <div class="route-line" aria-hidden="true"></div>
              <div class="route-block">
                <span class="route-dot finish" aria-hidden="true"></span>
                <div><small>{{ t().trackCar.dropoff }}</small><strong>{{ record.dropoffLocation || record.pickupLocation || t().trackCar.notSpecified }}</strong></div>
              </div>
            </section>

            <section class="panel" aria-labelledby="payment-title">
              <div class="panel-head"><mat-icon aria-hidden="true">payments</mat-icon><h2 id="payment-title">{{ t().trackCar.paymentTitle }}</h2></div>
              <dl>
                <div><dt>{{ t().trackCar.method }}</dt><dd>{{ paymentMethodLabel(record.paymentMethod) }}</dd></div>
                <div><dt>{{ t().trackCar.status }}</dt><dd>{{ paymentStatusLabel(record.paymentStatus) }}</dd></div>
                <div><dt>{{ t().trackCar.total }}</dt><dd class="price">{{ money(record.totalPrice || record.basePrice || 0, record.currency) }}</dd></div>
              </dl>
            </section>
          </div>
          ...
          <section class="telemetry" aria-labelledby="telemetry-title">
            ...
            <div>
              <p>{{ t().trackCar.telemetryKicker }}</p>
              <h2 id="telemetry-title">{{ t().trackCar.telemetryTitle }}</h2>
              <span>{{ t().trackCar.telemetryHint }}</span>
            </div>
          </section>

          @if (record.notes) {
            <section class="notes" aria-labelledby="notes-title"><h2 id="notes-title">{{ t().trackCar.notesTitle }}</h2><p>{{ record.notes }}</p></section>
          }
        }
      </section>
    </main>
...
export class TrackCarComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (!allowed) {
      this.accessError.set(this.t().trackCar.accessDenied);
  ...
  whatsappHref(phone: string, reference: string): string {
    ...
    const message = String(this.t().trackCar.whatsappMessage || "").replace("{ref}", reference);
    return `https://wa.me/${digits}?text=${encodeURIComponent(message)}`;
  }

  notFoundHint(): string {
    return String(this.t().trackCar.notFoundHint || "").replace("{id}", this.bookingId());
  }

  durationLabel(record: BookingRecord): string {
    if (record.days) return String(this.t().trackCar.days || "").replace("{n}", String(record.days));
    return record.rentalDuration || this.t().trackCar.notSpecified;
  }

  typeLabel(record: BookingRecord): string {
    const tc = this.t().trackCar;
    if (record.type === "RENTAL") return tc.typeRental;
    if (record.type === "TOUR") return tc.typeTour;
    if (record.type === "SALE_INQUIRY") return tc.typeSale;
    return tc.typeAppointment;
  }

  statusLabel(status: BookingRecord["status"]): string {
    const tc = this.t().trackCar;
    return ({ PENDING: tc.statusPending, APPROVED: tc.statusApproved, REJECTED: tc.statusRejected, COMPLETED: tc.statusCompleted, CANCELLED: tc.statusCancelled } as const)[status] || status;
  }
  ...
  dateLabel(value?: string): string {
    if (!value) return this.t().trackCar.notSpecified;
    const date = new Date(value);
    if (Number.isNaN(date.getTime())) return value;
    return new Intl.DateTimeFormat(this.localeTag(), { dateStyle: "medium", timeStyle: "short" }).format(date);
  }

  money(amount: number, currency = "TRY"): string {
    if (!Number.isFinite(amount) || amount <= 0) return this.t().trackCar.notSpecified;
    return new Intl.NumberFormat(this.localeTag(), { style: "currency", currency, maximumFractionDigits: 2 }).format(amount);
  }

  paymentMethodLabel(value?: string): string {
    const tc = this.t().trackCar;
    return ({ CARD: tc.payCard, EFT: tc.payEft, OFFICE: tc.payOffice, NONE: tc.payNone } as Record<string, string>)[String(value || "NONE")] || String(value || tc.notSpecified);
  }

  paymentStatusLabel(value?: string): string {
    const tc = this.t().trackCar;
    return ({ PAID: tc.payPaid, PENDING: tc.payPending, FAILED: tc.payFailed, REFUNDED: tc.payRefunded, NOT_REQUIRED: tc.payNotRequired } as Record<string, string>)[String(value || "NOT_REQUIRED")] || String(value || tc.notSpecified);
  }
}
