...
export class BookingService {
  ...
  private async refreshAdminRecords(showLoading = true): Promise<void> {
    ...
    try {
      ...
      const records = response.bookings.map((record) => this.fromApi(record)).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  ...
  private async request<T>(method: "GET" | "POST" | "PATCH" | "DELETE", body?: unknown): Promise<T> {
    try {
      if (method === "POST") return await firstValueFrom(this.http.post<T>("/api/bookings", body));
  ...
  private fromApi(record: ApiBooking): BookingRecord {
    return { ...record, createdAt: this.asDate(record.createdAt), updatedAt: this.asDate(record.updatedAt) };
  }
  ...
  private duplicateReport(bookingId: string, event: BookingNotificationEvent): NotificationDeliveryReport {
    const channel = { state: "skipped" as const, reason: "STATUS_UNCHANGED" };
    return { ok: true, event, bookingId, alreadyProcessed: true, email: channel, sms: channel, adminEmail: channel };
  }

  private upsertLocal(record: BookingRecord): void {
    this.bookings.update((records) => [record, ...records.filter((current) => current.id !== record.id)]);
  }
  ...
  private optionalAmount(value: number | undefined): number | undefined {
    ...
    if (!Number.isFinite(value) || value < 0 || value > 50_000_000) throw new Error("Tutar alanı geçerli değil.");
    return Math.round(value * 100) / 100;
  }

  private optionalInteger(value: number | undefined, min: number, max: number): number | undefined {
    if (value === undefined) return undefined;
    if (!Number.isInteger(value) || value < min || value > max) throw new Error("Sayısal alan geçerli değil.");
    return value;
  }

  private requiredText(value: string | undefined, field: string, max: number): string {
