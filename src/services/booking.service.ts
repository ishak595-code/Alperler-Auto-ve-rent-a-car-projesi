...
interface PublicAvailabilityAlternative { vehicleId:string; stockCode?:string; brand?:string; model?:string; coverImage?:string; branchId?:string; dailyPrice?:number; hourlyPrice?:number; bodyType?:string; seats?:number; score?:number; reason?:string; }
interface AvailabilityApiResponse { ok:boolean; available?:boolean; vehicleId?:string; startAt?:string; endAt?:string; branchTimezone?:string; alternatives?:PublicAvailabilityAlternative[]; code?:string; message?:string; requestId?:string; }
...
export class BookingService {
  ...
  private async evaluateRentalAvailability(input:CreateBookingInput):Promise<{startAt:string;endAt:string;branchTimezone?:string}>{
    const itemId=input.itemId===undefined?"":String(input.itemId).trim();const startAt=input.startDate?.trim()||"";const endAt=input.endDate?.trim()||"";if(!itemId)throw new Error("INVALID_RENTAL_VEHICLE:Kiralık araç seçimi geçerli değil.");if(!startAt||!endAt)throw new Error("INVALID_RENTAL_DATES:Teslim alma ve iade tarihlerini kontrol edin.");
    const startLocal=this.wallClockValue(startAt),endLocal=this.wallClockValue(endAt);if(endLocal<=startLocal)throw new Error("INVALID_RENTAL_DATES:Teslim alma ve iade tarihlerini kontrol edin.");const token=await this.customerAuth.getAccessToken().catch(()=>null);const requestId=crypto.randomUUID();const headers:Record<string,string>={"content-type":"application/json","x-request-id":requestId};if(token)headers.Authorization=`Bearer ${token}`;const body={vehicleId:itemId,startLocal,endLocal};
    try{return this.validateAvailability(await firstValueFrom(this.http.post<AvailabilityApiResponse>("/api/rental-availability",body,{headers})));}catch(error){if(error instanceof HttpErrorResponse&&this.shouldTryDirect(error))return await this.directAvailability(body,token,requestId);if(error instanceof HttpErrorResponse&&error.error&&typeof error.error==="object"){const payload=error.error as AvailabilityApiResponse;this.publicAlternatives.set(Array.isArray(payload.alternatives)?payload.alternatives:[]);throw new Error(`${String(payload.code||"AVAILABILITY_CHECK_FAILED")}:${String(payload.message||"Araç uygunluğu doğrulanamadı.")}`);}if(error instanceof Error)throw error;throw new Error("AVAILABILITY_CHECK_FAILED:Araç uygunluğu doğrulanamadı.");}
  }
  private validateAvailability(response:AvailabilityApiResponse):{startAt:string;endAt:string;branchTimezone?:string}{this.publicAlternatives.set(Array.isArray(response.alternatives)?response.alternatives:[]);if(!response.ok||!response.startAt||!response.endAt)throw new Error(`${response.code||"AVAILABILITY_CHECK_FAILED"}:${response.message||"Araç uygunluğu doğrulanamadı."}`);if(response.available!==true)throw new Error(`VEHICLE_UNAVAILABLE:${response.message||"Bu araç seçilen zaman aralığında dolu."}`);const startMs=new Date(response.startAt).getTime(),endMs=new Date(response.endAt).getTime();if(!Number.isFinite(startMs)||!Number.isFinite(endMs)||endMs<=startMs)throw new Error("INVALID_RENTAL_DATES:Kiralama tarihleri doğrulanamadı.");return{startAt:response.startAt,endAt:response.endAt,branchTimezone:response.branchTimezone};}
  private async directAvailability(body:{vehicleId:string;startLocal:string;endLocal:string},token:string|null,requestId:string):Promise<{startAt:string;endAt:string;branchTimezone?:string}>{let response:Response;try{response=await fetch(`${SUPABASE_PROJECT_URL}/functions/v1/rental-availability`,{method:"POST",headers:{"content-type":"application/json","x-request-id":requestId,...(token?{authorization:`Bearer ${token}`}:{})},body:JSON.stringify(body),signal:AbortSignal.timeout(12_000)});}catch{throw new Error("AVAILABILITY_SERVICE_UNAVAILABLE:Araç uygunluğu servisine ulaşılamıyor.");}const payload=await response.json().catch(()=>({})) as AvailabilityApiResponse;if(!response.ok){this.publicAlternatives.set(Array.isArray(payload.alternatives)?payload.alternatives:[]);throw new Error(`${String(payload.code||`AVAILABILITY_HTTP_${response.status}`)}:${String(payload.message||"Araç uygunluğu doğrulanamadı.")}`);}return this.validateAvailability(payload);}
  private wallClockValue(value:string):string{const raw=value.trim();const dateOnly=/^(\d{4})-(\d{2})-(\d{2})$/.exec(raw);if(dateOnly)return`${dateOnly[1]}-${dateOnly[2]}-${dateOnly[3]}T00:00:00`;const date=new Date(raw);if(Number.isNaN(date.getTime()))throw new Error("INVALID_RENTAL_DATES:Kiralama tarihi geçerli değil.");const pad=(part:number)=>String(part).padStart(2,"0");return`${date.getFullYear()}-${pad(date.getMonth()+1)}-${pad(date.getDate())}T${pad(date.getHours())}:${pad(date.getMinutes())}:${pad(date.getSeconds())}`;}
  ...
  private async adminAction(method:"GET"|"POST",body?:unknown):Promise<AdminActionResponse>{const token=await this.authService.getAccessToken();if(!token)throw new Error("ADMIN_SESSION_REQUIRED");const requestId=crypto.randomUUID();const headers={Authorization:`Bearer ${token}`,"content-type":"application/json","x-request-id":requestId};try{return method==="GET"?await firstValueFrom(this.http.get<AdminActionResponse>("/api/admin-booking-actions",{headers})):await firstValueFrom(this.http.post<AdminActionResponse>("/api/admin-booking-actions",body,{headers}));}catch(error){if(error instanceof HttpErrorResponse&&this.shouldTryDirect(error)){let response:Response;try{response=await fetch(`${SUPABASE_PROJECT_URL}/functions/v1/booking-admin-actions`,{method,headers:{authorization:`Bearer ${token}`,"content-type":"application/json","x-request-id":requestId},body:method==="GET"?undefined:JSON.stringify(body??{}),signal:AbortSignal.timeout(15_000)});}catch{throw new Error("ADMIN_BOOKING_ACTIONS_UNAVAILABLE:Yönetim işlemi servisine ulaşılamıyor.");}const payload=await response.json().catch(()=>({})) as AdminActionResponse;if(!response.ok)throw new Error(`${payload.code||`ADMIN_ACTION_HTTP_${response.status}`}:${payload.message||"Yönetim işlemi tamamlanamadı."}`);return payload;}throw this.normalizeRequestError(error);}}

  private async request<T>(method:"GET"|"POST"|"PATCH"|"DELETE",body?:unknown):Promise<T>{const token=method==="POST"?await this.customerAuth.getAccessToken().catch(()=>null):await this.authService.getAccessToken();if(method!=="POST"&&!token)throw new Error("ADMIN_SESSION_REQUIRED");const headers=token?{Authorization:`Bearer ${token}`} : undefined;try{if(method==="POST")return await firstValueFrom(this.http.post<T>("/api/bookings",body,headers?{headers}:{}));if(method==="GET")return await firstValueFrom(this.http.get<T>("/api/bookings",{headers:headers!}));return await firstValueFrom(this.http.request<T>(method,"/api/bookings",{body,headers:headers!}));}catch(error){if(error instanceof HttpErrorResponse&&this.shouldTryDirect(error))return await this.directGateway<T>(method,body,token);throw this.normalizeRequestError(error);}}
  private shouldTryDirect(error:HttpErrorResponse):boolean{return error.status===0||error.status===404||error.status===405||error.status===408||error.status===502||error.status===503||error.status===504;}
  private async directGateway<T>(method:"GET"|"POST"|"PATCH"|"DELETE",body:unknown,token:string|null):Promise<T>{let response:Response;try{response=await fetch(`${SUPABASE_PROJECT_URL}/functions/v1/booking-gateway`,{method,headers:{"content-type":"application/json","x-request-id":crypto.randomUUID(),...(token?{authorization:`Bearer ${token}`}:{})},body:method==="GET"?undefined:JSON.stringify(body??{}),signal:AbortSignal.timeout(20_000)});}catch{throw new Error("BOOKING_GATEWAY_UNAVAILABLE:Rezervasyon servisine ulaşılamıyor.");}const payload=await response.json().catch(()=>({})) as {code?:unknown;message?:unknown}&T;if(!response.ok)throw new Error(`${String(payload.code||`BOOKING_HTTP_${response.status}`)}:${String(payload.message||payload.code||"Rezervasyon işlemi tamamlanamadı.")}`);return payload as T;}
  ...
  private normalizeInput(input:CreateBookingInput):CreateBookingInput{const itemName=this.requiredText(input.itemName,"itemName",240),customerName=this.requiredText(input.customerName,"customerName",160),customerPhone=this.requiredText(input.customerPhone,"customerPhone",40),customerEmail=input.customerEmail?.trim().toLowerCase().slice(0,160)||"";if(customerEmail&&!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customerEmail))throw new Error("Geçerli bir e-posta adresi girilmelidir.");const paymentMethod=input.paymentMethod??"NONE",defaultPaymentStatus:PaymentStatus=paymentMethod==="NONE"?"NOT_REQUIRED":"PENDING",selectedExtraIds=Array.isArray(input.selectedExtraIds)?Array.from(new Set(input.selectedExtraIds.map((id)=>String(id||"").trim()).filter((id)=>/^[a-z0-9][a-z0-9_-]{0,63}$/i.test(id)))).slice(0,30):undefined,rentalDuration=input.rentalDuration;if(rentalDuration&&!["hourly","daily","weekly","monthly","longterm"].includes(rentalDuration))throw new Error("Kiralama türü geçerli değil.");return{...input,itemId:input.itemId===undefined?undefined:String(input.itemId).trim().slice(0,128),itemName,image:input.image?.trim().slice(0,2048),customerName,customerPhone,customerEmail:customerEmail||undefined,basePrice:this.optionalAmount(input.basePrice),totalPrice:this.optionalAmount(input.totalPrice),currency:input.currency??"TRY",personCount:this.optionalInteger(input.personCount,1,100),startDate:input.startDate?.slice(0,64),endDate:input.endDate?.slice(0,64),days:input.rentalHours?undefined:this.optionalInteger(input.days,1,3650),rentalHours:this.optionalInteger(input.rentalHours,1,23),pickupBranchId:this.optionalUuid(input.pickupBranchId),pickupLocation:input.pickupLocation?.trim().slice(0,240),dropoffLocation:input.dropoffLocation?.trim().slice(0,240),rentalDuration,selectedExtraIds,campaignId:this.optionalUuid(input.campaignId),notes:input.notes?.trim().slice(0,4000),paymentMethod,paymentStatus:input.paymentStatus??defaultPaymentStatus,externalPaymentReference:undefined,source:"WEB"};}
