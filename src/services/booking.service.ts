...
interface BookingApiResponse {
  ...
  approval?: ApprovalImpact;
  offer?: Record<string, unknown>;
  code?: string;
  message?: string;
}
interface PublicAvailabilityAlternative {
  vehicleId: string;
  stockCode?: string;
  brand?: string;
  model?: string;
...
export class BookingService {
  ...
  async create(input: CreateBookingInput): Promise<BookingRecord> {
    ...
    this.publicAlternatives.set([]);
    if (normalized.type === "RENTAL") {
      const availability = await this.evaluateRentalAvailability(normalized);
      normalized.startDate = availability.startAt;
      normalized.endDate = availability.endAt;
    }
    const envelope = { ...normalized, idempotencyKey, analyticsSessionId: currentAnalyticsSessionId() };
  ...
  async updateStatus(id: string, status: BookingStatus): Promise<NotificationDeliveryReport> {
    ...
    if (response.notification) record.notification = response.notification;
    this.upsertLocal(record);
    if (status === "APPROVED" && response.approval) this.approvalImpact.set(response.approval);
    if (status === "APPROVED" || status === "PENDING" || status === "CANCELLED" || status === "REJECTED") {
      await this.refreshAdminRecords(false).catch(() => undefined);
    }
  ...
  async offerAlternative(bookingId: string, offerId: string): Promise<void> {
    const response = await this.request<BookingApiResponse>("PATCH", {
      id: bookingId,
      operation: "alternative_offer",
      offerId,
      requestId: crypto.randomUUID(),
    });
    if (!response.ok) throw new Error(`${response.code || "ALTERNATIVE_OFFER_FAILED"}:${response.message || "Alternatif kaydedilemedi."}`);
    await this.refreshAdminRecords(false);
  }
  ...
  private async evaluateRentalAvailability(input: CreateBookingInput): Promise<{ startAt: string; endAt: string; branchTimezone?: string }> {
    ...
    const body = { vehicleId: itemId, startLocal, endLocal };

    try {
      const response = await firstValueFrom(this.http.post<AvailabilityApiResponse>("/api/rental-availability", body, { headers }));
      return this.validateAvailability(response);
    } catch (error) {
      if (error instanceof HttpErrorResponse && this.shouldTryDirect(error)) {
        return await this.directAvailability(body, token, requestId);
      }
      if (error instanceof HttpErrorResponse && error.error && typeof error.error === "object") {
        const payload = error.error as AvailabilityApiResponse;
        this.publicAlternatives.set(Array.isArray(payload.alternatives) ? payload.alternatives : []);
        throw new Error(`${String(payload.code || "AVAILABILITY_CHECK_FAILED")}:${String(payload.message || "Araç uygunluğu doğrulanamadı.")}`);
      }
  ...
  private validateAvailability(response: AvailabilityApiResponse): { startAt: string; endAt: string; branchTimezone?: string } {
    this.publicAlternatives.set(Array.isArray(response.alternatives) ? response.alternatives : []);
    if (!response.ok || !response.startAt || !response.endAt) {
      throw new Error(`${response.code || "AVAILABILITY_CHECK_FAILED"}:${response.message || "Araç uygunluğu doğrulanamadı."}`);
    }
    if (response.available !== true) throw new Error(`VEHICLE_UNAVAILABLE:${response.message || "Bu araç seçilen zaman aralığında dolu."}`);
    const startMs = new Date(response.startAt).getTime();
    const endMs = new Date(response.endAt).getTime();
    if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || endMs <= startMs) throw new Error("INVALID_RENTAL_DATES:Kiralama tarihleri doğrulanamadı.");
    return { startAt: response.startAt, endAt: response.endAt, branchTimezone: response.branchTimezone };
  }

  private async directAvailability(body: { vehicleId: string; startLocal: string; endLocal: string }, token: string | null, requestId: string): Promise<{ startAt: string; endAt: string; branchTimezone?: string }> {
    ...
    if (!response.ok) {
      this.publicAlternatives.set(Array.isArray(payload.alternatives) ? payload.alternatives : []);
      throw new Error(`${String(payload.code || `AVAILABILITY_HTTP_${response.status}`)}:${String(payload.message || "Araç uygunluğu doğrulanamadı.")}`);
    }
    return this.validateAvailability(payload);
  }
  ...
  private async request<T>(method: "GET" | "POST" | "PATCH" | "DELETE", body?: unknown): Promise<T> {
    const token = method === "POST" ? await this.customerAuth.getAccessToken().catch(() => null) : await this.authService.getAccessToken();
    ...
    try {
      ...
      if (error instanceof HttpErrorResponse && this.shouldTryDirect(error)) return await this.directGateway<T>(method, body, token);
      throw this.normalizeRequestError(error);
    }
  }

  private shouldTryDirect(error: HttpErrorResponse): boolean {
  ...
  private async directGateway<T>(method: "GET" | "POST" | "PATCH" | "DELETE", body: unknown, token: string | null): Promise<T> {
    ...
    try {
      response = await fetch(`${SUPABASE_PROJECT_URL}/functions/v1/booking-gateway`, {
        method,
        headers: { "content-type": "application/json", "x-request-id": crypto.randomUUID(), ...(token ? { authorization: `Bearer ${token}` } : {}) },
    ...
    if (!response.ok) throw new Error(`${String(payload.code || `BOOKING_HTTP_${response.status}`)}:${String(payload.message || payload.code || "Rezervasyon işlemi tamamlanamadı.")}`);
    return payload as T;
  }
  ...
  private normalizeRequestError(error: unknown): Error {
    ...
    if (error instanceof HttpErrorResponse && error.error && typeof error.error === "object") {
      ...
      return new Error(`${String(payload.code || "BOOKING_BACKEND_UNAVAILABLE")}:${String(payload.message || payload.code || "Rezervasyon servisine ulaşılamıyor.")}`);
    }
  ...
  private normalizeInput(input: CreateBookingInput): CreateBookingInput {
    ...
    const selectedExtraIds = Array.isArray(input.selectedExtraIds) ? Array.from(new Set(input.selectedExtraIds.map((id) => String(id || "").trim()).filter((id) => /^[a-z0-9][a-z0-9_-]{0,63}$/i.test(id)))).slice(0, 30) : undefined;
