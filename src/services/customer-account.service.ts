...
export class CustomerAccountService{
  ...
  private async patchProfile(body:Record<string,unknown>,token:string):Promise<void>{const userId=this.auth.user()?.id||'';if(!userId)throw new Error('CUSTOMER_SESSION_REQUIRED');const response=await fetch(`${SUPABASE_PROJECT_URL}/rest/v1/customer_profiles?user_id=eq.${encodeURIComponent(userId)}`,{method:'PATCH',headers:this.headers(token,{Prefer:'return=representation'}),body:JSON.stringify(body)});if(!response.ok)throw new Error(`PROFILE_UPDATE_${response.status}`);const rows=await response.json() as CustomerProfile[];this.profile.set(rows[0]||this.profile());}
