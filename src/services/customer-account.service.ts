...
export interface SafePaymentMethod { id:string;provider:string;provider_scope:'SANDBOX'|'LIVE';brand?:string|null;last4?:string|null;expiry_month?:number|null;expiry_year?:number|null;label?:string|null;is_default:boolean;status:string; }
...
export class CustomerAccountService{
  ...
  async refresh():Promise<void>{const token=await this.requireToken();this.loading.set(true);try{await this.rpc('ensure_customer_profile',{},token);await this.rpc<string>('get_or_create_customer_referral_code',{},token);const[profile,loyalty,ledger,bookings,methods,settings,referral,lifetime]=await Promise.all([
    ...
    this.getRows<SafePaymentMethod>('customer_payment_methods?status=eq.ACTIVE&select=id,provider,provider_scope,brand,last4,expiry_month,expiry_year,label,is_default,status&order=is_default.desc,created_at.desc',token),
