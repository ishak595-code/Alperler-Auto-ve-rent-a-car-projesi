...
export class CustomerAuthService {
  ...
  async signIn(email:string,password:string):Promise<boolean>{
    ...
    try{const response=await fetch(`${supabaseAuthUrl('token')}?grant_type=password`,{method:'POST',headers:this.publicHeaders(),body:JSON.stringify({email:cleanEmail,password})});const payload=(await response.json().catch(()=>({}))) as AuthPayload;if(!response.ok||!payload.access_token||!payload.refresh_token)return this.fail(this.authMessage(payload,'Giriş yapılamadı.'));this.savePayload(payload);await this.ensureProfile();await this.claimPendingReferral();this.publishSession();return true;}catch{return this.fail('Giriş servisine şu anda ulaşılamıyor.');}
  }
  ...
  async signUp(email:string,password:string,fullName:string):Promise<{created:boolean;confirmationRequired:boolean}>{
    ...
    try{
      ...
      if(payload.access_token&&payload.refresh_token){this.savePayload(payload);await this.ensureProfile();await this.claimPendingReferral();this.publishSession();return{created:true,confirmationRequired:false};}
  ...
  async signInWithProvider(provider:CustomerSocialProvider):Promise<void>{this._lastError.set(null);if(!this.providerEnabled(provider))return;const redirectTo=this.customerCallbackUrl(this.pendingReferral());const url=new URL(supabaseAuthUrl('authorize'));url.searchParams.set('provider',provider);url.searchParams.set('redirect_to',redirectTo);if(provider==='google')url.searchParams.set('prompt','select_account');window.location.assign(url.toString());}
  async resetPassword(email:string):Promise<boolean>{const cleanEmail=email.trim().toLowerCase();if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cleanEmail))return this.fail('Önce geçerli e-posta adresinizi girin.');try{const redirectTo=`${window.location.origin}/account/login`;const response=await fetch(`${supabaseAuthUrl('recover')}?redirect_to=${encodeURIComponent(redirectTo)}`,{method:'POST',headers:this.publicHeaders(),body:JSON.stringify({email:cleanEmail})});if(!response.ok)return this.fail('Parola yenileme e-postası gönderilemedi.');return true;}catch{return this.fail('Parola yenileme servisine ulaşılamıyor.');}}
  ...
  private async initialize():Promise<void>{try{this.captureReferralFromLocation();await this.loadSocialProviders();this.consumeRedirectSession();if(!this.session)this.restoreSession();if(this.session){if(this.session.expiresAt<=Date.now()+60_000)await this.refreshSession();if(this.session){const user=await this.fetchUser(this.session.accessToken);if(user){this.session.user=user;this.persist();await this.ensureProfile();await this.claimPendingReferral();this.publishSession();}else this.clearSession();}}}finally{this._ready.set(true);this.readyResolver();}}
  ...
  private publishSession():void{this._user.set(this.session?.user||null);this._isLoggedIn.set(Boolean(this.session?.user?.id));}private persist():void{if(this.session)localStorage.setItem(this.storageKey,JSON.stringify(this.session));}private clearSession():void{this.session=null;localStorage.removeItem(this.storageKey);this._user.set(null);this._isLoggedIn.set(false);}
  private publicHeaders():Record<string,string>{return{apikey:SUPABASE_PUBLISHABLE_KEY,'content-type':'application/json'};}private userHeaders(token:string):Record<string,string>{return{...this.publicHeaders(),authorization:`Bearer ${token}`};}private fail(message:string):false{this._lastError.set(message);return false;}
