...
export class FinanceService {
  ...
  async refresh(from:string,to:string):Promise<void>{this.loading.set(true);try{const response=await fetch(`${this.endpoint}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,{headers:await this.headers(),cache:'no-store'});const data=await response.json().catch(()=>({}));if(!response.ok||!data.ok)throw new Error(data.code||'FINANCE_READ_FAILED');this.transactions.set(Array.isArray(data.transactions)?data.transactions:[]);this.summary.set(data.summary||{income:0,expense:0,net:0,discount:0,count:0,byCategory:{}});}finally{this.loading.set(false);}}
  ...
  private async action(body:Record<string,unknown>){const response=await fetch(this.endpoint,{method:'POST',headers:await this.headers(),body:JSON.stringify(body),cache:'no-store'});const data=await response.json().catch(()=>({}));if(!response.ok||!data.ok)throw new Error(data.code||'FINANCE_ACTION_FAILED');return data;}
}
