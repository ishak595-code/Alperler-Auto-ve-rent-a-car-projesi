...
export class FinanceService {
  ...
  async refresh(from:string,to:string):Promise<void>{this.loading.set(true);try{const response=await adminFetch(`${this.endpoint}&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,{headers:await this.headers(),cache:'no-store',signal:AbortSignal.timeout(25_000)});const data=await response.json().catch(()=>({}));if(!response.ok||!data.ok)throw new Error(data.code||'FINANCE_READ_FAILED');this.transactions.set(Array.isArray(data.transactions)?data.transactions:[]);this.messageTemplates.set(Array.isArray(data.messageTemplates)?data.messageTemplates:[]);this.notificationProviders.set(this.normalizeProviders(data.notificationProviders));this.summary.set({...EMPTY_SUMMARY,...(data.summary||{}),byCategory:data.summary?.byCategory||{},byPaymentMethod:data.summary?.byPaymentMethod||{},byCurrency:data.summary?.byCurrency||{},receivablesByCurrency:data.summary?.receivablesByCurrency||{}});}finally{this.loading.set(false);}}
  ...
  async downloadPdf(from:string,to:string):Promise<void>{const token=await this.auth.getAccessToken();if(!token)throw new Error('ADMIN_SESSION_REQUIRED');const response=await adminFetch(`/api/finance/report?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`,{headers:{authorization:`Bearer ${token}`},signal:AbortSignal.timeout(30_000)});if(!response.ok)throw new Error('PDF_REPORT_FAILED');const blob=await response.blob();const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download=`alperler-muhasebe-${from.slice(0,10)}-${to.slice(0,10)}.pdf`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}
