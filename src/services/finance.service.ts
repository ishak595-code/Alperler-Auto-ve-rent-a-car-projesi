...
type OfflinePaymentAttempt={fingerprint:string;requestId:string};
...
export class FinanceService {
  ...
  private offlinePaymentAttempt:OfflinePaymentAttempt|null=null;
  ...
  async recordPayment(input:OfflinePaymentInput){
    ...
    try{const result=await this.action({action:'record_payment',...normalized},attempt.requestId);if(this.offlinePaymentAttempt?.requestId===attempt.requestId)this.offlinePaymentAttempt=null;return result;}catch(error){throw error;}
  }
