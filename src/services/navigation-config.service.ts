...
const NAVIGABLE_ROUTE_ROOTS = new Set([
  '', 'fleet', 'sales', 'search', 'campaigns', 'appointment', 'list-your-car',
...
export class NavigationConfigService {
  ...
  readonly mobileDockRendered = computed(() =>
    this._settings().mobileDockEnabled &&
    !this._mobileDockRouteHidden() &&
    !(this._settings().mobileDockAutoHide && this._mobileDockAutoHidden()),
  );
  ...
  itemsFor(surface: NavigationSurface, includeInactive = false): NavigationItem[] {
    return this._items()
      .filter((item) => item.surface === surface && !item.archivedAt && (includeInactive || item.isActive))
      .sort((a, b) => a.sortOrder - b.sortOrder || a.label.localeCompare(b.label, 'tr'));
  }

  archivedItemsFor(surface: NavigationSurface): NavigationItem[] {
    return this._items()
      .filter((item) => item.surface === surface && Boolean(item.archivedAt))
      .sort((a, b) => (b.archivedAt || '').localeCompare(a.archivedAt || ''));
  }

  setMobileDockAutoHidden(hidden: boolean): void {
    this._mobileDockAutoHidden.set(this._settings().mobileDockAutoHide ? hidden : false);
  }

  setMobileDockRouteHidden(hidden: boolean): void {
    this._mobileDockRouteHidden.set(hidden);
    if (hidden) this._mobileDockAutoHidden.set(false);
  }
  ...
  async refreshPublic(): Promise<void> {
    ...
    try {
      ...
      if (settingsResponse.ok) {
        const rows = await settingsResponse.json() as any[];
        const row = rows[0];
        if (row) this.applySettingsRow(row);
      }
      if (itemsResponse.ok) {
  ...
  async refreshAdmin(): Promise<void> {
    ...
    try {
      ...
      if (row) this.applySettingsRow(row);
      else this._settings.set({ ...DEFAULT_SETTINGS });
  ...
  async saveSettings(settings: NavigationSettings): Promise<void> {
    const token = await this.requiredToken();
    await this.rest('PATCH', 'navigation_settings?config_key=eq.main', {
      mobile_dock_enabled: settings.mobileDockEnabled,
      mobile_menu_enabled: settings.mobileMenuEnabled,
      mobile_dock_auto_hide: settings.mobileDockAutoHide,
      updated_at: new Date().toISOString(),
    }, token);
    if (!settings.mobileDockAutoHide) this._mobileDockAutoHidden.set(false);
    await this.refreshAdmin();
  }

  async addPreset(surface: NavigationSurface, preset: NavigationPreset): Promise<void> {
    const token = await this.requiredToken();
    const route = this.safeRoute(preset.route);
    if (!route) throw new Error('Bu hazır butonun bağlantısı uygulamadaki bir sayfayla eşleşmiyor.');
    const existing = this._items().filter((item) => item.surface === surface);
    ...
    if (duplicate) {
      await this.rest('PATCH', `navigation_items?id=eq.${encodeURIComponent(duplicate.id)}`, {
        label: preset.label,
        icon: preset.icon,
        route,
        sort_order: duplicate.archivedAt ? nextSort : duplicate.sortOrder,
        is_active: true,
        archived_at: null,
        updated_at: new Date().toISOString(),
      }, token);
    ...
    await this.rest('POST', 'navigation_items', {
      surface,
      item_key: preset.key,
      label: preset.label,
      icon: preset.icon,
  ...
  async addCustom(surface: NavigationSurface, input: { label: string; icon: string; route: string }): Promise<void> {
    ...
    const icon = input.icon.trim().slice(0, 60) || 'link';
    const route = this.safeRoute(input.route);
    if (!label || !route) throw new Error('Geçerli bir buton adı ve çalışan site içi bağlantı girin.');
  ...
  async updateItem(item: NavigationItem): Promise<void> {
    if (item.archivedAt) throw new Error('Kaldırılmış bir butonu düzenlemeden önce geri yükleyin.');
    const token = await this.requiredToken();
    const route = this.safeRoute(item.route);
    if (!route) throw new Error('Bu bağlantı uygulamadaki geçerli bir müşteri sayfasına gitmiyor.');
    await this.rest('PATCH', `navigation_items?id=eq.${encodeURIComponent(item.id)}`, {
      label: item.label.trim().slice(0, 60),
      icon: item.icon.trim().slice(0, 60) || 'link',
      route,
  ...
  async archiveItem(id: string): Promise<void> {
    const token = await this.requiredToken();
    await this.rest('PATCH', `navigation_items?id=eq.${encodeURIComponent(id)}`, {
      is_active: false,
  ...
  async restoreItem(id: string): Promise<void> {
    const item = this._items().find((candidate) => candidate.id === id && candidate.archivedAt);
    if (!item) throw new Error('Geri yüklenecek buton bulunamadı.');
    const route = this.safeRoute(item.route);
    if (!route) throw new Error('Bu butonun eski bağlantısı artık geçerli değil. Önce aynı butonu hazır seçeneklerden yeniden ekleyin.');
    const token = await this.requiredToken();
    const nextSort = this.itemsFor(item.surface, true).reduce((max, candidate) => Math.max(max, candidate.sortOrder), 0) + 10;
    await this.rest('PATCH', `navigation_items?id=eq.${encodeURIComponent(id)}`, {
      is_active: true,
      archived_at: null,
      sort_order: nextSort,
  ...
  async reorder(surface: NavigationSurface, orderedIds: string[]): Promise<void> {
    ...
    if (orderedIds.some((id) => !allowed.has(id))) throw new Error('Sıralama listesinde başka bir menüye ait buton bulundu.');
    await Promise.all(orderedIds.map((id, index) => this.rest('PATCH', `navigation_items?id=eq.${encodeURIComponent(id)}`, {
  ...
  private queueRefresh(): void {
    ...
    this.refreshTimer = window.setTimeout(() => {
      ...
      if (this.adminViewActive) {
        void this.refreshAdmin().catch(() => {
          this.adminViewActive = false;
          return this.refreshPublic();
        });
      } else {
        void this.refreshPublic();
      }
  ...
  private applySettingsRow(row: any): void {
    const next: NavigationSettings = {
      mobileDockEnabled: row.mobile_dock_enabled !== false,
      mobileMenuEnabled: row.mobile_menu_enabled !== false,
      mobileDockAutoHide: row.mobile_dock_auto_hide !== false,
    };
    this._settings.set(next);
    if (!next.mobileDockAutoHide) this._mobileDockAutoHidden.set(false);
  }

  private fromRow(row: any): NavigationItem {
    return {
      id: String(row.id || ''),
      surface: row.surface as NavigationSurface,
  ...
  private safeRoute(value: string): string {
    ...
    if (!/^\/[A-Za-z0-9_./?#=&%-]*$/.test(route)) return '';
    const path = route.split('?')[0].split('#')[0];
    const root = path.split('/').filter(Boolean)[0] || '';
    return NAVIGABLE_ROUTE_ROOTS.has(root) ? route : '';
  }
  ...
  private async rest<T = unknown>(method: 'GET' | 'POST' | 'PATCH' | 'DELETE', path: string, body: unknown, token: string): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new Error(payload.message || payload.code || `NAVIGATION_${response.status}`);
    }
