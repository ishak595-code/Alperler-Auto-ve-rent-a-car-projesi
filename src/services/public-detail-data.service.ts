...
export class PublicDetailDataService {
  ...
  async load(kind: DetailKind, routeId: string): Promise<Vehicle> {
    const records = kind === "TOUR" ? await this.loadToursDirect() : await this.catalog.loadVehicles(true);
  ...
  async resolveCampaignTarget(targetType?: string, targetId?: string, ctaUrl?: string): Promise<string> {
    ...
    if (targetType === "TOUR" && cleanTargetId) {
      try { const tour = await this.load("TOUR", cleanTargetId); return `/tour/${encodeURIComponent(String(tour.id))}`; } catch { /* fall through */ }
    }
  ...
  mediaUrls(item: Vehicle): string[] {
    ...
    for (const value of [item.image, ...(item.images || []), ...(item.gallery || [])]) {
      const url = this.mediaUrl(value); if (!url || seen.has(url)) continue; seen.add(url); result.push(url);
    }
  ...
  private async loadToursDirect(): Promise<Vehicle[]> {
    const response = await fetch(`${SUPABASE_PROJECT_URL}/rest/v1/tours?is_active=eq.true&publication_status=eq.PUBLISHED&select=*&order=is_featured.desc,updated_at.desc`, {
      method: "GET", cache: "no-store", headers: { apikey: SUPABASE_PUBLISHABLE_KEY, accept: "application/json", "cache-control": "no-cache" },
    });
  ...
  private mapTour(row: Record<string, any>): Vehicle {
    ...
    return {
      ...metadata,
      id: row["id"], category: "TOUR", title: String(row["title"] || ""),
      description: String(row["description"] || row["short_description"] || ""),
      price: Number(row["price_per_person"] || 0), duration: row["duration"] || undefined,
      capacity: Number(row["capacity"] || 0) || undefined, meetingPoint: row["meeting_point"] || undefined,
      ...
      image: row["cover_image"] || images[0] || undefined, images, gallery: images,
      isFeatured: Boolean(row["is_featured"]), isAvailable: published,
      cloudId: row["id"], cloudSlug: row["seo_slug"], publicationStatus: row["publication_status"],
      branchId: row["branch_id"] || undefined, listingOrigin: row["listing_origin"] || undefined,
  ...
  private prepare(item: Vehicle, kind: DetailKind): Vehicle {
    ...
    return {
      ...item, category: kind, price: Number(item.price || 0), image: this.mediaUrl(item.image || images[0]), images, gallery: images, videos,
      transmission: item.transmission || undefined, fuel: item.fuel || undefined, seats: Number(item.seats || 0) || undefined, km: Number(item.km || 0) || undefined,
  ...
  private matches(item: Vehicle, routeId: string): boolean {
    ...
    return [item.id, item.cloudId, item.cloudStockCode, item.cloudSlug].some((value) => value !== undefined && value !== null && String(value) === id);
  }
