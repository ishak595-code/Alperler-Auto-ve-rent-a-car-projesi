...
function json(request: Request, body: unknown, status = 200, id = requestId(request)): Response {
  return Response.json(body, { status, headers: { ...cors(request), "cache-control": "no-store", "x-request-id": id, "x-content-type-options": "nosniff" } });
}
...
async function requireAdmin(request: Request): Promise<AdminSession> {
  ...
  return { id, email: clean(user?.email, 160).toLowerCase(), role, permissions };
}
...
async function resolveGeo(provinceCode: string, districtCode: string): Promise<{ provinceCode: string; districtCode: string; city: string; district: string } | null> {
  ...
  const districtResponse = await db(`geo_districts?code=eq.${encodeURIComponent(districtCode)}&province_code=eq.${encodeURIComponent(provinceCode)}&select=code,name,province_code&limit=1`);
  const provinceResponse = await db(`geo_provinces?code=eq.${encodeURIComponent(provinceCode)}&select=code,name&limit=1`);
...
async function sendAcknowledgement(email: string, reference: string, city: string, district: string): Promise<void> {
  ...
  await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({ from, to: [email], subject: `Alperler Auto şube başvurunuz alındı | ${reference}`, text: `Başvurunuz merkezi sistemimize kaydedildi. Bölge: ${city} / ${district}. Referans: ${reference}. İnceleme tamamlandığında sizinle iletişime geçeceğiz.` }),
...
async function createApplication(request: Request, input: Record<string, unknown>, id: string): Promise<Response> {
  ...
  const clientHash = await sha256(`${clean(request.headers.get("x-client-ip"), 100)}|${clean(request.headers.get("user-agent"), 300)}`);
  const contactHash = await sha256(`${phone}|${email}`);
  if (!(await consumeRateLimit(clientHash, "branch_partner_network_minute", 60, 3)) || !(await consumeRateLimit(clientHash, "branch_partner_network_hour", 3600, 10)) || !(await consumeRateLimit(contactHash, "branch_partner_contact_day", 86400, 4))) {
...
async function linkOwner(branchId: string, row: Record<string, unknown>, admin: AdminSession, request: Request): Promise<{ email: string; membershipLinked: boolean; inviteSent: boolean }> {
  const email = clean(row.email, 160).toLowerCase();
  if (!email) throw new Error("BRANCH_OWNER_EMAIL_REQUIRED");
  const link = async () => {
    const response = await db("rpc/link_branch_owner_by_email", { method: "POST", body: JSON.stringify({ p_branch_id: branchId, p_email: email, p_partner_request_id: row.id, p_actor: admin.id }) });
    if (!response.ok) throw new Error("BRANCH_OWNER_LINK_FAILED");
    return await response.json();
  };

  let linked = await link();
  if (linked?.membershipLinked) return { email, membershipLinked: true, inviteSent: false };
  ...
  if (!invitation.ok) {
    ...
    if (inviteId) await db("rpc/mark_branch_invite_delivery", { method: "POST", body: JSON.stringify({ p_invite_id: inviteId, p_status: "FAILED", p_error: detail.slice(0, 500) }) }).catch(() => null);
    throw new Error("BRANCH_OWNER_INVITE_FAILED");
  }
  if (inviteId) await db("rpc/mark_branch_invite_delivery", { method: "POST", body: JSON.stringify({ p_invite_id: inviteId, p_status: "SENT" }) });
  linked = await link();
  return { email, membershipLinked: Boolean(linked?.membershipLinked), inviteSent: true };
}
...
async function provision(request: Request, input: Record<string, unknown>, id: string): Promise<Response> {
  ...
  const rpc = await db("rpc/provision_branch_partner_request", { method: "POST", body: JSON.stringify({ p_reference: reference, p_actor: admin.id, p_branch_name: clean(input.branchName, 160) || null }) });
  if (!rpc.ok) {
...
async function updateApplication(request: Request, input: Record<string, unknown>, id: string): Promise<Response> {
  ...
  if (!row) return json(request, { ok: false, code: "BRANCH_PARTNER_NOT_FOUND" }, 404, id);
  if (status === "APPROVED" && !row.email) return json(request, { ok: false, code: "BRANCH_OWNER_EMAIL_REQUIRED", message: "Şubeyi onaylamak için aday e-postası zorunludur." }, 409, id);
  const patch: Record<string, unknown> = { status, internal_notes: clean(input.internalNotes, 4000) || null, updated_at: new Date().toISOString() };
  if (status === "APPROVED") { patch.approved_at = new Date().toISOString(); patch.approved_by = admin.id; }
  const response = await db(`branch_partner_requests?reference=eq.${encodeURIComponent(reference)}&select=*`, { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify(patch) });
