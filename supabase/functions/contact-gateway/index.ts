...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const adminMail = adminTo
      ? await sendMail(
          adminTo,
          `Yeni İletişim Mesajı | ${saved.reference}`,
          `<div style="font-family:Arial,sans-serif;color:#0f172a;line-height:1.65"><h2>Yeni iletişim mesajı</h2><p><strong>Referans:</strong> ${esc(saved.reference)}</p><p><strong>Ad Soyad:</strong> ${esc(`${name} ${surname}`)}</p><p><strong>Telefon:</strong> ${esc(phone)}</p><p><strong>E-posta:</strong> ${esc(email)}</p><hr><p>${esc(message).replaceAll("\n", "<br>")}</p>${footer ? `<hr><small>${footer}</small>` : ""}</div>`,
    ...
    const customerMail = await sendMail(
      email,
      `Mesajınız Alındı | ${saved.reference} | Alperler Auto`,
      `<div style="font-family:Arial,sans-serif;color:#0f172a;line-height:1.65"><h2>Mesajınız alındı</h2><p>Sayın ${esc(`${name} ${surname}`)},</p><p>Mesajınız güvenli şekilde kaydedildi. Referans numaranız <strong>${esc(saved.reference)}</strong>. Ekibimiz gerekli olduğunda sizinle iletişime geçecektir.</p>${footer ? `<hr><small>${footer}</small>` : ""}<p><strong>Alperler Auto</strong></p></div>`,
