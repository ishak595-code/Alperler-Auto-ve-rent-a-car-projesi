...
async function list(request: Request) {
  ...
  for (const integration of integrations) {
    if (integration.provider === 'GOOGLE_ADS') integration.runtime_configured = googleReady();
    if (integration.provider === 'META_ADS') integration.runtime_configured = metaReady();
  }
...
async function googlePost(path: string, body: any) {
...
async function publishGoogle(campaignRecord: any): Promise<GooglePublishResult> {
  ...
  const landing = clean(campaignRecord.creative?.landingUrl, 1500);
  const headline = clean(campaignRecord.creative?.headline, 30);
  const headline2 = clean(campaignRecord.creative?.headline2 || 'Aracınızı Hemen Ayırın', 30);
  const description = clean(campaignRecord.creative?.description, 90);
  if (!landing || !headline || !description) throw new Error('GOOGLE_CREATIVE_INCOMPLETE');
  ...
  const budget = await googlePost('campaignBudgets:mutate', {
    operations: [{ create: {
      ...
      amountMicros: String(budgetMicros), deliveryMethod: 'STANDARD', explicitlyShared: false,
    } }],
  });
  ...
  const group = await googlePost('adGroups:mutate', {
    operations: [{ create: {
      name: `${campaignRecord.name} Grup`, campaign: campaignResource, status: 'ENABLED', type: 'SEARCH_STANDARD',
    } }],
  });
  const adGroupResource = group?.results?.[0]?.resourceName;
  if (!adGroupResource) throw new Error('GOOGLE_ADGROUP_FAILED');

  const ad = await googlePost('adGroupAds:mutate', {
    operations: [{ create: {
      ...
      ad: {
        finalUrls: [landing],
        responsiveSearchAd: {
          headlines: [{ text: headline }, { text: headline2 }, { text: 'Alperler Rent A Car' }],
          descriptions: [{ text: description }, { text: 'Yuksekova arac kiralama, transfer, satis ve tur hizmetleri.' }],
        },
      },
    } }],
  });

  return { campaignResource, adGroupResource, adResource: ad?.results?.[0]?.resourceName };
}
...
async function publishMeta(campaignRecord: any): Promise<MetaPublishResult> {
  ...
  const landing = clean(campaignRecord.creative?.landingUrl, 1500);
  const headline = clean(campaignRecord.creative?.headline, 255);
  const message = clean(campaignRecord.creative?.primaryText, 1000);
  const description = clean(campaignRecord.creative?.description, 255);
  if (!landing || !headline || !message) throw new Error('META_CREATIVE_INCOMPLETE');

  const campaign = await metaCall(`act_${account}/campaigns`, {
    name: campaignRecord.name, objective: 'OUTCOME_TRAFFIC', status: 'PAUSED', special_ad_categories: '[]',
  });
  ...
  const creative = await metaCall(`act_${account}/adcreatives`, {
    ...
    object_story_spec: JSON.stringify({
      page_id: page,
      link_data: {
        link: landing, message, name: headline, description,
  ...
  try {
    ...
    if (campaignRecord.provider === 'GOOGLE_ADS') {
      ...
      patch = {
        ...
        external_ad_group_id: googleResult.adGroupResource,
        external_ad_id: googleResult.adResource || null,
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const status = code === 'UNAUTHORIZED' ? 401
      ...
      : code.startsWith('INVALID_') || code.endsWith('_REQUIRED') || code.endsWith('_INCOMPLETE') ? 400
      : 500;
