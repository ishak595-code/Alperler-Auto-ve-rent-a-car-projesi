...
function responseHeaders(request: Request): HeadersInit {
  return {
    ...
    "access-control-allow-headers": "authorization, apikey, content-type, x-client-info, x-alperler-client",
    vary: "Origin",
  };
}
...
function authorizedClient(request: Request): boolean { return request.headers.get("apikey") === PUBLISHABLE_KEY && request.headers.get("x-alperler-client") === CLIENT_MARKER; }
...
async function sendWelcome(subscriber: { id: string; email: string; unsubscribe_token: string }, settings: Record<string, unknown>): Promise<{ state: string; reason?: string; providerMessageId?: string }> {
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { authorization: `Bearer ${key}`, "content-type": "application/json" }, body: JSON.stringify({ from, to: [subscriber.email], subject: "Alperler Rent A Car bültenine hoş geldiniz", html: welcomeHtml(subscriber.email, subscriber.unsubscribe_token, settings), text: "Aboneliğiniz başarıyla alındı. Kampanyalar, yeni araçlar ve tur fırsatları için sizi bilgilendireceğiz. - Alperler Rent A Car" }), signal: AbortSignal.timeout(10_000) });
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    const ip = (request.headers.get("x-forwarded-for")?.split(",")[0] || request.headers.get("cf-connecting-ip") || "unknown").trim().slice(0, 100), ua = (request.headers.get("user-agent") || "unknown").slice(0, 260), networkHash = await digest(`${SERVICE_KEY.slice(0, 24)}|${ip}|${ua}`), emailHash = await digest(email);
    if (!(await consume(networkHash, "newsletter_network", 3600, 20)) || !(await consume(emailHash, "newsletter_email", 86400, 6))) return json(request, { ok: false, code: "RATE_LIMITED" }, 429);
