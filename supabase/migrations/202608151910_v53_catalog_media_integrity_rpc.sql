-- V53 catalog media integrity RPCs
-- These routines keep cover replacement/removal atomic for live catalog items.
...
begin
  ...
  if media_row.vehicle_id is not null then
    ...
    if media_row.kind = 'IMAGE' and media_row.is_active then
      ...
      if live_owner and remaining_images < 1 then
        raise exception using errcode = '23514', message = 'CATALOG_LIVE_LAST_IMAGE_BLOCKED';
      end if;
