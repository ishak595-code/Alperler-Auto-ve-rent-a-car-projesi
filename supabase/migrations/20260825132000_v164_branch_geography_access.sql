-- V164: durable Türkiye geography and branch-owner access lifecycle.
-- Province/district reference data is synchronized by geo-directory and persisted
-- in PostgreSQL, so branch applications remain portable and database-backed.
...
alter table public.branch_partner_requests add constraint branch_partner_requests_province_fk
  foreign key (province_code) references public.geo_provinces(code) on update cascade on delete restrict not valid;
alter table public.branch_partner_requests drop constraint if exists branch_partner_requests_district_pair_fk;
alter table public.branch_partner_requests add constraint branch_partner_requests_district_pair_fk
  foreign key (province_code, district_code) references public.geo_districts(province_code, code) on update cascade on delete restrict not valid;

alter table public.branches drop constraint if exists branches_province_fk;
alter table public.branches add constraint branches_province_fk
  foreign key (province_code) references public.geo_provinces(code) on update cascade on delete restrict not valid;
alter table public.branches drop constraint if exists branches_district_pair_fk;
alter table public.branches add constraint branches_district_pair_fk
  foreign key (province_code, district_code) references public.geo_districts(province_code, code) on update cascade on delete restrict not valid;
...
create policy branch_access_invites_admin_read on public.branch_access_invites for select to authenticated
...
begin
  ...
  select u.id into v_user_id from auth.users u where lower(u.email)=v_email order by u.created_at asc limit 1;

  insert into public.branch_access_invites(branch_id,partner_request_id,email,auth_user_id,role,status,invited_by,invited_at,linked_at,metadata)
  values(p_branch_id,p_partner_request_id,v_email,v_user_id,'BRANCH_OWNER',case when v_user_id is null then 'PENDING' else 'LINKED' end,p_actor,now(),case when v_user_id is null then null else now() end,jsonb_build_object('source','BRANCH_PARTNER_PROVISION','version','V164'))
  on conflict (branch_id,email) where status in ('PENDING','SENT','LINKED','ACCEPTED') do update set
  ...
  if v_user_id is not null then
    ...
    on conflict(branch_id,user_id) do update set role='BRANCH_OWNER',is_active=true,invited_email=v_email,updated_at=now();
  end if;
  return jsonb_build_object('inviteId',v_invite_id,'branchId',p_branch_id,'email',v_email,'userId',v_user_id,'membershipLinked',v_user_id is not null);
end;
$$;
...
create or replace function public.mark_branch_invite_delivery(p_invite_id uuid,p_status text,p_auth_user_id uuid default null,p_error text default null)
returns void language plpgsql security definer set search_path=public,pg_catalog as $$
begin
  if p_status not in ('SENT','LINKED','FAILED','REVOKED') then raise exception using errcode='22023',message='INVALID_BRANCH_INVITE_STATUS'; end if;
  update public.branch_access_invites set status=p_status,auth_user_id=coalesce(p_auth_user_id,auth_user_id),linked_at=case when p_status='LINKED' then coalesce(linked_at,now()) else linked_at end,last_error=left(nullif(p_error,''),500),updated_at=now() where id=p_invite_id;
end;$$;
...
begin
  if new.provisioned_branch_id is not null and (old.provisioned_branch_id is distinct from new.provisioned_branch_id or old.province_code is distinct from new.province_code or old.district_code is distinct from new.district_code) then
    update public.branches set province_code=new.province_code,district_code=new.district_code,city=coalesce((select name from public.geo_provinces where code=new.province_code),new.city,city),district=coalesce((select name from public.geo_districts where code=new.district_code),new.district,district),updated_at=now() where id=new.provisioned_branch_id;
  end if;
  return new;
end;$$;
...
create trigger branch_partner_sync_provisioned_geo after update of provisioned_branch_id,province_code,district_code on public.branch_partner_requests for each row execute function private.sync_provisioned_branch_geo();

comment on table public.geo_provinces is 'Persisted Turkish province directory used by branch/application forms.';
comment on table public.geo_districts is 'Persisted Turkish district directory linked to provinces.';
