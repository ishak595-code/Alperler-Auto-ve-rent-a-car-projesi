...
as $$
  select btrim(regexp_replace(
    translate(lower(coalesce(value, '')), 'çğıöşüâîû', 'cgiosuaiu'),
    '[^a-z0-9]+', ' ', 'g'
  ));
$$;
...
create index if not exists vehicles_public_search_trgm_v217
  on public.vehicles using gin (
    private.search_normalize_v217(concat_ws(' ',
      id::text, stock_code, brand, model, model_year::text, fuel_type, transmission, body_type, color, engine, location, seo_slug,
      metadata->>'series', metadata->>'plate', metadata->>'plateNumber', metadata->>'vehicleNumber', metadata->>'vehicleNo',
      metadata->>'vin', metadata->>'chassisNumber', metadata->>'legacyId', metadata->>'legacy_id'
    )) extensions.gin_trgm_ops
  ) where is_active = true;

create index if not exists tours_public_search_trgm_v217
  on public.tours using gin (
    private.search_normalize_v217(concat_ws(' ', id::text, title, seo_slug, category, short_description, duration, meeting_point, location_name,
      metadata->>'badge', metadata->>'legacyId', metadata->>'legacy_id')) extensions.gin_trgm_ops
  ) where is_active = true;

create index if not exists blog_posts_public_search_trgm_v217
  on public.blog_posts using gin (
    private.search_normalize_v217(concat_ws(' ', id::text, title, slug, excerpt, author_name, seo_title, seo_description,
      metadata->>'legacyId', metadata->>'legacy_id')) extensions.gin_trgm_ops
  ) where status = 'PUBLISHED';

create index if not exists campaigns_public_search_trgm_v217
  on public.campaigns using gin (
    private.search_normalize_v217(concat_ws(' ', id::text, title, slug, short_description, description, badge, campaign_type, cta_label)) extensions.gin_trgm_ops
  ) where is_active = true;

create index if not exists branches_public_search_trgm_v217
  on public.branches using gin (
    private.search_normalize_v217(concat_ws(' ', id::text, name, code, slug, district, city, address_line, territory_label, public_description,
      operator_display_name, operator_legal_name)) extensions.gin_trgm_ops
  ) where is_active = true;

create index if not exists faqs_public_search_trgm_v217
  on public.faqs using gin (
    private.search_normalize_v217(concat_ws(' ', id::text, question, answer, category)) extensions.gin_trgm_ops
  ) where is_active = true;

create index if not exists homepage_sections_public_search_trgm_v217
  on public.homepage_sections using gin (
    private.search_normalize_v217(concat_ws(' ', section_key, title, settings->>'description', settings->>'badge')) extensions.gin_trgm_ops
  ) where is_enabled = true;

create index if not exists navigation_items_public_search_trgm_v217
  on public.navigation_items using gin (
    private.search_normalize_v217(concat_ws(' ', id::text, item_key, label, route)) extensions.gin_trgm_ops
...
with input as (
  ...
  select 'VEHICLE:'||v.id::text result_key,
         case when v.category='SALE' then 'SALE' else 'RENTAL' end kind,
         trim(concat_ws(' ',v.brand,v.model,v.model_year::text)) title,
         coalesce(v.description,v.location,'') summary,
         trim(concat_ws(' · ',v.model_year::text,v.transmission,v.fuel_type,v.location)) meta,
         coalesce(nullif(v.cover_image,''), nullif(v.images->>0,'')) image,
         (case when v.category='SALE' then '/sales/' else '/fleet/' end)||v.id::text route,
         case
           when private.search_normalize_v217(v.id::text)=i.q then 1000
           when private.search_normalize_v217(v.stock_code)=i.q then 990
           when private.search_normalize_v217(v.seo_slug)=i.q then 980
           when private.search_normalize_v217(concat_ws(' ',v.brand,v.model,v.model_year::text))=i.q then 950
           when private.search_normalize_v217(v.stock_code) like i.q||'%' then 920
           when private.search_normalize_v217(concat_ws(' ',v.brand,v.model,v.model_year::text)) like i.q||'%' then 850
           else 500+least(300,round(300*similarity(private.search_normalize_v217(concat_ws(' ',
             v.id::text,v.stock_code,v.brand,v.model,v.model_year::text,v.fuel_type,v.transmission,v.body_type,v.color,v.engine,v.location,v.seo_slug,
             v.metadata->>'series',v.metadata->>'plate',v.metadata->>'plateNumber',v.metadata->>'vehicleNumber',v.metadata->>'vehicleNo',
             v.metadata->>'vin',v.metadata->>'chassisNumber',v.metadata->>'legacyId',v.metadata->>'legacy_id')),i.q))::int) end score
