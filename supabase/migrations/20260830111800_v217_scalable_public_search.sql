...
as $$
declare
  v_q text := private.search_normalize_v217(left(coalesce(p_query,''),120));
...
begin
  ...
  with vehicle_rows as (
    select 'VEHICLE:'||v.id::text,case when v.category='SALE' then 'SALE' else 'RENTAL' end,
      ...
      case when private.search_normalize_v217(v.id::text)=v_q then 1000 when private.search_normalize_v217(v.stock_code)=v_q then 990 when private.search_normalize_v217(v.seo_slug)=v_q then 980 when private.search_join_v217(v.brand,v.model,v.model_year::text)=v_q then 950 when private.search_normalize_v217(v.stock_code) like v_q||'%' then 920 when private.search_join_v217(v.brand,v.model,v.model_year::text) like v_q||'%' then 850 else 500+least(300,round(300*similarity(private.search_join_v217(v.id::text,v.stock_code,v.brand,v.model,v.model_year::text,v.fuel_type,v.transmission,v.body_type,v.color,v.engine,v.location,v.seo_slug,v.metadata->>'series',v.metadata->>'plate',v.metadata->>'plateNumber',v.metadata->>'vehicleNumber',v.metadata->>'vehicleNo',v.metadata->>'vin',v.metadata->>'chassisNumber',v.metadata->>'legacyId',v.metadata->>'legacy_id'),v_q))::int) end
    from public.vehicles v
    where (p_kinds is null or (case when v.category='SALE' then 'SALE' else 'RENTAL' end)=any(p_kinds))
  ...
  tour_rows as (
    select 'TOUR:'||t.id::text,'TOUR',t.title,coalesce(t.short_description,t.description,''),trim(concat_ws(' · ',t.duration,t.location_name)),t.cover_image,'/tour/'||coalesce(nullif(t.seo_slug,''),t.id::text),
      case when private.search_normalize_v217(t.id::text)=v_q then 1000 when private.search_normalize_v217(t.seo_slug)=v_q then 980 when private.search_normalize_v217(t.title)=v_q then 950 when private.search_normalize_v217(t.title) like v_q||'%' then 850 else 500+least(300,round(300*similarity(private.search_join_v217(t.id::text,t.title,t.seo_slug,t.category,t.short_description,t.duration,t.meeting_point,t.location_name,t.metadata->>'badge',t.metadata->>'legacyId',t.metadata->>'legacy_id'),v_q))::int) end
    from public.tours t
    where (p_kinds is null or 'TOUR'=any(p_kinds)) and private.search_join_v217(t.id::text,t.title,t.seo_slug,t.category,t.short_description,t.duration,t.meeting_point,t.location_name,t.metadata->>'badge',t.metadata->>'legacyId',t.metadata->>'legacy_id') like '%'||v_q||'%'
  ),
  blog_rows as (
    select 'BLOG:'||b.id::text,'BLOG',b.title,coalesce(b.excerpt,''),coalesce(b.author_name,'Blog'),b.cover_image,'/blog/'||coalesce(nullif(b.slug,''),b.id::text),
      case when private.search_normalize_v217(b.id::text)=v_q then 1000 when private.search_normalize_v217(b.slug)=v_q then 980 when private.search_normalize_v217(b.title)=v_q then 950 when private.search_normalize_v217(b.title) like v_q||'%' then 850 else 500+least(300,round(300*similarity(private.search_join_v217(b.id::text,b.title,b.slug,b.excerpt,b.author_name,b.seo_title,b.seo_description,b.metadata->>'legacyId',b.metadata->>'legacy_id'),v_q))::int) end
    from public.blog_posts b
    where (p_kinds is null or 'BLOG'=any(p_kinds)) and private.search_join_v217(b.id::text,b.title,b.slug,b.excerpt,b.author_name,b.seo_title,b.seo_description,b.metadata->>'legacyId',b.metadata->>'legacy_id') like '%'||v_q||'%'
  ),
  campaign_rows as (
    select 'CAMPAIGN:'||c.id::text,'CAMPAIGN',c.title,coalesce(c.short_description,c.description,''),coalesce(c.badge,'Kampanya'),c.cover_image,coalesce(nullif(c.cta_url,''),'/campaigns?campaign='||c.id::text),
      case when private.search_normalize_v217(c.id::text)=v_q then 1000 when private.search_normalize_v217(c.slug)=v_q then 980 when private.search_normalize_v217(c.title)=v_q then 950 when private.search_normalize_v217(c.title) like v_q||'%' then 850 else 500+least(300,round(300*similarity(private.search_join_v217(c.id::text,c.title,c.slug,c.short_description,c.description,c.badge,c.campaign_type,c.cta_label),v_q))::int) end
    from public.campaigns c
    where (p_kinds is null or 'CAMPAIGN'=any(p_kinds)) and private.search_join_v217(c.id::text,c.title,c.slug,c.short_description,c.description,c.badge,c.campaign_type,c.cta_label) like '%'||v_q||'%'
  ),
  branch_rows as (
    select 'BRANCH:'||br.id::text,'BRANCH',br.name,coalesce(br.public_description,br.address_line,''),trim(concat_ws(' / ',br.city,br.district)),br.hero_image,case when nullif(br.slug,'') is null then '/branches' else '/branches/'||br.slug end,
      case when private.search_normalize_v217(br.id::text)=v_q then 1000 when private.search_normalize_v217(br.slug)=v_q then 980 when private.search_normalize_v217(br.name)=v_q then 950 when private.search_normalize_v217(br.name) like v_q||'%' then 850 else 500+least(300,round(300*similarity(private.search_join_v217(br.id::text,br.name,br.code,br.slug,br.district,br.city,br.address_line,br.territory_label,br.public_description,br.operator_display_name,br.operator_legal_name),v_q))::int) end
    from public.branches br
    where (p_kinds is null or 'BRANCH'=any(p_kinds)) and private.search_join_v217(br.id::text,br.name,br.code,br.slug,br.district,br.city,br.address_line,br.territory_label,br.public_description,br.operator_display_name,br.operator_legal_name) like '%'||v_q||'%'
  ),
  faq_rows as (
    select 'FAQ:'||f.id::text,'FAQ',f.question,f.answer,coalesce(f.category,'Sık Sorulan Sorular'),null::text,'/faq',case when private.search_normalize_v217(f.question)=v_q then 950 when private.search_normalize_v217(f.question) like v_q||'%' then 850 else 500+least(300,round(300*similarity(private.search_join_v217(f.id::text,f.question,f.answer,f.category),v_q))::int) end
    from public.faqs f where (p_kinds is null or 'FAQ'=any(p_kinds)) and private.search_join_v217(f.id::text,f.question,f.answer,f.category) like '%'||v_q||'%'
  ),
  section_rows as (
    select 'SECTION:'||s.section_key,'SECTION',s.title,coalesce(s.settings->>'description',''),'Ana sayfa bölümü',null::text,coalesce(nullif(s.settings->>'viewAllUrl',''),'/'),case when private.search_normalize_v217(s.title)=v_q then 950 when private.search_normalize_v217(s.title) like v_q||'%' then 850 else 500+least(300,round(300*similarity(private.search_join_v217(s.section_key,s.title,s.settings->>'description',s.settings->>'badge'),v_q))::int) end
    from public.homepage_sections s where (p_kinds is null or 'SECTION'=any(p_kinds)) and private.search_join_v217(s.section_key,s.title,s.settings->>'description',s.settings->>'badge') like '%'||v_q||'%'
  ),
  page_rows as (
    select 'PAGE:'||n.id::text,'PAGE',n.label,'Alperler Rent A Car hizmet sayfası','Hizmet',null::text,n.route,case when private.search_normalize_v217(n.label)=v_q then 950 when private.search_normalize_v217(n.label) like v_q||'%' then 850 else 500+least(300,round(300*similarity(private.search_join_v217(n.id::text,n.item_key,n.label,n.route),v_q))::int) end
    from public.navigation_items n where (p_kinds is null or 'PAGE'=any(p_kinds)) and private.search_join_v217(n.id::text,n.item_key,n.label,n.route) like '%'||v_q||'%'
  ),
