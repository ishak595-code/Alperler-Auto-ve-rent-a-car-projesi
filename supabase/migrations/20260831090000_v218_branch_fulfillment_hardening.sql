...
begin
  ...
  if new.vehicle_id is not null then
    select v.branch_id into resolved_branch
    from public.vehicles v
    where v.id = new.vehicle_id;
  elsif new.tour_id is not null then
    select t.branch_id into resolved_branch
    from public.tours t
...
on public.bookings
for each row
execute function public.assign_booking_fulfillment_branch();

create index if not exists bookings_fulfillment_branch_idx
  on public.bookings (fulfillment_branch_id, status, created_at desc);
